...
function draw_meter(name, xx, yy, wid, value, minval, maxval, def, snapval, tbx, script, tip = "")
{
	...
	var draggerwid = dragger_width; //(compact ? dragger_width : 80);
	
	draw_dragger(name + "input", (xx + wid - draggerwid), yy, draggerwid, value, snapval * .1, minval, maxval, def, snapval, tbx, script, null, false) // x: + (!compact * 6)
	...
	if (window_busy = name)
	{
		...
		if (app.setting_unlimited_values)
			meter_drag_value = (minval + (mouse_x - linex) * (max(1, (maxval - minval)) / linewid))
		else
