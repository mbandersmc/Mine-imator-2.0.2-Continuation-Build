...
function toast_draw(toast)
{
	...
	// Mouse moved off, refresh countdown
	if (!content_mouseon && toast.mouseon)
		toast.time_created = current_time
	
	if (content_mouseon)
		toast_mouseon = true
	
	toast.mouseon = content_mouseon
	...
	// Dismiss bar
	if (!toast.mouseon && toast.dismiss_time != no_limit)
	...
	// Time out dismiss
	if (!toast.mouseon && toast.dismiss_time != no_limit && ((current_time - toast.time_created) > (toast.dismiss_time * 1000)))
		toast.remove = true
