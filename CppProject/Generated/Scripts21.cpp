...
namespace CppProject
{
		...
		withOne (obj_timeline, (new obj_timeline)->id, self->id)
		{
			...
			else
			{
				...
				if (is_string(idVar(self.otherId, variant)))
					state_vars_add(VarType::CreateRef(self->block_state), ArrType::From({ /*"variant"*/ STR(790), idVar(self.otherId, variant) }));
				tl_update_scenery_part(ScopeAny(self));
	...
	RealType block_generate_liquid(ScopeAny self, RealType waterlogged)
	{
		...
		if (idVar(sInt(block_current), name) == /*"water"*/ STR(792))
		...
		slot = DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_liquid_slot_map).Value(/*"flowing_"*/ STR(793) + idVar(sInt(block_current), name));
	...
	IntType block_get_timeline(Scope<obj_builder_thread> self, VarType block, VarType stateid)
	{
		...
		withOne (obj_block_tl, (new obj_block_tl)->id, self->id)
		{
			...
			self->variant = block_get_state_id_value(block, stateid, /*"variant"*/ STR(790));
			return self->id;
		}
	...
	IntType block_load(VarType map, IntType typemap)
	{
		withOne (obj_block, (new obj_block)->id, noone)
		{
			...
			if (dev_mode_debug_names && !text_exists(/*"block"*/ STR(4) + self->name))
				log({ /*"block/"*/ STR(796) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"type"*/ STR(780))))
			{
				self->type = DsMap(map).Value(/*"type"*/ STR(780));
				self->set_script = asset_get_index(/*"block_set_"*/ STR(797) + self->type);
				self->generate_script = asset_get_index(/*"block_generate_"*/ STR(798) + self->type);
			}
			else
			{
				self->type = /*""*/ STR(0);
			...
			if (is_real(DsMap(map).Value(/*"emissive"*/ STR(799))))
				self->emissive = DsMap(map).Value(/*"emissive"*/ STR(799));
			else
				self->emissive = IntType(0);
			
			if (is_bool(DsMap(map).Value(/*"random_offset"*/ STR(800))))
				self->random_offset = DsMap(map).Value(/*"random_offset"*/ STR(800));
			else
				self->random_offset = false;
			
			if (is_bool(DsMap(map).Value(/*"random_offset_xz"*/ STR(801))))
				self->random_offset_xy = DsMap(map).Value(/*"random_offset_xz"*/ STR(801));
			else
			...
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(802))))
			{
				...
				VarType curstate;
				curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(802))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"blockstate"*/ STR(771) + curstate))
						log({ /*"block/state/"*/ STR(803) + curstate + dev_mode_name_translation_message });
					withOne (obj_block_state, (new obj_block_state)->id, self->id)
					{
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							if (ds_map_valid(curvalue))
							{
								...
								if (is_real(DsMap(curvalue).Value(/*"emissive"*/ STR(799))))
									self->value_emissive[v] = DsMap(curvalue).Value(/*"emissive"*/ STR(799));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset"*/ STR(800))))
									self->value_random_offset[v] = DsMap(curvalue).Value(/*"random_offset"*/ STR(800));
								if (is_bool(DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(801))))
									self->value_random_offset_xy[v] = DsMap(curvalue).Value(/*"random_offset_xz"*/ STR(801));
							}
							...
							if (dev_mode_debug_names && string_length(self->value_name.Value(v)) > IntType(3) && !text_exists(/*"blockstatevalue"*/ STR(5) + self->value_name.Value(v)))
								log({ /*"block/state/value/"*/ STR(805) + self->value_name.Value(v) + dev_mode_name_translation_message });
						}
			...
			if (ds_map_valid(windmap))
			{
				...
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(809))))
					self->wind_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(809));
			}
			if (is_bool(DsMap(map).Value(/*"waterlogged"*/ STR(810))))
				self->waterlogged = DsMap(map).Value(/*"waterlogged"*/ STR(810));
			else
				self->waterlogged = false;
			
			if (is_bool(DsMap(map).Value(/*"require_models"*/ STR(811))))
				self->require_models = DsMap(map).Value(/*"require_models"*/ STR(811));
			else
			...
			if (is_bool(DsMap(map).Value(/*"multithreaded"*/ STR(812))))
				self->multithreaded = DsMap(map).Value(/*"multithreaded"*/ STR(812));
			else
			...
			if (is_string(idmap))
			{
				self->mc_ids = ArrType::From({ string_replace(idmap, /*"minecraft:"*/ STR(813), /*""*/ STR(0)) });
				...
				if (ds_map_valid(idmap))
				{
					...
					while (!is_undefined(key))
					{
						array_add(VarType::CreateRef(self->mc_ids), string_replace(key, /*"minecraft:"*/ STR(813), /*""*/ STR(0)));
