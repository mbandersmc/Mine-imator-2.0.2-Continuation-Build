...
namespace CppProject
{
	RealType popup_pattern_editor_draw_layer(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, VarType index, RealType base)
	{
		...
		if (ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_edit_preview == null_)
			return IntType(0);
		editname = /*"patterneditoreditlayer"*/ STR(2081) + string(index);
		...
		layercolor = ((base > 0) ? minecraft_get_color(/*"dye:black"*/ STR(2082)) : DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_color_list_edit).Value(index));
		if (ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_edit_preview != null_)
		...
		if (base > 0)
		{
			...
			name = text_get({ /*"patterneditorbase"*/ STR(2083), text_get({ /*"swatchdye"*/ STR(2084) + colorname }) });
		}
		else
		{
			...
			name = text_get({ /*"patterneditorlayer"*/ STR(2085), text_get({ /*"swatchdye"*/ STR(2084) + colorname }), text_get({ /*"patterneditorpatterns"*/ STR(2086) + DsList(ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_list_edit).Value(index) }) });
		}
		...
		if (!(base > 0))
		{
			...
			if (draghover)
			{
				...
				if (sReal(mouse_move) > IntType(0) && !dragging)
				{
					...
					sVar(window_busy) = /*"patterneditordraglayer"*/ STR(2087);
				}
			}
			...
			if (mouseon && !dragging)
			{
				if (draw_button_icon(self, { /*"patterneditorremovelayer"*/ STR(2088) + string(index), actionx - IntType(24), yy + height / 2.0 - IntType(14), IntType(24), IntType(24), false, icons_DELETE }))
		...
		if (mouseon && !dragging)
		{
			...
			if (sStr(settings_menu_name) == editname && sStr(settings_menu_ani_type) != /*"hide"*/ STR(725))
	...
	{
		draw_label(self, { text_get({ /*"patterneditorcolors"*/ STR(2089) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(18);
		VarType colorsx;
		RealType size;
		colorsx = sVar(dx);
		...
		{
			if (draw_button_swatch(self, colorsx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1702) + ObjType(obj_swatch, global::minecraft_swatch_dyes)->name + ObjType(obj_swatch, global::minecraft_swatch_dyes)->color_names.Value(c), ObjType(obj_swatch, global::minecraft_swatch_dyes)->colors.Value(c)))
			{
		...
		{
			sVar(dy) += IntType(8);
			draw_label(self, { text_get({ /*"patterneditorpatterns"*/ STR(2086) }), sVar(dx), sVar(dy) + IntType(4), fa_left, fa_middle, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(18);
			...
			for (; p < ds_list_size(global::minecraft_pattern_list); p++)
			{
				...
				if (draw_button_icon(self, { /*"patterneditorpatterns"*/ STR(2086) + string(p), patternsx, sVar(dy), IntType(20), IntType(40), active, null_, null_, false, /*"patterneditorpatterns"*/ STR(2086) + DsList(global::minecraft_pattern_list).Value(p), ObjType(obj_popup, sInt(popup_pattern_editor))->pattern_sprites.Value(p) }))
				{
	...
	void popup_pattern_editor_show(ScopeAny self, VarType obj)
	{
		withOne (obj_popup, sInt(popup_pattern_editor), self->id)
		{
			...
			if (self->pattern_resource != res || array_length(VarType::CreateRef(self->pattern_sprites)) == IntType(0))
			{
				...
				self->res_ratio = ceil((RealType)sprite_get_width((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1816)))) / sprite_get_width((IntType)(DsMap(ObjType(obj_resource, global::mc_res)->model_texture_map).Value(/*"entity/banner_base"*/ STR(1816)))));
				...
				for (IntType i = IntType(0); i < ds_list_size(global::minecraft_pattern_list); i++)
				{
					...
					array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner/"*/ STR(1818) + bannername), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
				}
				global::shader_mask = false;
				array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1816)), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2090), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2090), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"newprojectname"*/ STR(2070), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2090), sVar(project_name) }), /*"top"*/ STR(196)) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2090), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2075), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2076) }))
		{
		...
		if (draw_button_label(self, /*"saveassave"*/ STR(2091), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2092), idVar(sInt(popup), name) });
	...
	RealType popup_switch(ScopeAny self, IntType argument0)
	{
		...
		sVar(window_busy) = /*"popup"*/ STR(1611) + idVar(sInt(popup_switch_to), name);
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		pagealpha = ease(/*"easeoutcirc"*/ STR(574), idReal(sInt(popup), page_ani));
		...
		if (idStr(sInt(popup), page_ani_type) == /*"left"*/ STR(197))
			pageoff -= 16.0;
		else
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2093), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) - IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
			idStr(sInt(popup), page_ani_type) = /*"left"*/ STR(197);
		}
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2094), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
			idStr(sInt(popup), page_ani_type) = /*"right"*/ STR(198);
		}
		sVar(dy) += IntType(14);
		draw_set_font(sInt(font_caption));
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2095) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2096) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(111), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2097), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2098), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2099) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, /*"upgradecontinue"*/ STR(2100), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2101);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2102) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, /*"welcomecontinue"*/ STR(2103), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2104) : /*"previewmove"*/ STR(2105));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2104))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2105))
			{
				sInt(mouse_cursor) = cr_size_all;
		...
		withOne (Object, preview, self->id)
		{
			...
			if (isplaying > 0)
				sBool(update) = true;
			if (idVar(sVar(select), type) == e_res_type_PACK && sStr(pack_image) == /*"blocksheet"*/ STR(546) && sReal(pack_block_sheet_ani) > 0)
				sBool(update) = true;
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uFogShow"*/ STR(1945), IntType(0));
					render_set_uniform_int(/*"uColorsExt"*/ STR(2106), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2107), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2108), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2109), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2110), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2111), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2112), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2113), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2114), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2115) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2114), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2115) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2116), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2117) : /*"tooltipplay"*/ STR(2118) }))
			{
				if (isplaying > 0)
				{
