...
namespace CppProject
{
	...
	BoolType json_load_word()
	{
		if (global::json_char == e_json_char_T)
		{
			...
			if (global::json_char == e_json_char_F)
			{
				...
				{
					global::json_error = /*"Unrecognized word"*/ STR(1227);
					return false;
				}
	...
	void json_save_var_bool(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		buffer_write_string((val > 0) ? /*"true"*/ STR(751) : /*"false"*/ STR(752));
		global::json_add_comma = true;
	}
	...
	void json_save_var_color(VarType name, VarType val)
	{
		...
		buffer_write_byte(e_json_char_COLON);
		buffer_write_byte(e_json_char_SPACE);
		json_save_value(/*"#"*/ STR(834) + color_to_hex(val));
		global::json_add_comma = true;
	}
	...
	BoolType keybind_check(VarType keybindobj, StringType checkscript)
	{
		...
		if (checkscript == /*"keyboard_check"*/ STR(680))
		{
	...
	RealType keyframes_save(ScopeAny self)
	{
		...
		log({ /*"Saving keyframes"*/ STR(1243), fn });
	...
	BoolType key_valid(VarType key)
	{
		...
		keystr = /*"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"*/ STR(1245);
	...
	void langauge_new(StringType fn)
	{
		...
		withOne (obj_language, obj, noone)
		{
			...
			self->name = text_get({ /*"filelanguage"*/ STR(1246) });
			self->locale = text_exists(/*"filelocale"*/ STR(1247)) ? string(text_get({ /*"filelocale"*/ STR(1247) })) : /*""*/ STR(0);
		}
	...
	RealType languages_load()
	{
		...
		if (!file_exists_lib(fn))
			return IntType(0);
		log({ /*"Loading languages"*/ STR(1248), fn });
		...
		VarType list, obj;
		list = DsMap(map).Value(/*"languages"*/ STR(1250));
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			map = DsList(list).Value(i);
			VarType name, locale, fn;
			name = value_get_string(DsMap(map).Value(/*"name"*/ STR(794)), /*""*/ STR(0));
			locale = value_get_string(DsMap(map).Value(/*"locale"*/ STR(1251)), /*""*/ STR(0));
			fn = value_get_string(DsMap(map).Value(/*"filename"*/ STR(1252)), /*""*/ STR(0));
			language_remove(fn);
			IntType obj = (new obj_language)->id;
	...
	void languages_save()
	{
		log({ /*"Saving languages"*/ STR(1253), languages_file });
		json_save_start(languages_file);
		json_save_object_start();
		json_save_var(/*"format"*/ STR(92), settings_format);
		json_save_array_start({ /*"languages"*/ STR(1250) });
		withAll (obj_language, noone)
		{
			json_save_object_start();
			json_save_var(/*"name"*/ STR(794), json_string_encode(self->name));
			json_save_var(/*"locale"*/ STR(1251), self->locale);
			json_save_var(/*"filename"*/ STR(1252), self->filename);
			json_save_object_done();
		}
	...
	RealType language_add(ScopeAny self)
	{
		...
		if (file_exists_lib(languages_directory + filename_name(fn)))
			if (!question(/*"This file already exists the language file directory. Do you want to replace the file?"*/ STR(1255)))
				return IntType(0);
	...
	{
		log({ /*"Loading language file"*/ STR(1256), fn });
		ds_map_clear(map);
		if (filename_ext(fn) == /*".milanguage"*/ STR(1257))
		{
			StringType convfn = file_directory + /*"conv.tmp"*/ STR(1258);
			json_file_convert_unicode(fn, convfn);
			if (!file_exists_lib(convfn))
			{
				StringType msg = /*"An error occurred while reading the language file:\nCould not convert."*/ STR(1259);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1260));
				show_message(msg);
			...
			{
				StringType msg = /*"An error occured while reading the language file:\n"*/ STR(1261) + global::json_error + /*" on line "*/ STR(1219) + string(global::json_line) + /*", column "*/ STR(1220) + string(global::json_column);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1260));
				show_message(msg);
				window_set_caption(/*""*/ STR(0));
		...
		if (!reload && map != global::language_english_map)
		{
			...
			if (ds_list_size(missingkeyslist) > IntType(0))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(missingkeyslist); i++)
					msg += DsList(missingkeyslist).Value(i) + /*": "*/ STR(1031) + string_replace_all(DsMap(global::language_english_map).Value(DsList(missingkeyslist).Value(i)), /*"\n"*/ STR(948), /*"\\n"*/ STR(1238)) + /*"\n"*/ STR(948);
				log({ msg });
				window_set_caption(/*"Error"*/ STR(1260));
	...
	RealType language_load_legacy(VarType fn, IntType map)
	{
		...
		while (!file_text_eof(f))
		{
			...
			commapos = string_pos(/*":"*/ STR(763), line);
	...
	BoolType legacy_startup()
	{
		...
		log({ /*"Loading legacy file"*/ STR(1265), legacy_file });
		global::legacy_model_id_05_map = DsMap(map).Value(/*"legacy_model_id_05"*/ STR(1266));
		global::legacy_model_id_06_map = DsMap(map).Value(/*"legacy_model_id_06"*/ STR(1267));
		global::legacy_model_id_100_demo_map = DsMap(map).Value(/*"legacy_model_id_100_demo"*/ STR(1268));
		ds_map_merge(global::legacy_model_id_05_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1269)));
		ds_map_merge(global::legacy_model_id_06_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1269)));
		ds_map_merge(global::legacy_model_id_100_demo_map, DsMap(map).Value(/*"legacy_model_id"*/ STR(1269)));
		global::legacy_model_part_map = DsMap(map).Value(/*"legacy_model_part"*/ STR(1270));
		global::legacy_model_name_map = DsMap(map).Value(/*"legacy_model_name"*/ STR(1271));
		global::legacy_block_set[IntType(255)] = false;
		global::legacy_block_id = DsMap(map).Value(/*"legacy_block_id"*/ STR(1272));
		global::legacy_block_texture_name_map = DsMap(map).Value(/*"legacy_block_texture_name"*/ STR(1273));
		global::legacy_block_05_texture_list = DsMap(map).Value(/*"legacy_block_05_textures"*/ STR(1274));
		global::legacy_block_07_demo_texture_list = DsMap(map).Value(/*"legacy_block_07_demo_textures"*/ STR(1275));
		global::legacy_block_100_texture_list = DsMap(map).Value(/*"legacy_block_100_textures"*/ STR(1276));
		...
		global::legacy_item_texture_name_map = DsMap(map).Value(/*"legacy_item_texture_name"*/ STR(1277));
		global::legacy_biomes_map = DsMap(map).Value(/*"legacy_biomes"*/ STR(1278));
		global::legacy_biomes_ids_map = ds_int_map_create();
		VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1279))));
		while (!is_undefined(key))
		{
			DsMap(global::legacy_biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1279))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"legacy_biome_ids"*/ STR(1279))), key);
		}
		
		global::biomes_ids_map = ds_int_map_create();
		key = ds_map_find_first((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1280))));
		while (!is_undefined(key))
		{
			DsMap(global::biomes_ids_map)[string_get_real(key)] = ds_map_find_value((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1280))), key);
			key = ds_map_find_next((IntType)(DsMap(map).Value(/*"biome_ids"*/ STR(1280))), key);
		}
		
		global::legacy_model_names_map = DsMap(map).Value(/*"legacy_model_names"*/ STR(1281));
		global::legacy_model_states_map = DsMap(map).Value(/*"legacy_model_states"*/ STR(1282));
		global::legacy_model_state_values_map = DsMap(map).Value(/*"legacy_model_state_values"*/ STR(1283));
		global::legacy_block_names_map = DsMap(map).Value(/*"legacy_block_names"*/ STR(1284));
		global::legacy_particles_map = DsMap(map).Value(/*"legacy_particles"*/ STR(1285));
		return true;
	}
	...
	BoolType lib_startup()
	{
		...
		if (is_cpp())
			return true;
		log({ /*"External library init"*/ STR(1286) });
		StringType libpath, pathfile, pathmovie, pathwindow;
		libpath = /*"Data/Libraries/"*/ STR(1287);
		pathfile = libpath + /*"file.dll"*/ STR(1288);
		pathmovie = libpath + /*"movie.dll"*/ STR(1289);
		pathwindow = libpath + /*"window.dll"*/ STR(1290);
		...
		log({ /*"External library"*/ STR(1291), pathwindow });
		global::lib_window_maximize = external_define({ pathwindow, /*"window_maximize"*/ STR(1292), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_window_set_focus = external_define({ pathwindow, /*"window_set_focus"*/ STR(1293), dll_cdecl, ty_real, IntType(1), ty_string });
		log({ /*"External library"*/ STR(1291), pathfile });
		global::file_copy_temp = true;
		global::lib_open_url = external_define({ pathfile, /*"open_url"*/ STR(1294), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_execute = external_define({ pathfile, /*"execute"*/ STR(1121), dll_cdecl, ty_real, IntType(3), ty_string, ty_string, ty_real });
		global::lib_unzip = external_define({ pathfile, /*"unzip"*/ STR(1295), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_gzunzip = external_define({ pathfile, /*"gzunzip"*/ STR(1296), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_rename = external_define({ pathfile, /*"file_rename"*/ STR(1297), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_copy = external_define({ pathfile, /*"file_copy"*/ STR(1298), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_file_delete = external_define({ pathfile, /*"file_delete"*/ STR(1299), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_file_exists = external_define({ pathfile, /*"file_exists"*/ STR(1300), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_create = external_define({ pathfile, /*"directory_create"*/ STR(1301), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_delete = external_define({ pathfile, /*"directory_delete"*/ STR(1302), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_directory_exists = external_define({ pathfile, /*"directory_exists"*/ STR(1303), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_json_file_convert_unicode = external_define({ pathfile, /*"json_file_convert_unicode"*/ STR(1304), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		log({ /*"External library"*/ STR(1291), pathmovie });
		global::lib_movie_init = external_define({ pathmovie, /*"movie_init"*/ STR(1305), dll_cdecl, ty_real, IntType(0) });
		global::lib_movie_set = external_define({ pathmovie, /*"movie_set"*/ STR(1306), dll_cdecl, ty_real, IntType(5), ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_start = external_define({ pathmovie, /*"movie_start"*/ STR(1307), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_decode = external_define({ pathmovie, /*"movie_audio_file_decode"*/ STR(1308), dll_cdecl, ty_real, IntType(2), ty_string, ty_string });
		global::lib_movie_audio_file_add = external_define({ pathmovie, /*"movie_audio_file_add"*/ STR(1309), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_audio_sound_add = external_define({ pathmovie, /*"movie_audio_sound_add"*/ STR(1310), dll_cdecl, ty_real, IntType(6), ty_real, ty_real, ty_real, ty_real, ty_real, ty_real });
		global::lib_movie_frame = external_define({ pathmovie, /*"movie_frame"*/ STR(1311), dll_cdecl, ty_real, IntType(1), ty_string });
		global::lib_movie_done = external_define({ pathmovie, /*"movie_done"*/ STR(1312), dll_cdecl, ty_real, IntType(0) });
		log({ /*"External library"*/ STR(1291), /*"movie init"*/ STR(1313) });
		lib_movie_init();
	...
	IntType list_init(ScopeAny self, VarType name)
	{
		...
		{
			case 739: // benchskin
			case 740: // benchskinmaterial
			case 741: // benchskinnormal
			case 743: // benchspblocktex
			case 744: // benchspblocktexmaterial
			case 745: // benchspblocktexnormal
			case 746: // benchbodypartskin
			case 747: // benchbodypartskinmaterial
			case 748: // benchbodypartskinnormal
			case 1314: // libraryskin
			case 1315: // libraryskinmaterial
			case 1316: // libraryskinnormal
			case 1317: // libraryspblocktex
			case 1318: // libraryspblocktexmaterial
			case 1319: // libraryspblocktexnormal
			case 1320: // librarybodypartskin
			case 1321: // librarybodypartskinmaterial
			case 1322: // librarybodypartskinnormal
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(727)))
					temp = sInt(bench_settings);
				else
					temp = global::temp_edit;
				
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				if (idVar(temp, model_file) != null_ && idVar(idVar(temp, model_file), player_skin) > 0 && (name == /*"benchskin"*/ STR(739) || name == /*"libraryskin"*/ STR(1314)))
					menu_add_item({ e_option_DOWNLOAD_SKIN, text_get({ /*"libraryskindownload"*/ STR(1324) }), null_, icons_DOWNLOAD });
				VarType tex;
				withOne (obj_resource, global::mc_res, self->id)
				{
					if (string_contains(name, /*"material"*/ STR(617)))
						...
						if (string_contains(name, /*"normal"*/ STR(606)))
			...
			case 784: // benchmodeltex
			case 1325: // librarymodeltex
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(727)))
					temp = sInt(bench_settings);
				...
				if (texobj != null_)
				{
					...
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }), tex });
				}
				else
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), text_get({ /*"listnone"*/ STR(762) }) }) });
			...
			case 785: // benchmodeltexmaterial
			case 1326: // librarymodeltexmaterial
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(727)))
					temp = sInt(bench_settings);
			...
			case 786: // benchmodeltexnormal
			case 1327: // librarymodeltexnormal
			{
				VarType temp;
				if (string_contains(ObjType(obj_menu, sVar(menu_current))->menu_name, /*"bench"*/ STR(727)))
					temp = sInt(bench_settings);
			...
			case 758: // benchscenery
			case 1328: // libraryscenery
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(762) }) });
				menu_add_item({ e_option_IMPORT_WORLD, text_get({ /*"librarysceneryimport"*/ STR(1329) }), null_, icons_SCENERY });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 759: // benchblocktex
			case 760: // benchblocktexmaterial
			case 761: // benchblocktexnormal
			case 1330: // libraryblocktex
			case 1331: // libraryblocktexmaterial
			case 1332: // libraryblocktexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
			...
			case 768: // benchitemtex
			case 769: // benchitemtexmaterial
			case 770: // benchitemtexnormal
			case 1333: // libraryitemtex
			case 1334: // libraryitemtexmaterial
			case 1335: // libraryitemtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 749: // benchbodypart
			{
				for (IntType p = IntType(0); p < ds_list_size(idInt(ObjType(obj_bench_settings, sInt(bench_settings))->model_file, file_part_list)); p++)
				{
					...
					menu_add_item({ ObjType(obj_model_part, part)->name, minecraft_asset_get_name(/*"modelpart"*/ STR(753), ObjType(obj_model_part, part)->name) });
				}
				break;
			}
			
			case 1336: // templateeditorbodypart
			{
			...
			case 773: // benchtextfont
			case 1337: // librarytextfont
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
			...
			case 776: // benchshapetype
			{
				for (IntType i = IntType(0); i < e_shape_type_amount; i++)
					menu_add_item({ i, text_get({ /*"type"*/ STR(780) + DsList(global::tl_type_name_list).Value(e_tl_type_CUBE + i) }) });
				break;
			}
			
			case 777: // benchshapetex
			case 778: // benchshapetexmaterial
			case 779: // benchshapetexnormal
			case 1338: // libraryshapetex
			case 1339: // libraryshapetexmaterial
			case 1340: // libraryshapetexnormal
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(762) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				...
				if (name == /*"benchshapetex"*/ STR(777) || name == /*"libraryshapetex"*/ STR(1338))
				{
			...
			case 1342: // particleeditorspawnregiontype
			{
				menu_add_item({ /*"sphere"*/ STR(531), text_get({ /*"particleeditorspawnregiontypesphere"*/ STR(1343) }), null_, icons_BOUNDARY_CIRCLE });
				menu_add_item({ /*"cube"*/ STR(3), text_get({ /*"particleeditorspawnregiontypecube"*/ STR(1344) }), null_, icons_BOUNDARY_CUBE });
				menu_add_item({ /*"box"*/ STR(1345), text_get({ /*"particleeditorspawnregiontypebox"*/ STR(1346) }), null_, icons_BOUNDARY_BOX });
				menu_add_item({ /*"path"*/ STR(537), text_get({ /*"particleeditorspawnregiontypepath"*/ STR(1347) }), null_, icons_PATH });
				break;
			}
			
			case 1348: // particleeditorspawnregionpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(762) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1349: // particleeditorboundingbox
			{
				menu_add_item({ /*"none"*/ STR(883), text_get({ /*"particleeditorboundingboxtypenone"*/ STR(1350) }) });
				menu_add_item({ /*"spawn"*/ STR(1351), text_get({ /*"particleeditorboundingboxtypespawn"*/ STR(1352) }) });
				menu_add_item({ /*"ground"*/ STR(142), text_get({ /*"particleeditorboundingboxtypeground"*/ STR(1353) }) });
				menu_add_item({ /*"custom"*/ STR(605), text_get({ /*"particleeditorboundingboxtypecustom"*/ STR(1354) }) });
				break;
			}
			
			case 1355: // particleeditortypetemp
			{
				menu_add_item({ particle_template, text_get({ /*"particleeditortypetemplate"*/ STR(1356) }) });
				menu_add_item({ particle_sheet, text_get({ /*"particleeditortypespritesheet"*/ STR(1357) }) });
			...
			case 1358: // particleeditortypespritetex
			{
			...
			case 1359: // particleeditortypespritetemplatepack
			{
				VarType img = idVar(global::ptype_edit, sprite_tex_image);
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
			...
			case 1360: // particleeditortypespritetemplate
			{
				for (IntType i = IntType(0); i < ds_list_size(global::particle_template_list); i++)
				{
					...
					if (ObjType(obj_particle_template, temp)->animated)
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1360) + ObjType(obj_particle_template, temp)->name }) + /*" "*/ STR(21) + text_get({ /*"particleeditortypespritetemplateframes"*/ STR(1361), ObjType(obj_particle_template, temp)->frames }) });
					else
						menu_add_item({ ObjType(obj_particle_template, temp)->name, text_get({ /*"particleeditortypespritetemplate"*/ STR(1360) + ObjType(obj_particle_template, temp)->name }) });
					
				}
				break;
			}
			
			case 1362: // backgroundimage
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(762) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
			...
			case 1363: // backgroundimagetype
			{
				menu_add_item({ /*"image"*/ STR(94), text_get({ /*"backgroundimagetypeimage"*/ STR(1364) }) });
				menu_add_item({ /*"sphere"*/ STR(531), text_get({ /*"backgroundimagetypesphere"*/ STR(1365) }) });
				menu_add_item({ /*"box"*/ STR(1345), text_get({ /*"backgroundimagetypebox"*/ STR(1366) }) });
				break;
			}
			
			case 1367: // backgroundskysuntex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
			...
			case 1368: // backgroundskymoontex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
			...
			case 1369: // backgroundskymoonphase
			{
				for (IntType p = IntType(0); p < IntType(8); p++)
					menu_add_item({ p, text_get({ /*"backgroundskymoonphase"*/ STR(1369) + string(p + IntType(1)) }), idArr(sVar(background_sky_moon_tex), moon_texture).Value(p) });
				break;
			}
			
			case 1370: // backgroundskycloudstex
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
			...
			case 1371: // backgroundgroundtex
			case 1372: // backgroundgroundtexmaterial
			case 1373: // backgroundgroundtexnormal
			{
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, icons_FOLDER });
				menu_add_item({ global::mc_res, ObjType(obj_resource, global::mc_res)->display_name, ObjType(obj_resource, global::mc_res)->block_preview_texture });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (name == /*"backgroundgroundtexmaterial"*/ STR(1372))
					{
						if (res != global::mc_res && idInt(res, block_sheet_texture_material) != null_)
							menu_add_item({ res, idVar(res, display_name), idInt(res, block_preview_texture) });
					}
					else
						if (name == /*"backgroundgroundtexnormal"*/ STR(1373))
						{
			...
			case 1374: // resourcespackimage
			{
				menu_add_item({ /*"preview"*/ STR(1375), text_get({ /*"resourcespackpreview"*/ STR(1376) }) });
				menu_add_item({ /*"modeltextures"*/ STR(1377), text_get({ /*"resourcespackmodeltextures"*/ STR(1378) }) });
				menu_add_item({ /*"blocksheet"*/ STR(546), text_get({ /*"resourcespackblocksheet"*/ STR(1379) }) });
				menu_add_item({ /*"colormap"*/ STR(1380), text_get({ /*"resourcespackcolormap"*/ STR(1381) }) });
				menu_add_item({ /*"itemsheet"*/ STR(544), text_get({ /*"resourcespackitemsheet"*/ STR(1382) }) });
				menu_add_item({ /*"particlesheet"*/ STR(548), text_get({ /*"resourcespackparticlesheet"*/ STR(1383) }) });
				menu_add_item({ /*"suntexture"*/ STR(1384), text_get({ /*"resourcespacksuntexture"*/ STR(1385) }) });
				menu_add_item({ /*"moontexture"*/ STR(1386), text_get({ /*"resourcespackmoontexture"*/ STR(1387) }) });
				menu_add_item({ /*"cloudtexture"*/ STR(1388), text_get({ /*"resourcespackcloudtexture"*/ STR(1389) }) });
				break;
			}
			
			case 1390: // resourcespackmaterial
			{
				menu_add_item({ /*"diffuse"*/ STR(612), text_get({ /*"resourcespackmaterialdiffuse"*/ STR(1391) }) });
				menu_add_item({ /*"material"*/ STR(617), text_get({ /*"resourcespackmaterialmaterial"*/ STR(1392) }) });
				menu_add_item({ /*"normal"*/ STR(606), text_get({ /*"resourcespackmaterialnormal"*/ STR(1393) }) });
				break;
			}
			
			case 1394: // resourcespackimagemodeltexture
			{
			...
			case 1395: // resourcesscenerystructurepalette
			{
				for (IntType p = IntType(0); p < idInt(global::res_edit, scenery_palette_size); p++)
					menu_add_item({ p, text_get({ /*"resourcesscenerystructurepalettenumber"*/ STR(1396), p + IntType(1) }) });
				break;
			}
			
			case 1397: // frameeditorpath
			{
				menu_add_item({ null_, text_get({ /*"listnone"*/ STR(762) }) });
				withAll (obj_timeline, self->id)
				{
			...
			case 1398: // frameeditorchartex
			case 1399: // frameeditorspblocktex
			case 1400: // frameeditorbodyparttex
			case 1401: // frameeditormodeltex
			{
				...
				if (texobj != null_)
				{
					...
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), text_get({ /*"listnone"*/ STR(762) }) }), null_ });
			...
			case 1402: // frameeditorchartexmaterial
			case 1403: // frameeditorspblocktexmaterial
			case 1404: // frameeditorbodyparttexmaterial
			case 1405: // frameeditormodeltexmaterial
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1406: // frameeditorchartexnormal
			case 1407: // frameeditorspblocktexnormal
			case 1408: // frameeditorbodyparttexnormal
			case 1409: // frameeditormodeltexnormal
			{
				VarType temp = idVar(global::tl_edit, temp);
			...
			case 1410: // frameeditorblocktex
			case 1411: // frameeditorblocktexmaterial
			case 1412: // frameeditorblocktexnormal
			{
				VarType texobj = null_;
				if (name == /*"frameeditorblocktexmaterial"*/ STR(1411))
					texobj = idVar(idVar(global::tl_edit, temp), block_tex_material);
				else
					if (name == /*"frameeditorblocktexnormal"*/ STR(1412))
						texobj = idVar(idVar(global::tl_edit, temp), block_tex_normal);
					else
						if (name == /*"frameeditorblocktex"*/ STR(1410))
				...
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }), idInt(texobj, block_preview_texture) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			case 1413: // frameeditoritemtex
			case 1414: // frameeditoritemtexmaterial
			case 1415: // frameeditoritemtexnormal
			{
				VarType texobj;
				if (name == /*"frameeditoritemtexmaterial"*/ STR(1414))
					texobj = idVar(idVar(global::tl_edit, temp), item_tex_material);
				else
					if (name == /*"frameeditoritemtexnormal"*/ STR(1415))
				...
				if (idVar(texobj, type) == e_res_type_TEXTURE)
					menu_add_item({ texobj, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }), idVar(texobj, texture) });
				else
					if (idInt(texobj, item_sheet_texture) != null_)
						menu_add_item({ texobj, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }), idInt(texobj, block_preview_texture) });
			...
			case 1416: // frameeditorshapetex
			case 1417: // frameeditorshapetexmaterial
			case 1418: // frameeditorshapetexnormal
			{
				VarType texobj;
				if (idVar(global::tl_edit, temp) == null_)
					texobj = null_;
				else
					if (name == /*"frameeditorshapetex"*/ STR(1416))
						texobj = idVar(idVar(global::tl_edit, temp), shape_tex);
					else
						if (name == /*"frameeditorshapetexmaterial"*/ STR(1417))
				...
				if (texobj != null_)
				{
					if (Obj(texobj)->subAssetId == ID_obj_timeline)
						menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }) });
					else
						menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), idVar(texobj, display_name) }), idVar(texobj, texture) });
					
					menu_add_item({ IntType(0), text_get({ /*"listnone"*/ STR(762) }) });
				}
				else
					menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), text_get({ /*"listnone"*/ STR(762) }) }) });
				...
				if (name == /*"frameeditorshapetex"*/ STR(1416) && idVar(global::tl_edit, type) != e_tl_type_PATH)
				{
					withAll (obj_timeline, self->id)
			...
			case 1419: // frameeditorcameralensdirttexture
			{
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), text_get({ /*"listnone"*/ STR(762) }) }) });
				menu_add_item({ e_option_BROWSE, text_get({ /*"listbrowse"*/ STR(1323) }), null_, null_, ID_action_tl_frame_cam_lens_dirt_tex_browse });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
					if (idVar(res, texture) > 0)
			...
			{
				menu_add_item({ null_, text_get({ /*"listdefault"*/ STR(787), idVar(idVar(idVar(global::tl_edit, temp), text_font), display_name) }) });
				for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(res_list), display_list)); i++)
				{
					VarType res = DsList(idInt(sInt(res_list), display_list)).Value(i);
			...
			{
				menu_add_item({ /*"left"*/ STR(197), text_get({ /*"settingswatermarkleft"*/ STR(1439) }) });
				menu_add_item({ /*"center"*/ STR(1440), text_get({ /*"settingswatermarkcenter"*/ STR(1441) }) });
				menu_add_item({ /*"right"*/ STR(198), text_get({ /*"settingswatermarkright"*/ STR(1442) }) });
				break;
			}
			
			case 1443: // settingswatermarkpositiony
			{
				menu_add_item({ /*"top"*/ STR(196), text_get({ /*"settingswatermarktop"*/ STR(1444) }) });
				menu_add_item({ /*"center"*/ STR(1440), text_get({ /*"settingswatermarkcenter"*/ STR(1441) }) });
				menu_add_item({ /*"bottom"*/ STR(195), text_get({ /*"settingswatermarkbottom"*/ STR(1445) }) });
				break;
			}
			...
			case 1449: // frameeditorcameravideosize
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(global::videotemplate_list); i++)
					withOne (Object, DsList(global::videotemplate_list).Value(i), self->id)
						menu_add_item({ self->id, text_get({ /*"projectvideosizetemplate"*/ STR(1451) + idVar(self->id, name) }) + /*" ("*/ STR(1131) + string(sVar(width)) + /*"x"*/ STR(825) + string(sVar(height)) + /*")"*/ STR(1107) });
			...
			{
				menu_add_item({ /*"mp4"*/ STR(61), text_get({ /*"exportmovieformatmp4"*/ STR(1454) }) });
				menu_add_item({ /*"mov"*/ STR(62), text_get({ /*"exportmovieformatmov"*/ STR(1455) }) });
				menu_add_item({ /*"wmv"*/ STR(63), text_get({ /*"exportmovieformatwmv"*/ STR(1456) }) });
				menu_add_item({ /*"png"*/ STR(64), text_get({ /*"exportmovieformatpng"*/ STR(1457) }) });
				break;
			}
			...
			case 1506: // armoreditorpatternhelmet
			case 1507: // armoreditorpatternchestplate
			case 1508: // armoreditorpatternleggings
			case 1509: // armoreditorpatternboots
			{
				menu_add_item({ /*"none"*/ STR(883), text_get({ /*"armoreditorpattern"*/ STR(1510) + /*"none"*/ STR(883) }) });
				for (IntType i = IntType(0); i < ds_list_size((IntType)(global::minecraft_armor_trim_pattern_list)); i++)
					menu_add_item({ DsList(global::minecraft_armor_trim_pattern_list).Value(i), text_get({ /*"armoreditorpattern"*/ STR(1510) + DsList(global::minecraft_armor_trim_pattern_list).Value(i) }) });
				break;
			}
			
			case 1511: // armoreditormaterialhelmet
			case 1512: // armoreditormaterialchestplate
			case 1513: // armoreditormaterialleggings
			case 1514: // armoreditormaterialboots
			{
				for (IntType i = IntType(0); i < ds_list_size((IntType)(global::minecraft_armor_trim_material_list)); i++)
					menu_add_item({ DsList(global::minecraft_armor_trim_material_list).Value(i), text_get({ /*"armoreditormaterial"*/ STR(1515) + DsList(global::minecraft_armor_trim_material_list).Value(i) }) });
				break;
			}
			
			case 1516: // timelineeditorglinttex
			{
			...
			case 1517: // frameeditortransition
			{
				for (IntType i = IntType(0); i < ds_list_size(global::transition_list_order); i++)
					menu_add_item({ DsList(global::transition_list_order).Value(i), text_get({ /*"menu"*/ STR(1077) + DsList(global::transition_list_order).Value(i) }) });
				break;
			}
