...
namespace CppProject
{
	...
	void marker_editor_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, /*"timelinemarkerlabel"*/ STR(1649), sVar(dx), sVar(dy), sVar(settings_menu_w) - IntType(24), IntType(24), idInt(sVar(timeline), tbx_marker_name), null_))
	...
	void math_lib_startup(StringType path)
	{
		...
		global::lib_math_simplex1d = external_define({ path, /*"simplex1D"*/ STR(1650), dll_cdecl, ty_real, IntType(1), ty_real });
		global::lib_math_simplex2d = external_define({ path, /*"simplex2D"*/ STR(1651), dll_cdecl, ty_real, IntType(2), ty_real, ty_real });
		global::lib_math_simplex3d = external_define({ path, /*"simplex3D"*/ STR(1652), dll_cdecl, ty_real, IntType(3), ty_real, ty_real, ty_real });
		global::lib_math_simplex4d = external_define({ path, /*"simplex4D"*/ STR(1653), dll_cdecl, ty_real, IntType(4), ty_real, ty_real, ty_real, ty_real });
	}
	...
	RealType menu_add_timeline(VarType tl, RealType level, VarType menu)
	{
		if (tl == global::tl_edit && !ObjType(obj_menu, menu)->menu_include_tl_edit)
			return IntType(0);
		if (tl == null_)
			list_item_add(text_get({ /*"timelinenone"*/ STR(1654) }), tl);
		else
		{
			list_item_add(string_remove_newline(idVar(tl, display_name)), tl);
			if (ds_list_size(idInt(tl, tree_list)))
				list_item_add_action(global::list_item_last, string(tl) + /*"extend"*/ STR(1655), ID_menu_item_set_extend, idVar(tl, tree_extend), tl, null_, /*"left"*/ STR(188), (idVar(tl, tree_extend) > 0) ? /*"tooltiptlcollapse"*/ STR(1656) : /*"tooltiptlexpand"*/ STR(1657), ID_spr_chevron_ani);
		}
	...
	IntType menu_biome_init(ScopeAny self, VarType menu)
	{
		...
		for (IntType b = IntType(0); b < ds_list_size(global::biome_list); b++)
		{
			...
			menu_add_item({ ObjType(obj_biome, biome)->name, minecraft_asset_get_name(/*"biome"*/ STR(1658), ObjType(obj_biome, biome)->name) });
			if (ObjType(obj_biome, biome)->biome_variants != null_ && ds_list_size(ObjType(obj_biome, biome)->biome_variants) > IntType(0))
			{
				list_item_add_action(global::list_item_last, string(biome) + /*"extend"*/ STR(1655), ID_menu_item_set_extend, ObjType(obj_biome, biome)->variants_extend, biome, null_, /*"left"*/ STR(188), (ObjType(obj_biome, biome)->variants_extend > 0) ? /*"tooltiptlcollapse"*/ STR(1656) : /*"tooltiptlexpand"*/ STR(1657), ID_spr_chevron_ani);
				if (ObjType(obj_biome, biome)->variants_extend > 0)
				{
					for (IntType v = IntType(0); v < ds_list_size(ObjType(obj_biome, biome)->biome_variants); v++)
					{
						if (ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name == /*"normal"*/ STR(601))
							continue;
						menu_add_item({ ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name, minecraft_asset_get_name(/*"biome"*/ STR(1658), ObjType(obj_biome, DsList(ObjType(obj_biome, biome)->biome_variants).Value(v))->name) });
