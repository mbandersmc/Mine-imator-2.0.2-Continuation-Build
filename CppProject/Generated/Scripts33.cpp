...
namespace CppProject
{
	...
	void list_item_add_action(IntType item, StringType name, IntType script, VarType active, VarType value, IntType icon, StringType side, StringType tip, IntType sprite)
	{
		IntType list;
	...
	RealType list_item_draw(ScopeAny self, VarArgs argument)
	{
		...
		if (animation > 0)
		{
			microani_set(string(item), /*""*/ STR(0), idVar(item, hover), idVar(item, hover) > 0 && sBool(mouse_left) && idBool(item, interact), toggled > 0 && idBool(item, interact), idVar(item, disabled));
			microani_update({ idVar(item, hover), idVar(item, hover) > 0 && sBool(mouse_left) && idBool(item, interact), toggled > 0 && idBool(item, interact), idVar(item, disabled) });
