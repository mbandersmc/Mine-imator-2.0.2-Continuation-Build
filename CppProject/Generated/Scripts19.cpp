...
namespace CppProject
{
	void background_sky_update()
	{
		...
		RealType alphay;
		if (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(717))
		...
		global::_app->background_clouds_alpha = (global::_app->background_sky_clouds_mode == /*"faded"*/ STR(717) ? IntType(1) - min({ global::_app->background_night_alpha, 0.95 }) : .8 - min({ global::_app->background_night_alpha, 0.75 })) * alphay;
	...
	RealType background_sky_update_clouds(ScopeAny self)
	{
		...
		hei = IntType(0);
		if (sVar(background_sky_clouds_mode) == /*"faded"*/ STR(717))
		{
		...
		if (sVar(background_sky_clouds_mode) != /*"flat"*/ STR(718))
		{
		...
		col = (sVar(background_sky_clouds_mode) == /*"flat"*/ STR(718) ? coltop : colbottom);
	...
	RealType bench_click(ScopeAny self, VarType type)
	{
		if (ObjType(obj_bench_settings, sInt(bench_settings))->type == type && sStr(bench_show_ani_type) == /*""*/ STR(0))
		{
			...
			sStr(bench_show_ani_type) = /*"hide"*/ STR(719);
			return IntType(0);
		}
	...
	RealType bench_draw(ScopeAny self)
	{
		...
		func = /*""*/ STR(0);
		if (sStr(bench_show_ani_type) == /*"show"*/ STR(720))
		{
			...
			func = /*"easeoutcirc"*/ STR(557);
		}
		else
			if (sStr(bench_show_ani_type) == /*"hide"*/ STR(719))
			{
				...
				if (sVar(bench_show_ani) == IntType(0))
					sStr(bench_show_ani_type) = /*""*/ STR(0);
				func = /*"easeincirc"*/ STR(556);
			}
		
		if (sVar(bench_show_ani) == IntType(0))
		{
			if (sVar(window_busy) == /*"bench"*/ STR(721))
		...
		for (IntType i = IntType(0); i < ds_list_size(idInt(sInt(bench_type_list), item)); i++)
		{
			...
			{
				if (ObjType(obj_list_item, DsList(idInt(sInt(bench_type_list), item)).Value(i))->name == /*"typemodel"*/ STR(166) || ObjType(obj_list_item, DsList(idInt(sInt(bench_type_list), item)).Value(i))->name == /*"typebackground"*/ STR(178))
					skipasset = true;
			}
			if (!skipasset)
			{
		...
		clip_end();
		draw_set_alpha(1.0);
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(bench_show_ani_type) != /*"hide"*/ STR(719))
	...
	void bench_draw_settings(ScopeAny self, VarType bx, VarType by, VarType bw, VarType bh)
	{
		...
		VarType aniease = ease(/*"easeoutcirc"*/ STR(557), sVar(bench_settings_ani));
		...
		if (type_is_timeline(ObjType(obj_bench_settings, sInt(bench_settings))->type))
		{
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_LIGHT_SOURCE)
			{
				...
				togglebutton_add(self, { /*"typepointlight"*/ STR(722), null_, e_tl_type_POINT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT, ID_action_bench_light_type });
				togglebutton_add(self, { /*"typespotlight"*/ STR(723), null_, e_tl_type_SPOT_LIGHT, ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_SPOT_LIGHT, ID_action_bench_light_type });
				draw_togglebutton(self, { /*"benchlighttype"*/ STR(724), sVar(dx), sVar(dy) });
				tab_next(self);
				if (ObjType(obj_bench_settings, sInt(bench_settings))->light_type == e_tl_type_POINT_LIGHT)
					draw_tooltip_label(self, /*"benchpointlighttip"*/ STR(725), icons_LIGHT_POINT, e_toast_INFO);
				else
					draw_tooltip_label(self, /*"benchspotlighttip"*/ STR(726), icons_LIGHT_SPOT, e_toast_INFO);
			...
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_CAMERA)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchcameratip"*/ STR(727), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_BACKGROUND)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchbackgroundtip"*/ STR(728), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_AUDIO)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchaudiotip"*/ STR(729), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH)
			{
				...
				sVar(dy) += IntType(144) + IntType(8);
				draw_tooltip_label(self, /*"benchpathtip"*/ STR(730), icons_INFO, e_toast_INFO);
			}
			if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_tl_type_PATH_POINT)
				draw_tooltip_label(self, /*"benchpathpointtip"*/ STR(731), icons_INFO, e_toast_INFO);
		}
		else
		{
			...
			switch ((IntType)ObjType(obj_bench_settings, sInt(bench_settings))->type)
			{
				...
				case e_temp_type_BODYPART:
				{
					...
					{
						labeltext = text_get({ /*"benchmodel"*/ STR(732) });
						list = ObjType(obj_bench_settings, sInt(bench_settings))->char_list;
						texcap = /*"benchskin"*/ STR(733);
						texmatcap = /*"benchskinmaterial"*/ STR(734);
						texnormcap = /*"benchskinnormal"*/ STR(735);
						...
						{
							labeltext = text_get({ /*"benchblock"*/ STR(736) });
							list = ObjType(obj_bench_settings, sInt(bench_settings))->special_block_list;
							texcap = /*"benchspblocktex"*/ STR(737);
							texmatcap = /*"benchspblocktexmaterial"*/ STR(738);
							texnormcap = /*"benchspblocktexnormal"*/ STR(739);
							capwid = text_caption_width(self, { texcap, texmatcap, texnormcap });
							part = ObjType(obj_bench_settings, sInt(bench_settings))->model_file;
						}
						else
							if (ObjType(obj_bench_settings, sInt(bench_settings))->type == e_temp_type_BODYPART)
							{
								...
								texcap = /*"benchbodypartskin"*/ STR(740);
								texmatcap = /*"benchbodypartskinmaterial"*/ STR(741);
								texnormcap = /*"benchbodypartskinnormal"*/ STR(742);
								capwid = text_caption_width(self, { /*"benchbodypart"*/ STR(743), texcap, texmatcap, texnormcap });
								part = ObjType(obj_bench_settings, sInt(bench_settings))->model_part;
							}
					
					
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, { list, sVar(dx), sVar(dy), sVar(dw), sVar(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->model_name, false, labeltext });
					tab_next(self);
					VarType model;
					IntType statelen;
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"modelstate"*/ STR(744), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"true"*/ STR(745) && ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) != /*"false"*/ STR(746))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i);
						...
						if (draw_checkbox(self, /*"modelstate"*/ STR(744) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(745), null_))
						{
							sInt(menu_model_state) = sInt(menu_model_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)) == /*"true"*/ STR(745))
								action_bench_model_state(self, /*"false"*/ STR(746));
							else
								action_bench_model_state(self, /*"true"*/ STR(745));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_model_current) = model;
						sInt(menu_model_state_current) = DsMap(ObjType(obj_model, model)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"modelstatevalue"*/ STR(747), ObjType(obj_bench_settings, sInt(bench_settings))->model_state.Value(i + IntType(1))), ID_action_bench_model_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					...
					{
						draw_button_menu(self, { /*"benchbodypart"*/ STR(743), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name, minecraft_asset_get_name(/*"modelpart"*/ STR(748), ObjType(obj_bench_settings, sInt(bench_settings))->model_part_name), ID_action_bench_model_part_name, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
					VarType text, tex;
				...
				case e_temp_type_SCENERY:
				{
					IntType capwid;
					VarType text, tex;
					capwid = text_caption_width(self, { /*"benchscenery"*/ STR(750), /*"benchblocktex"*/ STR(751), /*"benchblocktexmaterial"*/ STR(752), /*"benchblocktexnormal"*/ STR(753) });
				...
				case e_temp_type_ITEM:
				{
					...
					capwid = text_caption_width(self, { /*"typeitem"*/ STR(167) });
					res = ObjType(obj_bench_settings, sInt(bench_settings))->item_tex;
					if (!res_is_ready(res))
						res = global::mc_res;
					draw_label(self, { text_get({ /*"typeitem"*/ STR(167) }) + /*":"*/ STR(755), sVar(dx), sVar(dy) + IntType(12), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
					...
					draw_checkbox(self, /*"benchitem3d"*/ STR(756), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_3d, ID_action_bench_item_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemfacecamera"*/ STR(757), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_face_camera, ID_action_bench_item_face_camera);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitembounce"*/ STR(758), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_bounce, ID_action_bench_item_bounce);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchitemspin"*/ STR(759), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->item_spin, ID_action_bench_item_spin);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					capwid = text_caption_width(self, { /*"benchitemtex"*/ STR(760), /*"benchitemtexmaterial"*/ STR(761), /*"benchitemtexnormal"*/ STR(762) });
					VarType tex = idInt(res, block_preview_texture);
					if (tex == null_)
						tex = idVar(res, texture);
					draw_button_menu(self, { /*"benchitemtex"*/ STR(760), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, display_name), ID_action_bench_item_tex, false, idInt(ObjType(obj_bench_settings, sInt(bench_settings))->item_tex, block_preview_texture), null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						VarType tex = idInt(res, block_preview_texture);
				...
				case e_temp_type_BLOCK:
				{
					...
					IntType capwid;
					VarType text, sprite;
					capwid = text_caption_width(self, { /*"benchblocktex"*/ STR(751), /*"benchblocktexmaterial"*/ STR(752), /*"benchblocktexnormal"*/ STR(753) });
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, { ObjType(obj_bench_settings, sInt(bench_settings))->block_list, sVar(dx), sVar(dy), sVar(dw), sVar(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->block_name, false, text_get({ /*"benchblock"*/ STR(736) }) });
					tab_next(self);
					VarType block;
					IntType statelen;
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						capwid = max({ capwid, text_caption_width(self, { minecraft_asset_get_name(/*"blockstate"*/ STR(763), state) }) });
					}
					tab_set_collumns(self, { true, IntType(2) });
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"true"*/ STR(745) && ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) != /*"false"*/ STR(746))
							continue;
						VarType state = ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i);
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						tab_control(self, sReal(ui_small_height));
						if (draw_checkbox(self, /*"blockstate"*/ STR(763) + state, sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(745), null_))
						{
							sInt(menu_block_state) = sInt(menu_block_state_current);
							if (ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)) == /*"true"*/ STR(745))
								action_bench_block_state(self, /*"false"*/ STR(746));
							else
								action_bench_block_state(self, /*"true"*/ STR(745));
					...
					for (IntType i = IntType(0); i < statelen; i += IntType(2))
					{
						...
						sVar(menu_block_current) = block;
						sInt(menu_block_state_current) = DsMap(ObjType(obj_block, block)->states_map).Value(state);
						draw_button_menu(self, { state, e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1)), minecraft_asset_get_name(/*"blockstatevalue"*/ STR(764), ObjType(obj_bench_settings, sInt(bench_settings))->block_state.Value(i + IntType(1))), ID_action_bench_block_state, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += IntType(32);
					}
				...
				{
					tab_control_sortlist(self, IntType(6));
					sortlist_draw(self, { ObjType(obj_bench_settings, sInt(bench_settings))->particles_list, sVar(dx), sVar(dy), sVar(dw), sVar(tab_control_h), ObjType(obj_bench_settings, sInt(bench_settings))->particle_preset, false, text_get({ /*"benchparticlespreset"*/ STR(765) }) });
					tab_next(self);
				...
				{
					IntType capwid;
					capwid = text_caption_width(self, { /*"benchtextfont"*/ STR(766) });
					if (sReal(setting_advanced_mode) > 0)
					{
						draw_button_menu(self, { /*"benchtextfont"*/ STR(766), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->text_font, idVar(ObjType(obj_bench_settings, sInt(bench_settings))->text_font, display_name), ID_action_bench_text_font, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					VarType sx;
					sx = sVar(dx_start);
					...
					tab_set_collumns(self, { true, IntType(2) });
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtext3d"*/ STR(767), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_3d, ID_action_bench_text_3d);
					tab_next(self);
					tab_control_checkbox(self);
					draw_checkbox(self, /*"benchtextfacecamera"*/ STR(768), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->text_face_camera, ID_action_bench_text_face_camera);
					tab_next(self);
					tab_set_collumns(self, { false });
					sVar(dx_start) = sx;
					break;
				...
				case e_tl_type_SHAPE:
				{
					IntType capwid;
					VarType text;
					capwid = text_caption_width(self, { /*"benchshapetype"*/ STR(769), /*"benchshapetex"*/ STR(770), /*"benchshapetexmaterial"*/ STR(771), /*"benchshapetexnormal"*/ STR(772) });
					text = text_get({ /*"type"*/ STR(773) + DsList(global::tl_type_name_list).Value(e_tl_type_CUBE + ObjType(obj_bench_settings, sInt(bench_settings))->shape_type) });
					draw_button_menu(self, { /*"benchshapetype"*/ STR(769), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), ObjType(obj_bench_settings, sInt(bench_settings))->shape_type, text, ID_action_bench_shape_type, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += IntType(32);
					VarType tex;
					text = text_get({ /*"listnone"*/ STR(754) });
					tex = null_;
					...
					draw_button_menu(self, { /*"benchshapetex"*/ STR(770), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex, text, ID_action_bench_shape_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						text = text_get({ /*"listnone"*/ STR(754) });
						...
						draw_button_menu(self, { /*"benchshapetexmaterial"*/ STR(771), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_material, text, ID_action_bench_shape_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						text = text_get({ /*"listnone"*/ STR(754) });
						tex = null_;
						...
						draw_button_menu(self, { /*"benchshapetexnormal"*/ STR(772), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_normal, text, ID_action_bench_shape_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					if (ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CUBE || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CYLINDER || ObjType(obj_bench_settings, sInt(bench_settings))->shape_type == e_shape_type_CONE)
					{
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapetexmap"*/ STR(774), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_tex_mapped, ID_action_bench_shape_tex_map, /*"benchshapetexmaptip"*/ STR(775));
							tab_next(self);
						}
					}
					else
						...
						{
							tab_control_checkbox(self);
							draw_checkbox(self, /*"benchshapefacecamera"*/ STR(776), sVar(dx), sVar(dy), ObjType(obj_bench_settings, sInt(bench_settings))->shape_face_camera, ID_action_bench_shape_face_camera);
							tab_next(self);
						}
				...
				{
					IntType capwid = text_caption_width(self, { /*"benchmodel"*/ STR(732), /*"benchmodeltex"*/ STR(777), /*"benchmodeltexmaterial"*/ STR(778), /*"benchmodeltexnormal"*/ STR(779) });
					VarType text;
					...
					draw_button_menu(self, { /*"benchmodel"*/ STR(732), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model, text, ID_action_bench_model, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					VarType texobj, tex;
					...
					draw_button_menu(self, { /*"benchmodeltex"*/ STR(777), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex, text, ID_action_bench_model_tex, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
					sVar(dy) += (sReal(ui_large_height) + IntType(8));
					if (sVar(project_render_material_maps) > 0)
					{
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material == null_)
							text = text_get({ /*"listdefault"*/ STR(780), text });
						draw_button_menu(self, { /*"benchmodeltexmaterial"*/ STR(778), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_material, text, ID_action_bench_model_tex_material, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
						withOne (obj_bench_settings, sInt(bench_settings), global::_app->id)
						{
						...
						if (ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal == null_)
							text = text_get({ /*"listdefault"*/ STR(780), text });
						draw_button_menu(self, { /*"benchmodeltexnormal"*/ STR(779), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), sReal(ui_large_height), ObjType(obj_bench_settings, sInt(bench_settings))->model_tex_normal, text, ID_action_bench_model_tex_normal, false, tex, null_, /*""*/ STR(0), null_, null_, capwid });
						sVar(dy) += (sReal(ui_large_height) + IntType(8));
					}
					break;
				}
		...
		if (draw_button_label(self, { /*"benchcreate"*/ STR(781), (edit > 0) ? (sVar(dx) + wid + 8.0) : sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_ASSET_ADD }))
		{
			action_bench_create(self);
			sStr(bench_show_ani_type) = /*"hide"*/ STR(719);
		}
		if (edit > 0)
		{
			if (draw_button_label(self, { /*"benchcreateedit"*/ STR(782), sVar(dx), sy + sVar(dh) - IntType(56), wid, icons_PENCIL, e_button_SECONDARY }))
			{
	...
	{
		ArrType files = file_find(particles_directory, /*".miparticles"*/ STR(685));
		...
		withOne (obj_timeline, (new obj_timeline)->id, self->id)
		{
			...
			else
			{
				...
				if (is_string(idVar(self.otherId, variant)))
					state_vars_add(VarType::CreateRef(self->block_state), ArrType::From({ /*"variant"*/ STR(783), idVar(self.otherId, variant) }));
				tl_update_scenery_part(ScopeAny(self));
			...
			if (idVar(self.otherId, has_text) > 0)
			{
				VarType text, textpos, textscale, textcolor;
				text = idVar(self.otherId, text);
				textpos = idVar(self.otherId, text_position);
				textscale = idVar(self.otherId, text_scale);
				textcolor = idVar(self.otherId, text_color);
			...
			if (idVar(self.otherId, pattern_type) != /*""*/ STR(0))
			{
				...
				if (self->pattern_type == /*"banner"*/ STR(784))
				{
					...
					ObjType(obj_timeline, banner)->pattern_type = /*"banner"*/ STR(784);
				}
