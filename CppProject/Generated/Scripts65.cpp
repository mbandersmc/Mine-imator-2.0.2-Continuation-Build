...
namespace CppProject
{
	...
	void tl_update_list(ScopeAny self, VarArgs argument)
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(sInt(tree_list)); t++)
		{
			...
			withOne (obj_timeline, DsList(sInt(tree_list)).Value(t), self->id)
				tl_update_list(ScopeAny(self), { false, idVar(self.otherId, level) + IntType(1), global::_app->tree_close_parent != null_ });
	...
	RealType tl_update_matrix(ScopeAny self, BoolType usepaths, BoolType updateik)
	{
		...
		for (IntType i = start; i < tlamount; i++)
		{
			...
			withOne (Object, curtl, self->id)
			{
				...
				{
					for (IntType t = IntType(0); t < ds_list_size(sInt(tree_list)); t++)
						ObjType(obj_timeline, DsList(sInt(tree_list))[t])->update_matrix = true;
				}
	...
	void tl_update_parent_is_selected(ScopeAny self)
	{
		...
		for (IntType t = IntType(0); t < ds_list_size(sInt(tree_list)); t++)
			withOne (obj_timeline, DsList(sInt(tree_list)).Value(t), self->id)
				tl_update_parent_is_selected(ScopeAny(self));
	...
	RealType tl_update_path(Scope<obj_timeline> self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(self->tree_list); i++)
		{
			...
			if (ObjType(obj_timeline, tl)->type == e_tl_type_PATH_POINT)
				ds_list_add({ self->path_points_list, ArrType::From({ ObjType(obj_timeline, tl)->value.Value(e_value_POS_X), ObjType(obj_timeline, tl)->value.Value(e_value_POS_Y), ObjType(obj_timeline, tl)->value.Value(e_value_POS_Z), ObjType(obj_timeline, tl)->value.Value(e_value_PATH_POINT_ANGLE), ObjType(obj_timeline, tl)->value.Value(e_value_PATH_POINT_SCALE), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0) }) });
		}
		withAll (obj_timeline, self->id)
		{
			if (self->value.Value(e_value_PATH_OBJ) == self.otherId)
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(773) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					...
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3742) });
				...
				if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
				{
					if (sVar(model_file) != null_)
						sVar(type_name) = minecraft_asset_get_name(/*"model"*/ STR(515), idVar(sVar(model_file), name));
				}
				else
					if (sVar(type) == e_tl_type_BLOCK)
					{
						sVar(type_name) = minecraft_asset_get_name(/*"block"*/ STR(508), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
					}
			
			
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3743), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3744), idVar(sVar(temp), display_name) });
		
	}
	
	void tl_update_values(ScopeAny self)
	{
		sVar(keyframe_prev) = sVar(keyframe_current);
		sVar(keyframe_current) = null_;
		sVar(keyframe_next) = null_;
		...
		{
			sVar(keyframe_next) = DsList(sInt(keyframe_list)).Value(k);
			if (ObjType(obj_keyframe, sVar(keyframe_next))->position > global::_app->timeline_marker)
				break;
			sVar(keyframe_current) = sVar(keyframe_next);
		}
		sReal(keyframe_progress) = tl_update_values_progress(Scope<obj_timeline>(self), global::_app->timeline_marker);
		sBool(keyframe_animate) = (sVar(keyframe_current) > 0 && sVar(keyframe_next) > 0 && sVar(keyframe_current) != sVar(keyframe_next));
		if (sVar(keyframe_current) != null_)
			sVar(keyframe_current_values) = ObjType(obj_keyframe, sVar(keyframe_current))->value;
		if (sVar(keyframe_next) != null_)
			sVar(keyframe_next_values) = ObjType(obj_keyframe, sVar(keyframe_next))->value;
		...
		if (sVar(keyframe_transition) == /*"bezier"*/ STR(537))
		...
		{
			if (sVar(keyframe_current) > 0)
			{
				if (sVar(value).Value(e_value_SOUND_OBJ) > 0 && idBool(sVar(value).Value(e_value_SOUND_OBJ), ready) && sVar(keyframe_prev) != sVar(keyframe_current))
				{
					ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index = audio_play_sound(idInt(sVar(value).Value(e_value_SOUND_OBJ), sound_index), IntType(0), (sVar(value).Value(e_value_SOUND_END) > IntType(0) ? true : false));
					audio_sound_pitch(ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index, sVar(value).Value(e_value_SOUND_PITCH));
					audio_sound_set_track_position(ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index, (mod(sVar(value).Value(e_value_SOUND_START), (idReal(sVar(value).Value(e_value_SOUND_OBJ), sound_samples) / sample_rate_))) * sVar(value).Value(e_value_SOUND_PITCH));
					audio_sound_gain(ObjType(obj_keyframe, sVar(keyframe_current))->sound_play_index, sVar(value).Value(e_value_SOUND_VOLUME), 0.0);
				}
				for (IntType k = IntType(0); k < ds_list_size(sInt(keyframe_list)); k++)
				{
					...
					if (sVar(keyframe_current) == DsList(sInt(keyframe_list)).Value(k))
						break;
				}
			}
		}
		if (sVar(type) == e_temp_type_PARTICLE_SPAWNER && global::_app->timeline_marker > global::_app->timeline_marker_previous && sVar(keyframe_prev) != sVar(keyframe_current))
		{
