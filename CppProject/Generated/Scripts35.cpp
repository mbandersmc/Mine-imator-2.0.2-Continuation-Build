...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1725), /*"b"*/ STR(1726), /*"c"*/ STR(1727), /*"d"*/ STR(1728), /*"e"*/ STR(1201), /*"f"*/ STR(1202), /*"g"*/ STR(1729), /*"h"*/ STR(1730), /*"i"*/ STR(1731), /*"j"*/ STR(1732), /*"k"*/ STR(1733), /*"l"*/ STR(1734), /*"m"*/ STR(1735), /*"n"*/ STR(1203), /*"o"*/ STR(1736), /*"p"*/ STR(1737), /*"q"*/ STR(1738), /*"r"*/ STR(1739), /*"s"*/ STR(1740), /*"t"*/ STR(1204), /*"u"*/ STR(1205), /*"v"*/ STR(1741), /*"w"*/ STR(1742), /*"x"*/ STR(818), /*"y"*/ STR(801), /*"z"*/ STR(819) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1743))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1743));
			...
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real(DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1744)), false);
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1745);
		...
		{
			log({ /*"Could not load "*/ STR(1746) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1747), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1748)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1748) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1749));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1750));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1751), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1752), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1753), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1754), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1755), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1756));
		if (!is_real(patch))
			patch = IntType(1);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1757) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1754), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1692));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1696));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1758) + name + /*"]"*/ STR(1199) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1759) + name + /*"]"*/ STR(1199) : /*""*/ STR(0));
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(784) ? /*"entity/banner_base"*/ STR(1760) : /*"entity/shield_base"*/ STR(1761));
		patterndir = (type == /*"banner"*/ STR(784) ? /*"entity/banner/"*/ STR(1762) : /*"entity/shield/"*/ STR(1763));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1764), fn });
		show_message(/*"The file "*/ STR(1765) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1766));
		return false;
	}
	...
	IntType model_file_load(StringType fname, IntType res)
	{
		...
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1767) });
			return null_;
		}
		...
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1770))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1771) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			self->player_skin = value_get_real(DsMap(map).Value(/*"player_skin"*/ STR(1776)), false);
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1770));
	...
	VarType model_file_load_part(ScopeAny self, VarType map, IntType root, IntType res, IntType model)
	{
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1779) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1780), DsMap(map).Value(/*"name"*/ STR(787)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(748) + self->name))
				log({ /*"model/part/"*/ STR(1781) + self->name + dev_mode_name_translation_message });
			...
			self->show_position = value_get_real(DsMap(map).Value(/*"show_position"*/ STR(1789)), false);
			...
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1790))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1790))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1790));
				if (self->lock_bend > 0)
				{
