...
namespace CppProject
{
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1562) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	VarType model_file_load_part(ScopeAny self, VarType map, IntType root, IntType res, IntType model)
	{
		...
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			self->backfaces = value_get_real(DsMap(map).Value(/*"backfaces"*/ STR(1790)), false);
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1791))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1791));
				self->bend_inherit = value_get_real(DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1792)), false);
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1793))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1794) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1793));
				self->bend_end_offset = value_get_real(DsMap(bendmap).Value(/*"end_offset"*/ STR(1795)), IntType(0));
				self->bend_size = value_get_real(DsMap(bendmap).Value(/*"size"*/ STR(974)), null_);
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(871))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1796) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(871))).id)
				{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1800) });
						return null_;
				}
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(800))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(800))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1801) });
							return null_;
					}
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1802) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1803))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1803));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1803))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1803)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1803))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1804))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1804));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1804))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1804)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1804))), i);
					}
				...
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(898))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(898))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1808) });
							return null_;
					}
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1809))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1809)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1809)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1809))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1809)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1809))), i);
					}
					else
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1810));
