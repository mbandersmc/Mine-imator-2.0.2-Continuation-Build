...
namespace CppProject
{
	...
	{
		ArrType letterlist = ArrType::From({ /*"a"*/ STR(1722), /*"b"*/ STR(1723), /*"c"*/ STR(1724), /*"d"*/ STR(1725), /*"e"*/ STR(1200), /*"f"*/ STR(1201), /*"g"*/ STR(1726), /*"h"*/ STR(1727), /*"i"*/ STR(1728), /*"j"*/ STR(1729), /*"k"*/ STR(1730), /*"l"*/ STR(1731), /*"m"*/ STR(1732), /*"n"*/ STR(1202), /*"o"*/ STR(1733), /*"p"*/ STR(1734), /*"q"*/ STR(1735), /*"r"*/ STR(1736), /*"s"*/ STR(1737), /*"t"*/ STR(1203), /*"u"*/ STR(1204), /*"v"*/ STR(1738), /*"w"*/ STR(1739), /*"x"*/ STR(818), /*"y"*/ STR(801), /*"z"*/ STR(819) });
		for (IntType i = IntType(0); i < ds_list_size((IntType)(particlelist)); i++)
		{
			...
			if (!is_undefined(DsMap(tempmap).Value(/*"frames"*/ STR(1740))))
			{
				ObjType(obj_particle_template, ptemp)->frames = DsMap(tempmap).Value(/*"frames"*/ STR(1740));
			...
			ObjType(obj_particle_template, ptemp)->size = value_get_real({ DsMap(tempmap).Value(/*"size"*/ STR(973)), IntType(8) });
			ObjType(obj_particle_template, ptemp)->letter_suffix = value_get_real({ DsMap(tempmap).Value(/*"letter_suffix"*/ STR(1741)), false });
			...
			if (ObjType(obj_particle_template, ptemp)->animated)
			{
				for (IntType j = IntType(0); j < ObjType(obj_particle_template, ptemp)->frames; j++)
				{
					if (ObjType(obj_particle_template, ptemp)->letter_suffix > 0)
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1095) + letterlist.Value(j) });
					else
						ds_list_add({ ObjType(obj_particle_template, ptemp)->texture_list, ObjType(obj_particle_template, ptemp)->texture_name + /*"_"*/ STR(1095) + string(j) });
	...
	BoolType minecraft_assets_load_startup(ScopeAny self)
	{
		...
		global::load_assets_stage = /*"unzip"*/ STR(1262);
		...
		withOne (obj_resource, global::mc_res, self->id)
		{
			...
			self->display_name = /*"Minecraft"*/ STR(1742);
		...
		{
			log({ /*"Could not load "*/ STR(1743) + string(global::_app->setting_minecraft_assets_version) + /*" assets. Resetting to default"*/ STR(1744), minecraft_version });
		...
		if (file_exists_lib(splash_directory + /*"splashes.json"*/ STR(1745)))
		{
			IntType map;
			VarType splashlist, splash;
			StringType splashfile;
			map = json_load({ splash_directory + /*"splashes.json"*/ STR(1745) });
			splashlist = DsMap(map).Value(/*"splashes"*/ STR(1746));
			...
			if (file_exists_lib(splashfile))
			{
				...
				global::load_assets_credits = DsMap(splash).Value(/*"credits"*/ STR(1747));
			}
		}
		return true;
	}
	...
	BoolType minecraft_assets_load_startup_version()
	{
		...
		log({ /*"Loading Minecraft assets version"*/ STR(1748), version });
		...
		{
			log({ /*"Could not find Minecraft assets file"*/ STR(1749), global::load_assets_file });
			return false;
		}
		if (!file_exists_lib(global::load_assets_zip_file))
		{
			log({ /*"Could not find Minecraft assets archive"*/ STR(1750), global::load_assets_zip_file });
			return false;
		}
		...
		{
			log({ /*"Could not parse JSON"*/ STR(1751), global::load_assets_file });
		...
		{
			log({ /*"Unsupported archive, format"*/ STR(1752), format });
			return false;
		}
		VarType patch = DsMap(global::load_assets_map).Value(/*"patch"*/ STR(1753));
		if (!is_real(patch))
			patch = IntType(1);
		global::load_assets_startup_dir = mc_file_directory + version + (patch > IntType(1) ? /*"_"*/ STR(1095) + string(patch) : /*""*/ STR(0)) + /*"/"*/ STR(16);
	...
	BoolType minecraft_assets_reload()
	{
		...
		log({ /*"Reloading models"*/ STR(1754) });
		IntType typemap, map;
		typemap = ds_int_map_create();
		map = json_load({ fname, typemap });
		if (!ds_map_valid(map))
		{
			log({ /*"Could not parse JSON"*/ STR(1751), fname });
		...
		withOne (obj_minecraft_assets, global::mc_assets, noone)
		{
			...
			VarType characterslist = DsMap(map).Value(/*"characters"*/ STR(1689));
			...
			VarType specialblockslist = DsMap(map).Value(/*"special_blocks"*/ STR(1693));
	...
	VarType minecraft_asset_get_name(StringType type, VarType name)
	{
		...
		if (!text_exists(type + name))
			return (dev_mode_debug_names ? /*"?????? ["*/ STR(1755) + name + /*"]"*/ STR(1198) : string_format_snakecase(name));
		else
			return text_get({ type + name }) + (dev_mode_debug_names ? /*" ["*/ STR(1756) + name + /*"]"*/ STR(1198) : /*""*/ STR(0));
	...
	{
		if (global::pattern_update != null_ && global::_app->window_busy != /*"popup"*/ STR(1560) + ObjType(obj_popup, global::_app->popup_loading)->name)
		{
	...
	IntType minecraft_update_pattern_generate(VarType type, VarType color, VarType patternlist, VarType colorlist, VarType res)
	{
		...
		patternbase = (type == /*"banner"*/ STR(784) ? /*"entity/banner_base"*/ STR(1757) : /*"entity/shield_base"*/ STR(1758));
		patterndir = (type == /*"banner"*/ STR(784) ? /*"entity/banner/"*/ STR(1759) : /*"entity/shield/"*/ STR(1760));
		skinratio = IntType(1);
		maskarray = ArrType();
	...
	{
		log({ /*"Missing file"*/ STR(1761), fn });
		show_message(/*"The file "*/ STR(1762) + fn + /*" could not be found. If you are running from an archive, extract all the files and try again. If problems persist, re-install the program."*/ STR(1763));
		return false;
	}
	...
	IntType model_file_load(VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType fname, res;
		fname = argument[IntType(0)];
		...
		{
			log({ /*"Missing parameter \"texture\""*/ STR(1764) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"texture_size"*/ STR(1765))))
		{
			log({ /*"Missing array \"texture_size\""*/ STR(1766) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"parts"*/ STR(1767))))
		{
			log({ /*"Missing array \"parts\""*/ STR(1768) });
			return null_;
		}
		withOne (obj_model_file, (new obj_model_file)->id, noone)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"model"*/ STR(515) + self->name))
				log({ /*"model/"*/ STR(1769) + self->name + dev_mode_name_translation_message });
			self->description = value_get_string({ DsMap(map).Value(/*"description"*/ STR(1770)), /*""*/ STR(0) });
			...
			if (res != null_)
			{
				self->texture_material_name = value_get_string({ DsMap(map).Value(/*"texture_material"*/ STR(1771)), /*""*/ STR(0) });
				if (self->texture_material_name != /*""*/ STR(0))
					model_file_load_texture_material(self->texture_material_name, res);
				self->texture_normal_name = value_get_string({ DsMap(map).Value(/*"texture_normal"*/ STR(1772)), /*""*/ STR(0) });
			...
			self->texture_size = value_get_point2D({ DsMap(map).Value(/*"texture_size"*/ STR(1765)) });
			...
			self->player_skin = value_get_real({ DsMap(map).Value(/*"player_skin"*/ STR(1773)), false });
			self->scale = value_get_point3D({ DsMap(map).Value(/*"scale"*/ STR(1774)), vec3(IntType(1), IntType(1), IntType(1)) });
			...
			VarType partlist = DsMap(map).Value(/*"parts"*/ STR(1767));
			...
			{
				VarType part = model_file_load_part(Scope<obj_model_part>(self), DsList(partlist).Value(p), self->id, res, self->id);
				if (part == null_)
					return null_;
				if (part > IntType(0))
	...
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1775))) && !(DsMap(map).Value(/*"visible"*/ STR(1775)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"name"*/ STR(787))))
		{
			log({ /*"Missing parameter \"name\""*/ STR(788) });
		...
		{
			log({ /*"Missing array \"position\""*/ STR(1776) });
			return null_;
		}
		for (IntType i = IntType(0); i < ds_list_size(ObjType(obj_model_file, root)->file_part_list); i++)
		{
			...
			{
				log({ /*"Duplicate part name found"*/ STR(1777), DsMap(map).Value(/*"name"*/ STR(787)) });
				return null_;
			}
		}
		withOne (obj_model_part, (new obj_model_part)->id, self->id)
		{
			...
			if (res == null_ && dev_mode_debug_names && !text_exists(/*"modelpart"*/ STR(748) + self->name))
				log({ /*"model/part/"*/ STR(1778) + self->name + dev_mode_name_translation_message });
			...
			self->color_inherit = value_get_real({ DsMap(map).Value(/*"color_inherit"*/ STR(1779)), true });
			self->color_blend = value_get_color({ DsMap(map).Value(/*"color_blend"*/ STR(1780)), c_white });
			self->color_alpha = value_get_real({ DsMap(map).Value(/*"color_alpha"*/ STR(1781)), IntType(1) });
			if (is_real(/*"color_emissive"*/ STR(1782)))
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_emissive"*/ STR(1782)), IntType(0) });
			else
				self->color_emissive = value_get_real({ DsMap(map).Value(/*"color_brightness"*/ STR(1783)), IntType(0) });
			
			self->color_mix = value_get_color({ DsMap(map).Value(/*"color_mix"*/ STR(1784)), c_black });
			self->color_mix_percent = value_get_real({ DsMap(map).Value(/*"color_mix_percent"*/ STR(1785)), IntType(0) });
			self->part_mixing_shapes = false;
			...
			self->show_position = value_get_real({ DsMap(map).Value(/*"show_position"*/ STR(1786)), false });
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idVar(self.otherId, locked) > 0)
				self->locked = true;
			else
				self->locked = value_get_real({ DsMap(map).Value(/*"locked"*/ STR(908)), false });
			
			self->lock_bend = true;
			if (Obj(self.otherId)->subAssetId == ID_obj_model_part && idInt(self.otherId, bend_part) != null_)
			{
				if (is_bool(DsMap(map).Value(/*"lock_bend"*/ STR(1787))) || is_real(DsMap(map).Value(/*"lock_bend"*/ STR(1787))))
					self->lock_bend = DsMap(map).Value(/*"lock_bend"*/ STR(1787));
				if (self->lock_bend > 0)
				{
			...
			if (!is_undefined(DsMap(map).Value(/*"bend"*/ STR(1788))))
			{
				VarType bendmap = DsMap(map).Value(/*"bend"*/ STR(1788));
				self->bend_inherit = value_get_real({ DsMap(bendmap).Value(/*"inherit_bend"*/ STR(1789)), false });
				if (!is_real(DsMap(bendmap).Value(/*"offset"*/ STR(1790))))
				{
					log({ /*"Missing parameter \"offset\""*/ STR(1791) });
					return null_;
				}
				self->bend_offset = DsMap(bendmap).Value(/*"offset"*/ STR(1790));
				self->bend_end_offset = value_get_real({ DsMap(bendmap).Value(/*"end_offset"*/ STR(1792)), IntType(0) });
				self->bend_size = value_get_real({ DsMap(bendmap).Value(/*"size"*/ STR(973)), null_ });
				if (!is_string(DsMap(bendmap).Value(/*"part"*/ STR(870))))
				{
					log({ /*"Missing parameter \"part\""*/ STR(1793) });
					return null_;
				}
				switch (StringType(DsMap(bendmap).Value(/*"part"*/ STR(870))).id)
				{
					case 189: // right
					{
					...
					default:
						log({ /*"Invalid parameter \"part\""*/ STR(1797) });
						return null_;
				}
				...
				if (is_string(DsMap(bendmap).Value(/*"axis"*/ STR(800))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"axis"*/ STR(800))).id)
					{
						...
						default:
							log({ /*"Invalid parameter \"axis\""*/ STR(1798) });
							return null_;
					}
					...
					{
						log({ /*"Missing parameter \"axis\""*/ STR(1799) });
						return null_;
					}
				
				
				self->bend_direction_min = vec3(-IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_min"*/ STR(1800))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_min.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_min"*/ STR(1800));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_min"*/ STR(1800))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1800)))); i++)
							self->bend_direction_min.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_min"*/ STR(1800))), i);
					}
				
				self->bend_direction_max = vec3(IntType(180));
				if (is_real(DsMap(bendmap).Value(/*"direction_max"*/ STR(1801))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_direction_max.Real(axis.Value(IntType(0))) = DsMap(bendmap).Value(/*"direction_max"*/ STR(1801));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction_max"*/ STR(1801))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1801)))); i++)
							self->bend_direction_max.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction_max"*/ STR(1801))), i);
					}
				
				self->bend_direction = ArrType::From({ IntType(0), IntType(0), IntType(0) });
				self->bend_direction_legacy = false;
				if (is_string(DsMap(bendmap).Value(/*"direction"*/ STR(897))))
				{
					switch (StringType(DsMap(bendmap).Value(/*"direction"*/ STR(897))).id)
					{
						case 1802: // forward
						{
						...
						default:
							log({ /*"Invalid parameter \"direction\""*/ STR(1805) });
							return null_;
					}
					...
					if (ds_list_valid(DsMap(bendmap).Value(/*"direction"*/ STR(897))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(897)))); i++)
						{
							switch (StringType(ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"direction"*/ STR(897))), i)).id)
							{
								case 1802: // forward
								{
				...
				if ((is_real(DsMap(bendmap).Value(/*"invert"*/ STR(1806))) || is_bool(DsMap(bendmap).Value(/*"invert"*/ STR(1806)))) && array_length(VarType::CreateRef(axis)) == IntType(1))
				{
					self->bend_invert = vec3(DsMap(bendmap).Value(/*"invert"*/ STR(1806)));
				}
				else
					if (ds_list_valid(DsMap(bendmap).Value(/*"invert"*/ STR(1806))))
					{
						for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1806)))); i++)
							self->bend_invert.Real(axis.Value(i)) = ds_list_find_value((IntType)(DsMap(bendmap).Value(/*"invert"*/ STR(1806))), i);
					}
					else
				...
				self->ik_supported = (self->bend_end_offset != IntType(0) && self->bend_axis.Real(X_) && !self->bend_axis.Real(Y_) && !self->bend_axis.Real(Z_));
			}
			else
			{
			...
			self->has_3d_plane = false;
			VarType shapelist = DsMap(map).Value(/*"shapes"*/ STR(1807));
			...
			if (ds_list_valid(partlist))
			{
				...
				for (IntType p = IntType(0); p < ds_list_size((IntType)(partlist)); p++)
				{
					VarType part = model_file_load_part(self, DsList(partlist).Value(p), root, res, model);
					if (part == null_)
						return null_;
					if (part > IntType(0))
						ds_list_add({ self->part_list, part });
