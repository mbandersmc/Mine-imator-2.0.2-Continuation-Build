...
namespace CppProject
{
	void menu_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			if (ObjType(obj_menu, m)->menu_window != window_get_current())
				continue;
			if (ObjType(obj_menu, m)->menu_ani_type == /*"hide"*/ STR(725))
			{
				...
				if (ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(726))
				{
					ObjType(obj_menu, m)->menu_ani += test_reduced_motion(IntType(1), (0.1 * global::delta));
			...
			if (ObjType(obj_menu, m)->menu_steps == IntType(0))
				draw_set_alpha(0.0);
			ObjType(obj_menu, m)->menu_ani_ease = ease(((ObjType(obj_menu, m)->menu_ani_type == /*"show"*/ STR(726)) ? /*"easeoutexpo"*/ STR(571) : /*"easeinexpo"*/ STR(570)), ObjType(obj_menu, m)->menu_ani);
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1077) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(725) && menu_active)
			...
			if (keyboard_check_pressed(vk_enter) || keyboard_check_pressed(vk_escape))
			{
				if (ObjType(obj_menu, m)->menu_nav_use)
				{
					...
					ObjType(obj_menu, m)->menu_ani_type = /*"hide"*/ STR(725);
			...
			if ((!(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_vertical)->mouseon) && !(ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->needed && ObjType(obj_scrollbar, ObjType(obj_menu, m)->menu_scroll_horizontal)->mouseon) && sBool(mouse_left_released) && menu_active && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(725) && sVar(menu_search_busy) == /*""*/ STR(0)) || nav_close)
			{
				...
				if (close)
				{
					...
					if (ds_list_size(sInt(menu_list)) > IntType(1))
						sVar(window_busy) = /*"menu"*/ STR(1077);
					else
			...
			if (sVar(window_busy) == /*""*/ STR(0) && ObjType(obj_menu, m)->menu_ani_type != /*"hide"*/ STR(725) && menu_active)
				sVar(window_busy) = /*"menu"*/ STR(1077);
		}
	...
	void menu_grid_settings(ScopeAny self)
	{
		...
		IntType draggerwid;
		draggerwid = text_max_width({ /*"viewgridrows"*/ STR(1695), /*"viewgridcolumns"*/ STR(1696) }) + IntType(16) + dragger_width;
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridrows"*/ STR(1695), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_rows), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_rows), ID_action_project_grid_rows, null_, true);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewgridcolumns"*/ STR(1696), sVar(dx), sVar(dy), dragger_width, sVar(project_grid_columns), 0.05, IntType(1), IntType(10), IntType(3), IntType(1), sInt(tbx_grid_columns), ID_action_project_grid_columns, null_, true);
		tab_next(self);
	...
	void menu_quality_settings(ScopeAny self)
	{
		...
		VarType text;
		draw_set_font(sInt(font_label));
		capwid = max({ IntType(176), text_max_width({ /*"viewmodepass"*/ STR(1485) }) + IntType(16) });
		text = text_get({ /*"viewmodepass"*/ STR(1485) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) });
		tab_control_menu(self);
		draw_button_menu(self, { /*"viewmodepass"*/ STR(1485), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sInt(project_render_pass), text, ID_action_project_render_pass });
		tab_next(self);
	...
	RealType menu_settings_draw(ScopeAny self)
	{
		if (sInt(settings_menu_window) != window_get_current())
			return IntType(0);
		if (sStr(settings_menu_ani_type) == /*"hide"*/ STR(725))
		{
			...
			if (sStr(settings_menu_ani_type) == /*"show"*/ STR(726))
			{
				if (sReal(settings_menu_quick) > 0)
		...
		if (sStr(settings_menu_name) == /*""*/ STR(0))
			return IntType(0);
		VarType settingsmenuease = ease(((sStr(settings_menu_ani_type) == /*"show"*/ STR(726)) ? /*"easeoutexpo"*/ STR(571) : /*"easeinexpo"*/ STR(570)), sVar(settings_menu_ani));
		...
		if (sVar(window_busy) == /*"settingsmenu"*/ STR(1026) && sStr(settings_menu_ani_type) == /*""*/ STR(0) && sVar(settings_menu_ani) == IntType(1))
			sVar(window_busy) = /*""*/ STR(0);
		sBool(content_mouseon) = app_mouse_box(self, sVar(content_x), sVar(content_y), sVar(content_width), sVar(content_height));
		...
		if (sStr(settings_menu_ani_type) == /*""*/ STR(0) && sBool(mouse_left_released) && !sBool(context_menu_mouseon) && (ds_list_size(sInt(menu_list)) == IntType(0)) && (sVar(window_focus) == /*""*/ STR(0) || sVar(window_focus) == string(sInt(settings_menu_scroll))))
		{
			if (sInt(settings_menu_script) > 0)
			{
				...
				{
					sVar(settings_menu_ani) = IntType(1);
					sStr(settings_menu_ani_type) = /*"hide"*/ STR(725);
					sVar(window_busy) = /*"settingsmenu"*/ STR(1026);
		...
		if (sVar(window_busy) == /*""*/ STR(0) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(725))
			sVar(window_busy) = /*"settingsmenu"*/ STR(1026);
		return 0.0;
	}
	...
	void menu_settings_set(ScopeAny self, VarType xx, VarType yy, StringType name, RealType buttonheight)
	{
		...
		sVar(settings_menu_ani) = IntType(0);
		sStr(settings_menu_ani_type) = /*"show"*/ STR(726);
	...
	void menu_snap_settings(ScopeAny self)
	{
		...
		draggerwid = text_max_width({ /*"viewsnapmove"*/ STR(1697), /*"viewsnaprotate"*/ STR(1698), /*"viewsnapscale"*/ STR(1699) }) + IntType(16) + dragger_width;
		switchwid = text_max_width({ /*"viewsnapabsolute"*/ STR(1700) }) + IntType(28) + IntType(16) + IntType(24);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapmove"*/ STR(1697), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_position), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_position), ID_action_setting_snap_size_position);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnaprotate"*/ STR(1698), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_rotation), 0.01, snap_min, no_limit, IntType(15), snap_min, sInt(tbx_snap_rotation), ID_action_setting_snap_size_rotation);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"viewsnapscale"*/ STR(1699), sVar(dx), sVar(dy), dragger_width, sReal(setting_snap_size_scale), 0.01, snap_min, no_limit, IntType(1), snap_min, sInt(tbx_snap_scale), ID_action_setting_snap_size_scale);
		tab_next(self);
		draw_divide(sVar(content_x), sVar(dy), sVar(content_width));
		sVar(dy) += IntType(8);
		tab_control_switch(self);
		draw_switch(self, /*"viewsnapabsolute"*/ STR(1700), sVar(dx), sVar(dy), sVar(setting_snap_absolute), ID_action_setting_snap_absolute, /*"viewsnapabsolutetip"*/ STR(1701));
		tab_next(self);
	...
	BoolType menu_swatches_draw(ScopeAny self)
	{
		...
		for (IntType s = IntType(0); s < array_length(VarType::CreateRef(global::minecraft_swatch_array)); s++)
		{
			...
			sVar(dy) += IntType(14);
			draw_label(self, { text_get({ /*"swatch"*/ STR(1702) + ObjType(obj_swatch, swatch)->name }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(9);
			VarType xx;
			xx = sVar(dx);
			for (IntType c = IntType(0); c < array_length(VarType::CreateRef(ObjType(obj_swatch, swatch)->colors)); c++)
			{
				if (draw_button_swatch(self, xx, sVar(dy), IntType(20), IntType(20), /*"swatch"*/ STR(1702) + ObjType(obj_swatch, swatch)->name + ObjType(obj_swatch, swatch)->color_names.Value(c), ObjType(obj_swatch, swatch)->colors.Value(c)))
				{
					if (ObjType(obj_context_menu_level, sVar(context_menu_current))->name == /*"contextmenuswatchset"*/ STR(1532))
	...
	VarType menu_transitions(ScopeAny self, VarType xx, VarType yy, VarType wid, VarType hei)
	{
		...
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenuother"*/ STR(1703) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		...
		sVar(dx) = sVar(dx_start);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueasein"*/ STR(1704) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseout"*/ STR(1705) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		sVar(dy) += IntType(46);
		sVar(dy) += IntType(14);
		draw_label(self, { text_get({ /*"transitionmenueaseinout"*/ STR(1706) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
		sVar(dy) += IntType(12);
		for (IntType i = IntType(0); i < ds_list_size(global::transition_list); i++)
		{
		...
		if (sVar(context_menu_name) != /*""*/ STR(0) && transition != null_)
		{
			...
			if (transition != null_)
			{
				...
				ObjType(obj_menu, sVar(menu_current))->menu_ani_type = /*"hide"*/ STR(725);
				sVar(window_busy) = /*""*/ STR(0);
				return true;
			}
			...
			switch (StringType(global::load_assets_stage).id)
			{
				case 1295: // unzip
				{
					...
					if (!(exists > 0))
					{
						...
						{
							log({ /*"Could not unzip archive"*/ STR(1707) });
							directory_delete_lib(global::load_assets_dir);
							access_error();
							return false;
						}
						debug_timer_stop(/*"Unzip archive"*/ STR(1708));
					}
					else
						log({ /*"Archive already unzipped, re-using"*/ STR(1709), global::load_assets_dir + mc_assets_directory });
					
					global::load_assets_stage = /*"biomes"*/ STR(1710);
				...
				case 1710: // biomes
				{
					withOne (obj_resource, global::mc_res, self->id)
						minecraft_assets_load_biomes(global::biome_list, DsMap(global::load_assets_map).Value(/*"biomes"*/ STR(1710)));
					...
					global::load_assets_stage = /*"textures"*/ STR(817);
				...
				case 817: // textures
				{
					VarType modeltextureslist = DsMap(global::load_assets_map).Value(/*"model_textures"*/ STR(1711));
					if (is_undefined(modeltextureslist))
					{
						log({ /*"No model textures found"*/ STR(1712) });
						return false;
					}
					ds_list_copy((IntType)(self->model_texture_list), (IntType)(modeltextureslist));
					VarType blocktextureslist = DsMap(global::load_assets_map).Value(/*"block_textures"*/ STR(1713));
					if (is_undefined(blocktextureslist))
					{
						log({ /*"No block textures found"*/ STR(1714) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_list), (IntType)(blocktextureslist));
					VarType blocktexturesanimatedlist = DsMap(global::load_assets_map).Value(/*"block_textures_animated"*/ STR(1715));
					if (is_undefined(blocktexturesanimatedlist))
					{
						log({ /*"No animated block textures found"*/ STR(1716) });
						return false;
					}
					ds_list_copy((IntType)(self->block_texture_ani_list), (IntType)(blocktexturesanimatedlist));
					VarType blocktexturescolorlist = DsMap(global::load_assets_map).Value(/*"block_textures_color"*/ STR(1717));
					if (is_undefined(blocktexturescolorlist))
					{
						log({ /*"No block texture colors found"*/ STR(1718) });
						return false;
					}
					...
					{
						if (string_char_at(DsMap(self->block_texture_color_map).Value(key), IntType(1)) == /*"#"*/ STR(834))
					...
					VarType blocktexturepreview = DsMap(global::load_assets_map).Value(/*"block_textures_preview"*/ STR(1719));
					if (!is_undefined(blocktexturepreview))
						ds_map_copy((IntType)(self->block_texture_preview_map), (IntType)(blocktexturepreview));
					VarType itemtextureslist = DsMap(global::load_assets_map).Value(/*"item_textures"*/ STR(1720));
					if (is_undefined(itemtextureslist))
					{
						log({ /*"No item textures found"*/ STR(1721) });
						return false;
					}
					ds_list_copy((IntType)(self->item_texture_list), (IntType)(itemtextureslist));
					VarType particletextureslist = DsMap(global::load_assets_map).Value(/*"particle_textures"*/ STR(1722));
					if (is_undefined(particletextureslist))
					{
						log({ /*"No particle textures found"*/ STR(1723) });
						return false;
					}
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(528))))
					{
						log({ /*"No particle list found"*/ STR(1724) });
						return false;
					}
					...
					withOne (obj_resource, global::mc_res, self->id)
					{
						...
						res_load_pack_item_textures(ScopeAny(self), /*"diffuse"*/ STR(612), /*""*/ STR(0));
						...
						minecraft_assets_load_particles(DsMap(global::load_assets_map).Value(/*"particles"*/ STR(528)));
					...
					global::load_assets_stage = /*"misc"*/ STR(1725);
				...
				case 1725: // misc
				{
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1726))))
					{
						log({ /*"No pattern designs list found"*/ STR(1727) });
						return false;
					}
					VarType patternlist = DsMap(global::load_assets_map).Value(/*"patterns"*/ STR(1726));
					...
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(942))))
					{
						log({ /*"No sherd keys found"*/ STR(1728) });
						return false;
					}
					else
						ds_map_copy((IntType)(global::minecraft_sherd_map), (IntType)(DsMap(global::load_assets_map).Value(/*"sherds"*/ STR(942))));
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"armor"*/ STR(756))))
					{
						log({ /*"No armor data found"*/ STR(1729) });
						return false;
					}
					else
					{
						VarType armormap = DsMap(global::load_assets_map).Value(/*"armor"*/ STR(756));
						ds_list_copy((IntType)(global::minecraft_armor_trim_pattern_list), (IntType)(DsMap(armormap).Value(/*"pattern"*/ STR(1730))));
						ds_list_copy((IntType)(global::minecraft_armor_trim_material_list), (IntType)(DsMap(armormap).Value(/*"material"*/ STR(617))));
					}
					
					if (is_undefined(DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1731))))
					{
						log({ /*"No map colors found"*/ STR(1732) });
						return false;
					}
					VarType colors = DsMap(global::load_assets_map).Value(/*"map_colors"*/ STR(1731));
					...
					global::load_assets_stage = /*"models"*/ STR(1733);
					global::load_assets_progress = 0.5;
					VarType swatches = DsMap(global::load_assets_map).Value(/*"swatches"*/ STR(1734));
					if (is_undefined(swatches))
					{
						log({ /*"No swatches found"*/ STR(1735) });
						return false;
					}
					for (IntType i = IntType(0); i < ds_list_size((IntType)(swatches)); i++)
					{
						...
						ObjType(obj_swatch, s)->name = DsMap(swatch).Value(/*"name"*/ STR(794));
						...
						VarType colors = DsMap(swatch).Value(/*"colors"*/ STR(1736));
						for (IntType j = IntType(0); j < ds_list_size((IntType)(colors)); j++)
						{
							VarType c = DsList(colors).Value(j);
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->color_names), DsMap(c).Value(/*"name"*/ STR(794)));
							array_add(VarType::CreateRef(ObjType(obj_swatch, s)->colors), hex_to_color(DsMap(c).Value(/*"value"*/ STR(804))));
							ds_map_add(ObjType(obj_swatch, s)->map, DsMap(c).Value(/*"name"*/ STR(794)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(804))));
							ds_map_add(global::minecraft_swatch_color_map, DsMap(swatch).Value(/*"name"*/ STR(794)) + /*":"*/ STR(763) + DsMap(c).Value(/*"name"*/ STR(794)), hex_to_color(DsMap(c).Value(/*"value"*/ STR(804))));
						}
						ObjType(obj_swatch, s)->size = array_length(VarType::CreateRef(ObjType(obj_swatch, s)->colors));
					}
					global::minecraft_swatch_dyes = minecraft_get_swatch(/*"dye"*/ STR(1737));
					if (global::minecraft_swatch_dyes == null_)
					{
						log({ /*"No dyes found"*/ STR(1738) });
						return false;
					}
					break;
				}
				...
				case 1733: // models
				{
					VarType characterslist = DsMap(global::load_assets_map).Value(/*"characters"*/ STR(1739));
					...
					debug_timer_stop(/*"Load characters"*/ STR(1742));
					VarType specialblockslist = DsMap(global::load_assets_map).Value(/*"special_blocks"*/ STR(1743));
					...
					debug_timer_stop(/*"Load special blocks"*/ STR(1745));
					debug_timer_start();
					global::load_assets_stage = /*"blocks"*/ STR(999);
				...
				case 999: // blocks
				{
					VarType blockslist = DsMap(global::load_assets_map).Value(/*"blocks"*/ STR(999));
					if (is_undefined(blockslist))
					{
						log({ /*"No block list found"*/ STR(1746) });
						return false;
					}
					for (IntType _it = 0, _it_max = IntType(20); _it < _it_max; _it++)
					{
						...
						{
							global::load_assets_stage = /*"done"*/ STR(1747);
							break;
						}
						VarType blockmap;
						IntType block;
						...
						if (!(block > 0))
						{
							log({ /*"Could not load block"*/ STR(1748) });
							continue;
						}
					...
					if (dev_mode_skip_blocks)
						global::load_assets_stage = /*"done"*/ STR(1747);
					if (global::load_assets_stage == /*"done"*/ STR(1747))
					{
						DsMap(self->block_name_map)[/*"flowing_water"*/ STR(1749)] = DsMap(self->block_name_map).Value(/*"water"*/ STR(792));
						DsMap(self->block_name_map)[/*"flowing_lava"*/ STR(1750)] = DsMap(self->block_name_map).Value(/*"lava"*/ STR(1751));
						DsMap(self->block_liquid_slot_map)[/*"water"*/ STR(792)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_still"*/ STR(1752));
						DsMap(self->block_liquid_slot_map)[/*"lava"*/ STR(1751)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_still"*/ STR(1753));
						DsMap(self->block_liquid_slot_map)[/*"flowing_water"*/ STR(1749)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/water_flow"*/ STR(1754));
						DsMap(self->block_liquid_slot_map)[/*"flowing_lava"*/ STR(1750)] = ds_list_find_index((IntType)(self->block_texture_ani_list), /*"block/lava_flow"*/ STR(1755));
						if (!dev_mode_skip_blocks)
							DsMap(self->block_id_map)[/*"minecraft:grass_path"*/ STR(1756)] = DsMap(self->block_id_map).Value(/*"minecraft:dirt_path"*/ STR(1757));
						...
						while (!is_undefined(key))
						{
							...
							if (!is_undefined(newid))
								newidnomc = string_replace(DsMap(curmap).Value(/*"id"*/ STR(86)), /*"minecraft:"*/ STR(813), /*""*/ STR(0));
							else
								newidnomc = /*""*/ STR(0);
							...
							if (!is_undefined(DsMap(curmap).Value(/*"data"*/ STR(1758))))
								minecraft_assets_load_legacy_block_data(self, curid, DsMap(curmap).Value(/*"data"*/ STR(1758)), IntType(0), IntType(1));
							for (IntType d = IntType(0); d < IntType(16); d++)
						...
						debug_timer_stop(/*"Load blocks"*/ STR(1759));
						...
						{
							ArrType filesarr = file_find(global::load_assets_dir + mc_blockstates_directory, /*".json"*/ STR(702));
							...
							if (ds_list_size(unusedlist) > IntType(0))
							{
								...
								for (IntType i = IntType(0); i < ds_list_size(unusedlist); i++)
									str += /*"  "*/ STR(1761) + filename_name(DsList(unusedlist).Value(i)) + /*"\n"*/ STR(948);
								log({ str });
							}
							filesarr = file_find(global::load_assets_dir + mc_models_directory + /*"block/"*/ STR(796), /*".json"*/ STR(702));
						...
						log({ /*"Loaded assets successfully"*/ STR(1763) });
