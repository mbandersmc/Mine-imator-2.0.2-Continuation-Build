...
namespace CppProject
{
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3692), string(ds_list_size(sInt(pc_type_list))) });
			...
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3206), minecraft_asset_get_name(/*"modelpart"*/ STR(748), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(515), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3207) });
	...
	BoolType textbox_draw(ScopeAny self, VarType tbx, VarType xx, VarType yy, VarType w, RealType h, BoolType contextmenu, BoolType right)
	{
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3694)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1048))))
		...
		if (sVar(window_focus) == string(tbx))
		{
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3694);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3695), idArr(tbx, line).Value(l));
						if (p == IntType(0))
