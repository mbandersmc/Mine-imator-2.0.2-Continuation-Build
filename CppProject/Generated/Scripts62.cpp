...
namespace CppProject
{
	...
	void temp_particles_init(ScopeAny self)
	{
		...
		sVar(pc_spawn_region_type) = /*"sphere"*/ STR(513);
		...
		sVar(pc_bounding_box_type) = /*"none"*/ STR(876);
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3689), string(ds_list_size(sInt(pc_type_list))) });
	...
	void temp_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(773) + DsList(global::temp_type_name_list).Value(sVar(type)) });
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3203), minecraft_asset_get_name(/*"modelpart"*/ STR(748), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(515), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3204) });
	...
	void temp_update_model_timeline_tree(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType hobj;
		if (argument_count > IntType(0))
			hobj = argument[IntType(0)];
		else
			hobj = null_;
		...
		withAll (obj_timeline, self->id)
		{
					...
					withOne (Object, DsList(self->part_list).Value(p), self->id)
					{
						for (IntType t = IntType(0); t < ds_list_size(sInt(tree_list)); t++)
						{
							withOne (obj_timeline, DsList(sInt(tree_list)).Value(t), self->id)
							{
								if (self->part_of != null_)
									break;
								withOne (Object, hobj, self->id)
								{
									sArr(part_child_save_id)[sInt(part_child_amount)] = save_id_get(self.otherId);
									sArr(part_child_parent_save_id)[sInt(part_child_amount)] = save_id_get(ObjType(obj_timeline, self.otherId)->parent);
									sArr(part_child_parent_tree_index)[sInt(part_child_amount)] = t;
									sInt(part_child_amount)++;
								}
			...
			if (hobj != null_ && !(global::_app->history_undo > 0))
			{
				idInt(hobj, usage_tl_attractor_amount) = IntType(0);
				idInt(hobj, usage_tl_ik_target_amount) = IntType(0);
				idInt(hobj, usage_tl_ik_target_angle_amount) = IntType(0);
				idInt(hobj, usage_kf_ik_target_amount) = IntType(0);
				idInt(hobj, usage_kf_ik_target_angle_amount) = IntType(0);
				idInt(hobj, usage_kf_attractor_amount) = IntType(0);
			}
			for (IntType p = IntType(0); p < ds_list_size(self->part_list); p++)
			{
				withOne (Object, DsList(self->part_list).Value(p), self->id)
	...
	BoolType textbox_draw(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType tbx, xx, yy, w, h, contextmenu, right;
		...
		if (argument_count > IntType(5))
			contextmenu = argument[IntType(5)];
		if (argument_count > IntType(6))
			right = argument[IntType(6)];
		...
		if (!sBool(mouse_left) && ((sVar(window_busy) == string(tbx) + /*"tbxrelease"*/ STR(3691)) || (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1048))))
		...
		if (sVar(window_focus) == string(tbx))
		{
				...
				if (key_press.Value(vk_right) || key_press.Value(vk_left))
				{
					if (keyboard_check(vk_control))
					{
						...
						else
						{
							...
							if (char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(937) || sVar(textbox_select_mousepos) == IntType(0))
							{
								sVar(textbox_select_mousepos)--;
							...
							{
								char_ = string_char_at(idVar(tbx, text), (IntType)(sVar(textbox_select_mousepos)));
								if (char_ == /*""*/ STR(0) || char_ == /*" "*/ STR(17) || char_ == /*"\n"*/ STR(937) || (idArr(tbx, line).Value(sVar(textbox_select_mouseline)) != /*""*/ STR(0) && sVar(textbox_select_mousepos) == IntType(0)))
									break;
			...
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx) + /*"click"*/ STR(1048)))
				sVar(window_busy) = /*""*/ STR(0);
			if (!sBool(mouse_left) && (sVar(window_busy) == string(tbx)))
				sVar(window_busy) = string(tbx) + /*"tbxrelease"*/ STR(3691);
			...
			if (changetext || idVar(tbx, last_width) != w || inserttext != /*""*/ STR(0) || deletetext != IntType(0))
			{
				for (l = IntType(1); l < idVar(tbx, lines); l++)
				{
					...
					while (idArr(tbx, line).Value(l) != /*""*/ STR(0))
					{
						...
						if (p == IntType(0))
							p = string_pos(/*" - "*/ STR(3692), idArr(tbx, line).Value(l));
						if (p == IntType(0))
				...
				for (l = IntType(0); l < idVar(tbx, lines); l++)
				{
					if (string_width(idArr(tbx, line).Value(l)) > w)
					{
						...
						for (p = string_length(idArr(tbx, line).Value(l)); p > IntType(1); p--)
						{
							...
							if ((a == /*" "*/ STR(17) || a == /*"-"*/ STR(1075)) && string_width(string_copy(idArr(tbx, line).Value(l), IntType(1), (IntType)(p - IntType(1)))) < w)
								break;
						}
						if (p == IntType(1))
						{
				...
				if ((gmlGlobal::mouse_x >= xx || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_x < xx + w || sVar(window_busy) == string(tbx)) && (gmlGlobal::mouse_y >= yy + ly || (sVar(window_busy) == string(tbx) && ly == IntType(0))) && (gmlGlobal::mouse_y < yy + ly + hh || (sVar(window_busy) == string(tbx) && (ly + lineheight > h || l == idVar(tbx, lines) - IntType(1)))))
				{
					if (sBool(mouse_left))
					{
						if (idReal(tbx, select_on_focus) > 0 && sVar(textbox_lastfocus) != tbx)
						{
							...
							sVar(window_busy) = string(tbx) + /*"click"*/ STR(1048);
						}
						else
						{
								...
								if (sVar(textbox_select_clickline) == sVar(textbox_select_mouseline) && sVar(textbox_select_clickpos) == sVar(textbox_select_mousepos))
								{
									if (current_time() - sInt(textbox_click) < IntType(500))
									{
										...
										while (true)
										{
											...
											if (c == /*" "*/ STR(17) || c == /*", "*/ STR(1011) || c == /*"("*/ STR(1070) || c == /*")"*/ STR(1074) || c == /*"["*/ STR(959) || c == /*"]"*/ STR(1199) || c == /*"+"*/ STR(1076))
												break;
											sVar(textbox_select_startpos)--;
											if (sVar(textbox_select_startpos) <= IntType(0))
											{
