...
namespace CppProject
{
	...
	IntType toast_new(ScopeAny self, IntType variant, VarType text)
	{
		...
		log({ /*"New toast"*/ STR(3753), ObjType(obj_toast, toast)->text, type });
		return toast;
	}
	...
	void toolbar_draw(ScopeAny self)
	{
		...
		padding = IntType(0);
		draw_set_font(sInt(font_value));
		capwid = string_width(text_get({ /*"toolbarfile"*/ STR(1525) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarfile"*/ STR(1525), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		if (sStr(window_state) == /*""*/ STR(0))
		{
			capwid = string_width(text_get({ /*"toolbaredit"*/ STR(1534) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbaredit"*/ STR(1534), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
			capwid = string_width(text_get({ /*"toolbarrender"*/ STR(1543) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbarrender"*/ STR(1543), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
		}
		capwid = string_width(text_get({ /*"toolbarview"*/ STR(1546) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarview"*/ STR(1546), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		capwid = string_width(text_get({ /*"toolbarhelp"*/ STR(1555) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarhelp"*/ STR(1555), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		sVar(dx) += IntType(8);
		draw_label(self, { text_get({ /*"toolbarbackup"*/ STR(3754) }), sVar(dx), sVar(dy) + IntType(22), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary * clamp(sReal(backup_text_ani), IntType(0), IntType(1)), sInt(font_value) });
		if (!(sReal(setting_advanced_mode) > 0))
		{
			if (draw_button_label(self, /*"toolbarsimplemode"*/ STR(3755), sVar(content_x) + sVar(content_width) - IntType(10), sVar(dy), null_, null_, e_button_TOOLBAR, null_, fa_right))
			{
	...
	void trial_startup()
	{
		...
		{
			log({ /*"Found key_file"*/ STR(3756), key_file });
	...
	BoolType trial_upgrade(ScopeAny self, VarType argument0)
	{
		...
		settings_save(self);
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertupgraded"*/ STR(3757) }));
		return true;
	}
	...
	VarType unzip(VarType fn, StringType dir)
	{
		...
		log({ /*"Unzipping"*/ STR(3758), fn, dir });
		VarType num = lib_unzip(fn, dir);
		log({ string(num) + /*" files were extracted"*/ STR(3759) });
		return num;
	}
	...
	StringType unzip_asset(VarType fn)
	{
		...
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"miproject;.mproj;.mani;"*/ STR(3760));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miproject;.mproj;.mani;"*/ STR(3760));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory, /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3761));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3761));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	StringType unzip_model(StringType fn)
	{
		...
		name = filename_new_ext(filename_name(fn), /*""*/ STR(0));
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"json"*/ STR(3762));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*"json"*/ STR(3762));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/item/"*/ STR(3763), /*".json;"*/ STR(3764));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/block/"*/ STR(3765), /*".json;"*/ STR(3764));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	RealType window_drop_enter(ScopeAny self, ArrType files)
	{
		...
		if (array_length(VarType::CreateRef(files)) > IntType(1))
			return false;
		return string_contains(asset_exts, /*"*"*/ STR(1077) + filename_ext(files.Value(IntType(0))) + /*";"*/ STR(3768));
	}
