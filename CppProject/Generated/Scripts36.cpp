...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1777))) && !(DsMap(map).Value(/*"visible"*/ STR(1777)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"type"*/ STR(773))))
		{
			log({ /*"Missing parameter \"type\""*/ STR(1810) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(812))))
		{
			log({ /*"Missing array \"from\""*/ STR(1811) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(813))))
		{
			log({ /*"Missing array \"to\""*/ STR(1812) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(821))))
		{
			log({ /*"Missing array \"uv\""*/ STR(1813) });
			return null_;
		}
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			self->type = DsMap(map).Value(/*"type"*/ STR(773));
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1814)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1808)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1815)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1816)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1817))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1817));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1818)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1819)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1820)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(812)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(813)));
			if (self->type == /*"plane"*/ STR(1821))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1822)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1821))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1821))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1823)), false);
				if (self->is3d > 0)
				{
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1790)), true);
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(821)));
			...
			{
				if (is_string(DsMap(windmap).Value(/*"axis"*/ STR(800))))
				{
					if (DsMap(windmap).Value(/*"axis"*/ STR(800)) == /*"y"*/ STR(801))
						self->wind_wave = e_vertex_wave_Z_ONLY;
					else
						self->wind_wave = e_vertex_wave_ALL;
					
				}
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(802))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(802));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1824))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1824));
			}
			if (self->type == /*"block"*/ STR(508))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(872) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1825), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(6) && ext != /*".jpg"*/ STR(708) && ext != /*".jpeg"*/ STR(709))
			return IntType(0);
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1826) });
				return null_;
			}
			
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(515) + self->name))
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(532))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(532));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(532))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(532))));
				}
				else
			...
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(859)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(795))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(795))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(744) + curstate))
						log({ /*"model/state/"*/ STR(1827) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						self->name = curstate;
						IntType valuelist = ds_map_find_value((IntType)(DsMap(map).Value(/*"states"*/ STR(795))), curstate);
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							self->value_name[v] = DsMap(curvalue).Value(/*"value"*/ STR(797));
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(747) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1828) + self->value_name.Value(v) + dev_mode_name_translation_message });
							if (!is_undefined(DsMap(curvalue).Value(/*"file"*/ STR(115))))
								self->value_file[v] = model_file_load(dir + DsMap(curvalue).Value(/*"file"*/ STR(115)));
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(532))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(532));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(532))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(532)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(532)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(532)), true);
								}
							
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1829)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(719))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(719))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1830))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1830))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1831))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1831)), true);
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(859))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(859));
						}
						DsMap(ObjType(obj_model, self.otherId)->states_map)[curstate] = self->id;
						curstate = ds_map_find_next((IntType)(DsMap(map).Value(/*"states"*/ STR(795))), curstate);
					}
					
				}
				
			}
			if (is_string(DsMap(map).Value(/*"default_state"*/ STR(799))))
				self->default_state = string_get_state_vars(DsMap(map).Value(/*"default_state"*/ STR(799)));
			else
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1821) && self->is3d > 0)
				{
	...
	RealType model_part_fill_shape_vbuffer_map(VarType part, IntType vbufmap, IntType alphamap, VecType bend)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			withOne (obj_model_shape, DsList(idInt(part, shape_list)).Value(s), noone)
			{
				...
				if (self->type == /*"block"*/ STR(508) && self->bend_shape > 0 && isbent)
	...
	MatrixType model_part_get_bend_matrix(ScopeAny self, VarType part, VecType bend, VecType pos, VecType sca)
	{
		if (idInt(part, bend_part) == null_)
			return MAT_IDENTITY;
		for (IntType i = X_; i <= Z_; i++)
		{
			if (bend.Real(i) == IntType(0))
				continue;
			bend.Real(i) = clamp(bend.Real(i), idVec(part, bend_direction_min).Real(i), idVec(part, bend_direction_max).Real(i));
			if (idVec(part, bend_invert).Real(i))
				bend.Real(i) *= -1.0;
			if (!idVec(part, bend_axis).Real(i))
				bend.Real(i) = IntType(0);
		}
		switch ((IntType)idInt(part, bend_part))
		{
			...
			case e_part_LEFT:
			{
				pos.Real(X_) = idVar(part, bend_offset);
				if (self->subAssetId == ID_obj_model_shape)
					pos.Real(X_) -= sVar(position).Value(X_);
				break;
			}
			case e_part_FRONT:
			case e_part_BACK:
			{
				pos.Real(Y_) = idVar(part, bend_offset);
				if (self->subAssetId == ID_obj_model_shape)
					pos.Real(Y_) -= sVar(position).Value(Y_);
				break;
			}
			case e_part_UPPER:
			case e_part_LOWER:
			{
				pos.Real(Z_) = idVar(part, bend_offset);
				if (self->subAssetId == ID_obj_model_shape)
					pos.Real(Z_) -= sVar(position).Value(Z_);
				break;
			}
		}
		
		MatrixType mat = matrix_build(pos.Real(X_), pos.Real(Y_), pos.Real(Z_), bend.Real(X_), bend.Real(Y_), bend.Real(Z_), sca.Real(X_), sca.Real(Y_), sca.Real(Z_));
		if (self->subAssetId == ID_obj_model_shape)
			mat = matrix_multiply(matrix_build(-pos.Real(X_), -pos.Real(Y_), -pos.Real(Z_), sVar(rotation).Value(X_), sVar(rotation).Value(Y_), sVar(rotation).Value(Z_), 1.0, 1.0, 1.0), mat);
		return mat;
	}
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = (global::_app->project_bend_style == /*"blocky"*/ STR(1832)) && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_))) && sVar(bend_size) == null_;
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1833) ? 4.0 : 1.0)) : sVar(bend_size));
