...
namespace CppProject
{
	...
	RealType tl_update_matrix(ScopeAny self, BoolType usepaths, BoolType updateik, BoolType updatepose)
	{
	...
	{
		sVar(type_name) = text_get({ /*"type"*/ STR(780) + DsList(global::tl_type_name_list).Value(sVar(type)) });
		if (sVar(part_of) != null_)
		{
			if (sVar(type) == e_tl_type_BODYPART)
			{
				if (sVar(model_part) != null_)
					sVar(type_name) = minecraft_asset_get_name(/*"modelpart"*/ STR(753), idVar(sVar(model_part), name));
				else
					sVar(type_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3878) });
			...
			sVar(type_name) = text_get({ /*"timelinepartof"*/ STR(3879), sVar(type_name), string_remove_newline(idVar(sVar(part_of), display_name)) });
		}
		else
			if (sVar(temp) != null_)
				sVar(type_name) = text_get({ /*"timelineinstanceof"*/ STR(3880), idVar(sVar(temp), display_name) });
	...
	void tl_update_values(ScopeAny self)
	{
		...
		if (sVar(keyframe_transition) == /*"bezier"*/ STR(554))
