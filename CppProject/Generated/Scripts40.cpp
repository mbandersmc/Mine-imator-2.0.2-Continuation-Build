/*
	...
	[ Generated on 2024.01.31 13:42:39 ]
*/
...
namespace CppProject
{
	void panel_draw_content(ScopeAny self)
	{
		...
		else
		{
			...
			if (sVar(dh) < minh)
			{
				if (idInt(sVar(tab), panel) != DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(7))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1969));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*".miparticles;*.particles"*/ STR(1969));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1970));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		IntType rootmap;
		BoolType legacy;
		if (string_contains(filename_ext(fn), /*".miparticles"*/ STR(690)))
		{
			log({ /*"Opening particles"*/ STR(1971), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1972), fn });
		...
		if (!legacy)
		{
			...
			{
				temp_particles_type_clear(self);
				project_load_particles(self, DsMap(rootmap).Value(/*"particles"*/ STR(528)));
				temp_particles_restart(self);
			}
	...
	RealType particles_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miparticles"*/ STR(690));
		log({ /*"Saving particles"*/ STR(1974), fn });
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(537) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						{
							VarType att, pointdata, curdata;
							ArrType pointpos, curpos;
							RealType pointdis, curdis;
							IntType points;
							att = sVar(value).Value(e_value_ATTRACTOR);
							curdata = ArrType::From({ IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0) });
							pointdata = ArrType::From({ IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0), IntType(0) });
							curpos = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							...
							{
								curdata = idArr(att, path_table_matrix).Value(j);
								curpos = ArrType::From({ curdata.Value(X_), curdata.Value(Y_), curdata.Value(Z_) });
							...
							VarType d;
							ArrType t;
							v = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							d = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							p = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							t = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							n = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							b = ArrType::From({ IntType(0), IntType(0), IntType(0) });
							t = ArrType::From({ pointdata.Value(PATH_TANGENT_X), pointdata.Value(PATH_TANGENT_Y), pointdata.Value(PATH_TANGENT_Z) });
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(883) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
							if (idVar(temp, pc_bounding_box_type) == /*"ground"*/ STR(142))
							{
							...
							else
							{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1351))
								{
									if (!(idVar(temp, pc_spawn_region_use) > 0))
										continue;
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(531))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1345))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(537) && idVar(temp, pc_spawn_region_path) != null_)
												{
													VarType att, pointdata, curdata;
													ArrType pointpos, curpos;
													RealType pointdis, curdis;
													...
													d = vec3_dot(v, t);
													p = vec3_add(pointpos, vec3_mul(t, d));
													RealType dis = point3D_distance(ObjType(obj_particle, pt)->pos, p);
													if (dis > idVar(temp, pc_spawn_region_path_radius))
													{
														d = vec3_normalize(vec3_sub(ObjType(obj_particle, pt)->pos, p));
														ObjType(obj_particle, pt)->pos = vec3_add(p, vec3_mul(d, idVar(temp, pc_spawn_region_path_radius)));
												...
												else
												{
													...
													{
														curdata = idArr(att, path_table_matrix).Value(j);
														curpos = ArrType::From({ curpos.Real(X_), curpos.Real(Y_), curpos.Real(Z_) });
														curdis = point3D_distance(ObjType(obj_particle, pt)->pos, curpos);
														if (curdis < pointdis)
														{
															pointdis = curdis;
