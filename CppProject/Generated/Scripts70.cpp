...
namespace CppProject
{
	...
	void tl_value_set_start(ScopeAny self, IntType script, BoolType combine)
	{
		...
		else
		{
			...
			log({ /*"Action"*/ STR(1214), script_get_name(script) });
		}
	...
	void toast_draw(ScopeAny self, IntType toast)
	{
		...
		sVar(content_x) = floor(sReal(window_width) / 2.0 - ObjType(obj_toast, toast)->toast_width / 2.0) + (IntType(32) * ease(/*"easeoutcirc"*/ STR(574), ObjType(obj_toast, toast)->remove_alpha));
		...
		{
			if (draw_button_icon(self, { string(toast) + /*"close"*/ STR(1102), sVar(dx), sVar(dy), IntType(24), IntType(24), false, icons_CLOSE, null_, false }))
				ObjType(obj_toast, toast)->remove = true;
			hei = IntType(40);
			wid += IntType(32);
		}
	...
	IntType toast_new(ScopeAny self, IntType variant, VarType text)
	{
		...
		log({ /*"New toast"*/ STR(3886), ObjType(obj_toast, toast)->text, type });
		return toast;
	}
	...
	void toolbar_draw(ScopeAny self)
	{
		...
		padding = IntType(0);
		draw_set_font(sInt(font_value));
		capwid = string_width(text_get({ /*"toolbarfile"*/ STR(1574) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarfile"*/ STR(1574), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		if (sStr(window_state) == /*""*/ STR(0))
		{
			capwid = string_width(text_get({ /*"toolbaredit"*/ STR(1583) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbaredit"*/ STR(1583), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
			capwid = string_width(text_get({ /*"toolbarrender"*/ STR(1592) })) + IntType(16);
			toolbar_draw_button(self, /*"toolbarrender"*/ STR(1592), sVar(dx), sVar(dy), capwid);
			sVar(dx) += capwid + padding;
		}
		capwid = string_width(text_get({ /*"toolbarview"*/ STR(1595) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarview"*/ STR(1595), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		capwid = string_width(text_get({ /*"toolbarhelp"*/ STR(1604) })) + IntType(16);
		toolbar_draw_button(self, /*"toolbarhelp"*/ STR(1604), sVar(dx), sVar(dy), capwid);
		sVar(dx) += capwid + padding;
		sVar(dx) += IntType(8);
		draw_label(self, { text_get({ /*"toolbarbackup"*/ STR(3887) }), sVar(dx), sVar(dy) + IntType(22), fa_left, fa_bottom, global::c_text_secondary, global::a_text_secondary * clamp(sReal(backup_text_ani), IntType(0), IntType(1)), sInt(font_value) });
		if (!(sReal(setting_advanced_mode) > 0))
		{
			if (draw_button_label(self, /*"toolbarsimplemode"*/ STR(3888), sVar(content_x) + sVar(content_width) - IntType(10), sVar(dy), null_, null_, e_button_TOOLBAR, null_, fa_right))
			{
	...
	BoolType toolbar_draw_button(ScopeAny self, StringType name, VarType xx, VarType yy, IntType wid, BoolType hasmenu)
	{
		...
		trigger = false;
		click = false;
		if (sVar(window_busy) == /*"contextmenu"*/ STR(1029))
	...
	void trial_startup()
	{
		...
		{
			log({ /*"Found key_file"*/ STR(3889), key_file });
	...
	BoolType trial_upgrade(ScopeAny self, VarType argument0)
	{
		...
		settings_save(self);
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertupgraded"*/ STR(3890) }));
		return true;
	}
	...
	VarType unzip(VarType fn, StringType dir)
	{
		...
		log({ /*"Unzipping"*/ STR(3891), fn, dir });
		VarType num = lib_unzip(fn, dir);
		log({ string(num) + /*" files were extracted"*/ STR(3892) });
		return num;
	}
	...
	StringType unzip_asset(VarType fn)
	{
		...
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"miproject;.mproj;.mani;"*/ STR(3893));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"miproject;.mproj;.mani;"*/ STR(3893));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory, /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3894));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"miobject;miparticles;.object;.particles;.json;"*/ STR(3894));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	StringType unzip_model(StringType fn)
	{
		...
		name = filename_new_ext(filename_name(fn), /*""*/ STR(0));
		unzip(fn);
		validfile = file_find_single(unzip_directory, /*"json"*/ STR(3895));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + name + /*"/"*/ STR(20), /*"json"*/ STR(3895));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/item/"*/ STR(3896), /*".json;"*/ STR(3897));
		if (!file_exists_lib(validfile))
			validfile = file_find_single(unzip_directory + /*"assets/minecraft/models/block/"*/ STR(3898), /*".json;"*/ STR(3897));
		if (!file_exists_lib(validfile))
			return /*""*/ STR(0);
		return validfile;
	}
	...
	RealType window_drop_enter(ScopeAny self, ArrType files)
	{
		...
		if (array_length(VarType::CreateRef(files)) > IntType(1))
			return false;
		return string_contains(asset_exts, /*"*"*/ STR(1110) + filename_ext(files.Value(IntType(0))) + /*";"*/ STR(3901));
	}
	...
	VarType value_get_real(VarType val, VarType def)
	{
		if (is_int32(val) || is_int64(val) || is_real(val) || is_bool(val))
			...
			if (is_string(val) && val == /*"null"*/ STR(1228))
				return null_;
