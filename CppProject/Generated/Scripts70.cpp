...
namespace CppProject
{
	...
	RealType view_draw(ScopeAny self, IntType view)
	{
		...
		StringType listname;
		BoolType menuactive;
		listname = (view == sInt(view_main) ? /*"viewcameramain"*/ STR(1452) : /*"viewcamerasecond"*/ STR(1453));
		menuactive = false;
		...
		if (location != /*"full"*/ STR(2673) && location != /*"top"*/ STR(187) && location != /*"bottom"*/ STR(186))
		{
		...
		if (view == sInt(view_second) && window_get_current() != e_window_VIEW_SECOND)
		{
			...
			if (sVar(window_busy) == /*"viewclickcaption"*/ STR(3810))
			{
				...
				{
					ObjType(obj_view, sInt(view_main))->location = /*"full"*/ STR(2673);
			...
			if (sVar(window_busy) == /*"viewmove"*/ STR(3775))
			{
				...
				if (gmlGlobal::mouse_y < sReal(view_area_y) + sReal(view_area_height) * 0.3)
					mouselocation += /*"_top"*/ STR(1890);
				else
					if (gmlGlobal::mouse_y >= sReal(view_area_y) + sReal(view_area_height) * 0.7)
						mouselocation += /*"_bottom"*/ STR(1891);
				
				if (mouselocation == /*"_top"*/ STR(1890))
					mouselocation = /*"top"*/ STR(187);
				if (mouselocation == /*"_bottom"*/ STR(1891))
		...
		if (view == sInt(view_second))
		{
			...
			if ((mouseonresizever || mouseonresizehor) || (sVar(window_busy) == /*"viewresizeboth"*/ STR(3768) || sVar(window_busy) == /*"viewresizever"*/ STR(3769) || sVar(window_busy) == /*"viewresizehor"*/ STR(3767)))
			{
				...
				if (location == /*"right_bottom"*/ STR(2674))
				{
					lineleft = (mouseonresizehor || sVar(window_busy) == /*"viewresizehor"*/ STR(3767) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3768));
					linebottom = (mouseonresizever || sVar(window_busy) == /*"viewresizever"*/ STR(3769) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3768));
				}
		...
		if (ObjType(obj_view, view)->quality == e_view_mode_RENDER)
		{
			...
			{
				infotext = text_get({ /*"viewrenderpass"*/ STR(3815), text_get({ /*"viewmodepass"*/ STR(1451) + DsList(global::render_pass_list).Value(sInt(project_render_pass)) }) });
	...
	{
		self->show = true;
		self->location = /*"full"*/ STR(2673);
		self->location_last = /*"full"*/ STR(2673);
		self->surface = null_;
