...
namespace CppProject
{
	...
	void view_shape_particles(IntType tl)
	{
		...
		if (temp == global::temp_edit && ObjType(obj_tab, global::_app->template_editor)->show > 0)
		{
			...
			{
				if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1351))
			...
			if (idVar(temp, pc_bounding_box_type) == /*"custom"*/ STR(605))
			{
				draw_set_color(c_red);
					...
					if (app_mouse_box(self, sVar(content_x) + (nextcoord2d.Real(X_) - IntType(13)), sVar(content_y) + (nextcoord2d.Real(Y_) - IntType(13)), IntType(26), IntType(26)) && sBool(content_mouseon) && ObjType(obj_view, view)->control_mouseon_last == null_)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							...
							if (sVar(setting_timeline_select_jump) > 0)
								tl_jump(pointtl);
							sVar(window_busy) = /*"viewpathpointclick"*/ STR(3953);
						}
					}
				}
			}
	...
	RealType view_toolbar_draw(ScopeAny self, IntType view, RealType xx, RealType yy)
	{
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolselect"*/ STR(3954), xx, yy, IntType(24), IntType(24), sBool(setting_tool_select), icons_SELECT, null_, false, /*"viewtoolselecttip"*/ STR(3955) }))
			{
		...
		if (draw_button_icon(self, { /*"viewtoolmove"*/ STR(3956), xx, yy, IntType(24), IntType(24), sVar(setting_tool_move), icons_MOVE, null_, false, /*"viewtoolmovetip"*/ STR(3957) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolrotate"*/ STR(3958), xx, yy, IntType(24), IntType(24), sBool(setting_tool_rotate), icons_ROTATE, null_, false, /*"viewtoolrotatetip"*/ STR(3959) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolscale"*/ STR(3960), xx, yy, IntType(24), IntType(24), sBool(setting_tool_scale), icons_SCALE, null_, false, /*"viewtoolscaletip"*/ STR(3961) }))
			{
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolbend"*/ STR(3962), xx, yy, IntType(24), IntType(24), sBool(setting_tool_bend), icons_BEND, null_, false, /*"viewtoolbendtip"*/ STR(3963) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtooltransform"*/ STR(3964), xx, yy, IntType(24), IntType(24), sBool(setting_tool_transform), icons_MULTITRANSFORM, null_, false, /*"viewtooltransformtip"*/ STR(3965) }))
			{
				action_tools_disable_all(self);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(119)))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"viewclick"*/ STR(3966);
			...
			if ((!(cam > 0) || editcamobj) && sBool(mouse_right_pressed))
			{
				...
				sVar(window_busy) = /*"viewmovecamera"*/ STR(3967);
				sVar(window_focus) = string(view);
				if (cam > 0)
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(119)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3968) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		{
			if (sVar(window_busy) == /*"viewclick"*/ STR(3966))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if ((!(cam > 0) || editcamobj) && sReal(mouse_move) > IntType(5))
				{
					...
					{
						sVar(window_busy) = /*"viewpancamera"*/ STR(3969);
						sVar(window_focus) = string(view);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
					else
					{
						sInt(view_click_x) = display_mouse_get_x();
						sInt(view_click_y) = display_mouse_get_y();
						sVar(window_busy) = /*"viewrotatecamera"*/ STR(3968);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
			...
			if (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3968))
			{
				if (cam != null_)
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3967))
			{
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2959);
				else
				{
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2960);
					global::render_samples = -IntType(1);
				}
			...
			if (sVar(window_busy) == /*"viewpancamera"*/ STR(3969))
			{
		...
		if (sVar(window_busy) == /*"viewpathpointclick"*/ STR(3953))
			sVar(window_busy) = /*""*/ STR(0);
	}
		...
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				...
				if (sStr(window_state) == /*"startup"*/ STR(29))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2763))
					{
			...
			case e_window_VIEW_SECOND:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(ObjType(obj_view, sInt(view_second))->title + /*" - Mine-imator"*/ STR(679));
				break;
			}
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3221) }) + /*" - Mine-imator"*/ STR(679));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3970) : /*"exporttimelefthours"*/ STR(3971)), string(timelefthours) }) + /*", "*/ STR(1032);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3972) : /*"exporttimeleftminutes"*/ STR(3973)), string(timeleftmins) }) + /*" "*/ STR(21) + text_get({ /*"exporttimeleftand"*/ STR(3974) }) + /*" "*/ STR(21);
		timeleftstr += text_get({ ((timeleftsecs == IntType(1)) ? /*"exporttimeleftsecond"*/ STR(3975) : /*"exporttimeleftseconds"*/ STR(3976)), string(timeleftsecs) });
		draw_label(self, { text_get({ /*"exporttimeleft"*/ STR(3977), timeleftstr }), framex + (RealType)framew / 2.0, framey + frameh + IntType(33), fa_center, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading_big) });
		VarType loadtext, loadw;
		RealType sw, sh;
		loadtext = text_get({ /*"exportloading"*/ STR(3978), string(floor(perc * IntType(100))) });
		loadw = framew;
		...
		if (usesamples > 0)
			samplecount = text_get({ /*"exportsamples"*/ STR(3979), string(max({ global::render_samples, IntType(1) })), string(sVar(project_render_samples)) });
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
			text = text_get({ /*"exportframe"*/ STR(3980), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1131) + samplecount + /*")"*/ STR(1107)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		window_set_caption(loadtext + /*" - Mine-imator"*/ STR(679));
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(53) ? text_get({ /*"exportmovietitle"*/ STR(3981) }) : text_get({ /*"exportimagetitle"*/ STR(3982) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
		draw_label(self, { text_get({ /*"exportstop"*/ STR(3983) }), framex + (RealType)framew / 2.0, framey - IntType(16), fa_center, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		return 0.0;
	}
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		{
			global::load_assets_stage = /*"exit"*/ STR(3984);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(3984))
			{
		...
		RealType screenwid, screenhei, splashwid;
		screenwid = IntType(740);
		screenhei = IntType(450);
		splashwid = IntType(550);
		RealType xoff, yoff;
		xoff = floor((sReal(window_width) / 2.0) - (screenwid / 2.0));
		yoff = floor((sReal(window_height) / 2.0) - (screenhei / 2.0));
		...
		draw_dropshadow(self, xoff, yoff, screenwid, screenhei, global::c_accent, IntType(1));
		draw_box(xoff, yoff, screenwid, screenhei, false, global::c_level_middle, IntType(1));
		IntType pattern = (sInt(setting_theme) == global::theme_light ? IntType(0) : IntType(1));
		draw_sprite_ext(ID_spr_pattern_left, pattern, (IntType)(xoff), (IntType)(yoff), 138.0 / sprite_get_width(ID_spr_pattern_left), screenhei / sprite_get_height(ID_spr_pattern_left), 0.0, c_white, 1.0);
		draw_sprite(ID_spr_load_assets, IntType(0), (IntType)(xoff + IntType(95)), (IntType)(yoff + IntType(207)));
		draw_label(self, { /*"Mine-imator "*/ STR(3985) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading) });
		...
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(3986), global::_app->setting_minecraft_assets_version, floor(global::load_assets_progress * IntType(100)) }), xoff + IntType(95), yoff + IntType(437), fa_middle, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		draw_gradient(xoff + IntType(190), yoff, shadow_size * IntType(2), screenhei, c_black, shadow_alpha * IntType(2), IntType(0), IntType(0), shadow_alpha * IntType(2));
		draw_gradient(xoff + IntType(190), yoff, screenwid - IntType(190), shadow_size * IntType(2), c_black, shadow_alpha * IntType(2), shadow_alpha * IntType(2), IntType(0), IntType(0));
		if (global::load_assets_splash != null_)
		{
			RealType texwid, texhei;
			texwid = sprite_get_width(global::load_assets_splash);
			texhei = sprite_get_height(global::load_assets_splash);
			RealType scale = IntType(1);
			if (texwid > splashwid || texhei > screenhei)
				scale = max({ texwid / splashwid, texhei / screenhei });
			draw_sprite_ext(global::load_assets_splash, IntType(0), (IntType)(xoff + IntType(190) + (IntType(275) - ((texwid / scale) / 2.0))), (IntType)(yoff + (IntType(225) - ((texhei / scale) / 2.0))), 1.0 / scale, 1.0 / scale, 0.0, c_white, 1.0);
		}
		if (global::load_assets_credits != /*""*/ STR(0))
			draw_label(self, { text_get({ /*"startupsplashauthor"*/ STR(3987), global::load_assets_credits }), xoff + IntType(95), yoff + IntType(289) + IntType(31), fa_middle, fa_top, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		draw_box(xoff, yoff + screenhei - IntType(8), screenwid, IntType(8), false, global::c_level_top, .8);
		draw_box(xoff, yoff + screenhei - IntType(8), screenwid * global::load_assets_progress, IntType(8), false, global::c_accent, IntType(1));
		draw_outline(xoff, yoff, screenwid, screenhei, IntType(1), global::c_border, global::a_border, true);
		draw_dropshadow(self, xoff, yoff, screenwid, screenhei, c_black, IntType(1));
		global::current_step++;
		return 0.0;
	}
