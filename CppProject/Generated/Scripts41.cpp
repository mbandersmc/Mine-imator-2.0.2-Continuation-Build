...
namespace CppProject
{
	void popup_pattern_editor_show(ScopeAny self, VarType obj)
	{
		withOne (obj_popup, sInt(popup_pattern_editor), self->id)
		{
			...
			if (self->pattern_resource != res || array_length(VarType::CreateRef(self->pattern_sprites)) == IntType(0))
			{
				...
				self->res_ratio = ceil((RealType)sprite_get_width((IntType)(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1759)))) / sprite_get_width((IntType)(DsMap(ObjType(obj_resource, global::mc_res)->model_texture_map).Value(/*"entity/banner_base"*/ STR(1759)))));
				...
				for (IntType i = IntType(0); i < ds_list_size(global::minecraft_pattern_list); i++)
				{
					...
					array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner/"*/ STR(1761) + bannername), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
				}
				global::shader_mask = false;
				array_add(VarType::CreateRef(self->pattern_sprites), texture_create_crop(DsMap(idInt(res, model_texture_map)).Value(/*"entity/banner_base"*/ STR(1759)), self->res_ratio, self->res_ratio, IntType(20) * self->res_ratio, IntType(40) * self->res_ratio));
	...
	{
		ObjType(obj_textbox, ObjType(obj_popup, sInt(popup_saveas))->tbx_name)->text = text_get({ /*"saveascopy"*/ STR(2015), sVar(project_name) });
		ObjType(obj_popup, sInt(popup_saveas))->folder = text_get({ /*"saveascopy"*/ STR(2015), filename_name(sStr(project_folder)) });
	...
	void popup_saveas_draw(ScopeAny self)
	{
		...
		if (draw_textfield(self, { /*"newprojectname"*/ STR(1998), sVar(dx), sVar(dy), sVar(dw), IntType(24), idInt(sInt(popup), tbx_name), null_, text_get({ /*"saveascopy"*/ STR(2015), sVar(project_name) }), /*"top"*/ STR(187) }) || issue)
		{
			...
			if (idVar(sInt(popup), folder) == /*""*/ STR(0))
				idVar(sInt(popup), folder) = text_get({ /*"saveascopy"*/ STR(2015), sVar(project_name) });
		...
		if (draw_button_icon(self, { /*"newprojectchangefolder"*/ STR(2002), sVar(dx) + sVar(dw) - IntType(24), sVar(dy) + IntType(8), IntType(24), IntType(24), false, icons_FOLDER_EDIT, null_, null_, /*"tooltipchangefolder"*/ STR(2003) }))
		{
		...
		if (draw_button_label(self, { /*"saveassave"*/ STR(2016), sVar(dx) + sVar(dw), sVar(dy), null_, null_, e_button_PRIMARY, null_, e_anchor_RIGHT }))
	...
	void popup_show(ScopeAny self, IntType argument0)
	{
		...
		log({ /*"Show popup"*/ STR(2017), idVar(sInt(popup), name) });
	...
	RealType popup_switch(ScopeAny self, IntType argument0)
	{
		...
		sVar(window_busy) = /*"popup"*/ STR(1561) + idVar(sInt(popup_switch_to), name);
	...
	void popup_upgrade_draw(ScopeAny self)
	{
		...
		if (draw_button_icon(self, { /*"upgradeleft"*/ STR(2018), sVar(content_x) + IntType(12), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_LEFT }))
		{
		...
		if (draw_button_icon(self, { /*"upgraderight"*/ STR(2019), sVar(content_x) + sVar(content_width) - (IntType(12) + IntType(20)), sVar(dy) - (RealType)sprite_get_height(ID_spr_upgrade_img) / 2.0 - IntType(16), IntType(20), IntType(32), false, icons_CHEVRON_RIGHT }))
		{
			idReal(sInt(popup), page) = mod_fix((idReal(sInt(popup), page) + IntType(1)), IntType(3));
			idReal(sInt(popup), page_ani) = IntType(0);
		...
		StringType text = string_limit_ext(text_get({ /*"upgradepage"*/ STR(2020) + string(idReal(sInt(popup), page)) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		text = string_limit_ext(text_get({ /*"upgradeinfo"*/ STR(2021) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		...
		draw_inputbox(self, { /*"upgrade"*/ STR(106), sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), /*"XXXXXXXX"*/ STR(2022), ObjType(obj_popup, sInt(popup_upgrade))->tbx_key, null_, false, false, sVar(font_upgrade), e_inputbox_BIG });
		draw_box_hover({ sVar(dx) + (RealType)sVar(dw) / 2.0 - wid / 2.0, sVar(dy), wid, IntType(48), global::microani_arr.Value(e_microani_PRESS) });
		if (draw_button_icon(self, { /*"upgradekeypaste"*/ STR(2023), sVar(dx) + (RealType)sVar(dw) / 2.0 + wid / 2.0 + IntType(8), sVar(dy) + IntType(10), IntType(24), IntType(24), false, icons_PASTE, null_, false, /*"tooltippastekey"*/ STR(2024) }))
		...
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"upgradecontinue"*/ STR(2025), sVar(dx) + sVar(dw), sVar(dy), null_, icons_KEY, e_button_PRIMARY, null_, e_anchor_RIGHT }))
		{
			...
			if (upgrade)
			{
				...
				ObjType(obj_popup, sInt(popup_upgrade))->warntext = /*"errorupgrade"*/ STR(2026);
	...
	{
		draw_set_font(sInt(font_value));
		StringType text = string_limit_ext(text_get({ /*"welcomedescription"*/ STR(2027) }), (sVar(dw) - IntType(40)) + IntType(8), no_limit);
		draw_label(self, { text, floor(sVar(dx) + (RealType)sVar(dw) / 2.0), sVar(dy), fa_middle, fa_top, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += string_height(text) + IntType(19);
		tab_control_button_label(self);
		if (draw_button_label(self, { /*"welcomecontinue"*/ STR(2028), sVar(content_x) + sVar(content_width) - IntType(12), sVar(dy), null_, null_, e_button_PRIMARY, null_, fa_right }))
		{
			settings_save(self);
			popup_close(self);
		}
	...
	RealType preview_draw(ScopeAny self, IntType preview, VarType xx, VarType yy, VarType width, RealType height)
	{
		...
		if (mouseon && sBool(content_mouseon) && !playbutton)
		{
			...
			{
				sVar(window_busy) = ((is3d > 0) ? /*"previewrotate"*/ STR(2029) : /*"previewmove"*/ STR(2030));
		...
		{
			RealType zd, m;
			if (sVar(window_busy) == /*"previewrotate"*/ STR(2029))
			{
			...
			if (sVar(window_busy) == /*"previewmove"*/ STR(2030))
			{
		...
		withOne (Object, preview, self->id)
		{
				...
				surface_set_target((IntType)(sVar(surface)));
			{
				...
				if (is3d > 0)
				{
					...
					render_set_uniform_int(/*"uFogShow"*/ STR(1883), IntType(0));
					render_set_uniform_int(/*"uColorsExt"*/ STR(2031), IntType(1));
					render_set_uniform_color(/*"uRGBAdd"*/ STR(2032), tl_value_default(e_value_RGB_ADD), IntType(1));
					render_set_uniform_color(/*"uRGBSub"*/ STR(2033), tl_value_default(e_value_RGB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBAdd"*/ STR(2034), tl_value_default(e_value_HSB_ADD), IntType(1));
					render_set_uniform_color(/*"uHSBSub"*/ STR(2035), tl_value_default(e_value_HSB_SUB), IntType(1));
					render_set_uniform_color(/*"uHSBMul"*/ STR(2036), tl_value_default(e_value_HSB_MUL), IntType(1));
					render_set_uniform_color(/*"uMixColor"*/ STR(2037), tl_value_default(e_value_MIX_COLOR), tl_value_default(e_value_MIX_PERCENT));
				...
				else
				{
					...
					switch ((IntType)idVar(sVar(select), type))
					{
						case e_res_type_FONT:
						{
							...
							draw_text_transformed((IntType)(dx), (IntType)(dy), /*"AaBbCc"*/ STR(2038), sReal(zoom), sReal(zoom), 0.0);
		...
		if (Obj(idVar(preview, select))->subAssetId != ID_obj_resource && idVar(idVar(preview, select), type) == e_temp_type_PARTICLE_SPAWNER)
		{
			...
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2039), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), idBool(preview, spawn_active), icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2040) }))
					idBool(preview, spawn_active) = !idBool(preview, spawn_active);
			}
			else
			{
				if (draw_button_icon(self, { /*"previewspawn"*/ STR(2039), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, icons_PARTICLES, null_, false, /*"tooltipparticlesspawn"*/ STR(2040) }))
		...
		if (playbutton)
		{
			if (draw_button_icon(self, { /*"previewplay"*/ STR(2041), xx + width - IntType(36), yy + height - IntType(36), IntType(24), IntType(24), false, (isplaying > 0) ? icons_STOP : icons_PLAY, null_, false, (isplaying > 0) ? /*"tooltipstop"*/ STR(2042) : /*"tooltipplay"*/ STR(2043) }))
			{
				if (isplaying > 0)
				{
	...
	void preview_event_create(ScopeAny self)
	{
		...
		sStr(pack_model_texture) = /*"entity/player/wide/steve"*/ STR(2044);
	...
	void project_backup(ScopeAny self)
	{
		...
		log({ /*"Backup"*/ STR(2045), fn });
		for (RealType b = sReal(setting_backup_amount) - IntType(1); b > IntType(0); b--)
			if (file_exists_lib(fn + /*".backup"*/ STR(2046) + string(b)))
				file_rename_lib(fn + /*".backup"*/ STR(2046) + string(b), fn + /*".backup"*/ STR(2046) + string(b + IntType(1)));
		project_save(self, { fn + /*".backup"*/ STR(2046) + ((sReal(setting_backup_amount) > IntType(1)) ? /*"1"*/ STR(1079) : /*""*/ STR(0)) });
		project_reset_backup(self);
		log({ /*"Backup saved"*/ STR(2047) });
	}
	...
	RealType project_create(ScopeAny self)
	{
		...
		log({ /*"Creating project"*/ STR(2049), dirname });
		project_reset(self);
