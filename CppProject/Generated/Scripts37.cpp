...
namespace CppProject
{
	VarType model_file_load_shape(Scope<obj_model_part> self, VarType map, IntType res)
	{
		if (!is_undefined(DsMap(map).Value(/*"visible"*/ STR(1835))) && !(DsMap(map).Value(/*"visible"*/ STR(1835)) > 0))
			return IntType(0);
		if (!is_string(DsMap(map).Value(/*"type"*/ STR(780))))
		{
			log({ /*"Missing parameter \"type\""*/ STR(1867) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"from"*/ STR(819))))
		{
			log({ /*"Missing array \"from\""*/ STR(1868) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"to"*/ STR(820))))
		{
			log({ /*"Missing array \"to\""*/ STR(1869) });
			return null_;
		}
		if (!ds_list_valid(DsMap(map).Value(/*"uv"*/ STR(828))))
		{
			log({ /*"Missing array \"uv\""*/ STR(1870) });
			return null_;
		}
		withOne (obj_model_shape, (new obj_model_shape)->id, self->id)
		{
			self->type = DsMap(map).Value(/*"type"*/ STR(780));
			self->description = value_get_string(DsMap(map).Value(/*"description"*/ STR(1829)), /*""*/ STR(0));
			self->use_model_color = value_get_real(DsMap(map).Value(/*"use_model_color"*/ STR(1871)), false);
			...
			self->texture_mirror = value_get_real(DsMap(map).Value(/*"texture_mirror"*/ STR(1872)), false);
			self->invert = value_get_real(DsMap(map).Value(/*"invert"*/ STR(1865)), false);
			self->hide_front = value_get_real(DsMap(map).Value(/*"hide_front"*/ STR(1873)), false);
			self->hide_back = value_get_real(DsMap(map).Value(/*"hide_back"*/ STR(1874)), false);
			if (is_bool(DsMap(map).Value(/*"hide_backface"*/ STR(1875))))
				self->hide_back = DsMap(map).Value(/*"hide_backface"*/ STR(1875));
			self->face_camera = value_get_real(DsMap(map).Value(/*"face_camera"*/ STR(1876)), false);
			self->item_bounce = value_get_real(DsMap(map).Value(/*"item_bounce"*/ STR(1877)), false);
			self->move_required_array = value_get_point3D(DsMap(map).Value(/*"move_required"*/ STR(1878)), vec3(-IntType(1), -IntType(1), -IntType(1)));
			...
			self->floor_box_uvs = ObjType(obj_model_part, self.otherId)->floor_box_uvs;
			self->from_noscale = value_get_point3D(DsMap(map).Value(/*"from"*/ STR(819)));
			self->to_noscale = value_get_point3D(DsMap(map).Value(/*"to"*/ STR(820)));
			if (self->type == /*"plane"*/ STR(1879))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->inflate = vec3(value_get_real(DsMap(map).Value(/*"inflate"*/ STR(1880)), IntType(0)));
			if (self->type == /*"plane"*/ STR(1879))
				self->to_noscale[Y_] = self->from_noscale.Value(Y_);
			self->is3d = false;
			if (self->type == /*"plane"*/ STR(1879))
			{
				self->is3d = value_get_real(DsMap(map).Value(/*"3d"*/ STR(1881)), false);
				if (self->is3d > 0)
				{
			...
			self->position_noscale = value_get_point3D(DsMap(map).Value(/*"position"*/ STR(44)), point3D(IntType(0), IntType(0), IntType(0)));
			...
			self->bend_shape = value_get_real(DsMap(map).Value(/*"bend"*/ STR(1849)), true);
			...
			self->uv = value_get_point2D(DsMap(map).Value(/*"uv"*/ STR(828)));
			...
			{
				if (is_string(DsMap(windmap).Value(/*"axis"*/ STR(807))))
				{
					if (DsMap(windmap).Value(/*"axis"*/ STR(807)) == /*"y"*/ STR(808))
						self->wind_wave = e_vertex_wave_Z_ONLY;
					else
						self->wind_wave = e_vertex_wave_ALL;
					
				}
				if (is_real(DsMap(windmap).Value(/*"ymin"*/ STR(809))))
					self->wind_wave_zmin = DsMap(windmap).Value(/*"ymin"*/ STR(809));
				if (is_real(DsMap(windmap).Value(/*"ymax"*/ STR(1882))))
					self->wind_wave_zmax = DsMap(windmap).Value(/*"ymax"*/ STR(1882));
			}
			if (self->type == /*"block"*/ STR(4))
			{
				self->vbuffer_default = model_shape_generate_block(ScopeAny(self), vec3(IntType(0)));
				if (ObjType(obj_model_part, self.otherId)->name == /*"head"*/ STR(879) && global::player_head_vbuffer == null_)
				...
				else
				{
					...
					log({ /*"Invalid shape type"*/ STR(1883), self->type });
					return null_;
				}
	...
	RealType model_file_load_texture(VarType name, IntType res)
	{
		...
		if (ext != /*".png"*/ STR(10) && ext != /*".jpg"*/ STR(713) && ext != /*".jpeg"*/ STR(714))
			return IntType(0);
		...
		DsMap(ObjType(obj_resource, res)->model_texture_map)[name] = texture_create_square(global::load_folder + /*"/"*/ STR(20) + name);
		return 0.0;
	}
	...
	RealType model_file_load_texture_material(VarType name, IntType res)
	{
		...
		DsMap(ObjType(obj_resource, res)->model_texture_material_map)[name] = texture_create_square(global::load_folder + /*"/"*/ STR(20) + name);
		return 0.0;
	}
	...
	RealType model_file_load_tex_normal(VarType name, IntType res)
	{
		...
		DsMap(ObjType(obj_resource, res)->model_tex_normal_map)[name] = texture_create_square(global::load_folder + /*"/"*/ STR(20) + name);
		return 0.0;
	}
	...
	IntType model_load(VarType map, StringType dir)
	{
		withOne (obj_model, (new obj_model)->id, noone)
		{
			...
			{
				log({ /*"Could not find name for model"*/ STR(1884) });
				return null_;
			}
			
			if (dev_mode_debug_names && !text_exists(/*"model"*/ STR(8) + self->name))
				log({ /*"model/"*/ STR(1828) + self->name + dev_mode_name_translation_message });
			if (is_string(DsMap(map).Value(/*"file"*/ STR(121))))
				self->file = model_file_load(dir + DsMap(map).Value(/*"file"*/ STR(121)));
			else
				self->file = null_;
			...
			if (is_string(DsMap(map).Value(/*"texture"*/ STR(549))))
			{
				...
				DsMap(self->texture_name_map)[/*""*/ STR(0)] = DsMap(map).Value(/*"texture"*/ STR(549));
			}
			else
				if (ds_map_valid(DsMap(map).Value(/*"texture"*/ STR(549))))
				{
					self->texture_name_map = ds_string_map_create();
					ds_map_copy((IntType)(self->texture_name_map), (IntType)(DsMap(map).Value(/*"texture"*/ STR(549))));
				}
				else
					self->texture_name_map = null_;
			
			
			self->version = value_get_real(DsMap(map).Value(/*"version"*/ STR(91)), IntType(0));
			self->pattern_type = value_get_string(DsMap(map).Value(/*"pattern_type"*/ STR(866)), /*""*/ STR(0));
			self->states_map = null_;
			if (ds_map_valid(DsMap(map).Value(/*"states"*/ STR(802))))
			{
				self->states_map = ds_map_create();
				VarType curstate = ds_map_find_first((IntType)(DsMap(map).Value(/*"states"*/ STR(802))));
				while (!is_undefined(curstate))
				{
					if (dev_mode_debug_names && !text_exists(/*"modelstate"*/ STR(750) + curstate))
						log({ /*"model/state/"*/ STR(1885) + curstate + dev_mode_name_translation_message });
					withOne (obj_model_state, (new obj_model_state)->id, self->id)
					{
						self->name = curstate;
						IntType valuelist = ds_map_find_value((IntType)(DsMap(map).Value(/*"states"*/ STR(802))), curstate);
						...
						for (IntType v = IntType(0); v < self->value_amount; v++)
						{
							...
							self->value_name[v] = DsMap(curvalue).Value(/*"value"*/ STR(804));
							...
							if (dev_mode_debug_names && !text_exists(/*"modelstatevalue"*/ STR(9) + self->value_name.Value(v)))
								log({ /*"model/state/value/"*/ STR(1886) + self->value_name.Value(v) + dev_mode_name_translation_message });
							if (!is_undefined(DsMap(curvalue).Value(/*"file"*/ STR(121))))
								self->value_file[v] = model_file_load(dir + DsMap(curvalue).Value(/*"file"*/ STR(121)));
							if (is_string(DsMap(curvalue).Value(/*"texture"*/ STR(549))))
							{
								IntType texnamemap = ds_string_map_create();
								DsMap(texnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"texture"*/ STR(549));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"texture"*/ STR(549))))
								{
									self->value_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(549)), true);
									self->value_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(549)), true);
									self->value_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"texture"*/ STR(549)), true);
								}
							
							if (is_string(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887))))
							{
								IntType shapetexnamemap = ds_string_map_create();
								DsMap(shapetexnamemap)[/*""*/ STR(0)] = DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887));
								...
								if (ds_map_valid(DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887))))
								{
									self->value_shape_texture_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887)), true);
									self->value_shape_texture_material_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_texture_material_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887)), true);
									self->value_shape_tex_normal_name_map[v] = ds_map_create();
									ds_map_merge(self->value_shape_tex_normal_name_map.Value(v), DsMap(curvalue).Value(/*"shape_texture"*/ STR(1887)), true);
								}
							
							if (ds_list_valid(DsMap(curvalue).Value(/*"hide"*/ STR(725))))
							{
								self->value_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"hide"*/ STR(725))));
							}
							if (ds_list_valid(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1888))))
							{
								self->value_shape_hide_list[v] = ds_list_create();
								ds_list_copy((IntType)(self->value_shape_hide_list.Value(v)), (IntType)(DsMap(curvalue).Value(/*"shape_hide"*/ STR(1888))));
							}
							if (ds_map_valid(DsMap(curvalue).Value(/*"shape_color"*/ STR(1889))))
							{
								self->value_color_name_map[v] = ds_map_create();
								ds_map_merge(self->value_color_name_map.Value(v), DsMap(curvalue).Value(/*"shape_color"*/ STR(1889)), true);
							}
							if (!is_undefined(DsMap(curvalue).Value(/*"pattern_type"*/ STR(866))))
								self->value_pattern_type[v] = DsMap(curvalue).Value(/*"pattern_type"*/ STR(866));
						}
						DsMap(ObjType(obj_model, self.otherId)->states_map)[curstate] = self->id;
						curstate = ds_map_find_next((IntType)(DsMap(map).Value(/*"states"*/ STR(802))), curstate);
					}
					
				}
				
			}
			if (is_string(DsMap(map).Value(/*"default_state"*/ STR(806))))
				self->default_state = string_get_state_vars(DsMap(map).Value(/*"default_state"*/ STR(806)));
			else
	...
	RealType model_part_fill_shape_alpha_map(VarType part, IntType alphamap, VarType res, IntType texnamemap, IntType shapetexnamemap)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			...
			{
				if (self->type == /*"plane"*/ STR(1879) && self->is3d > 0)
				{
	...
	RealType model_part_fill_shape_vbuffer_map(VarType part, IntType vbufmap, IntType alphamap, VecType bend)
	{
		...
		for (IntType s = IntType(0); s < ds_list_size(idInt(part, shape_list)); s++)
		{
			withOne (obj_model_shape, DsList(idInt(part, shape_list)).Value(s), noone)
			{
				...
				if (self->type == /*"block"*/ STR(4) && self->bend_shape > 0 && isbent)
	...
	VarType model_shape_generate_block(ScopeAny self, VecType bend)
	{
		...
		VarType bendsize;
		BoolType invangle;
		sharpbend = global::_app->project_bend_style == /*"blocky"*/ STR(1890) && sVar(bend_size) == null_ && ((sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && sVec(bend_axis).Real(Y_) && !sVec(bend_axis).Real(Z_)) || (!sVec(bend_axis).Real(X_) && !sVec(bend_axis).Real(Y_) && sVec(bend_axis).Real(Z_)));
		bendsize = (sVar(bend_size) == null_ ? VarType((global::_app->project_bend_style == /*"realistic"*/ STR(1891) ? 4.0 : 1.0)) : sVar(bend_size));
