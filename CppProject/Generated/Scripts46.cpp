...
namespace CppProject
{
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2436) });
		...
		self->sprite_template = /*"generic"*/ STR(2437);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2438);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->last_opened = value_get_real({ DsMap(projectmap).Value(/*"last_opened"*/ STR(2439)), -IntType(1) });
				ObjType(obj_recent, recentobj)->pinned = value_get_real({ DsMap(projectmap).Value(/*"pinned"*/ STR(2440)), false });
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2441);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2442) });
		...
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"thumbnail"*/ STR(2443), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1220), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2439), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2440), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2444) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2445) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2446) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2447) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2448) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2449) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2450), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2451), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2452), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2453), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
