...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		json_save_var_nullable(/*"color_tag"*/ STR(2389), self->color_tag);
		json_save_var_bool(/*"hide"*/ STR(719), self->hide);
		json_save_var_bool(/*"lock"*/ STR(2390), self->lock);
		json_save_var_bool(/*"ghost"*/ STR(2391), self->ghost);
		json_save_var(/*"depth"*/ STR(611), self->depth);
		if (self->type == e_temp_type_BODYPART)
			json_save_var(/*"model_part_name"*/ STR(2392), json_string_encode(self->model_part_name));
		...
		if (self->part_of != null_)
		{
			...
			json_save_var_save_id(/*"part_of"*/ STR(2393), self->part_of);
			if (self->part_root != null_)
				json_save_var_save_id(/*"part_root"*/ STR(2394), self->part_root);
		...
		project_save_values(/*"default_values"*/ STR(2396), self->value_default, global::_app->value_default);
		...
		json_save_var(/*"parent_tree_index"*/ STR(2397), ds_list_find_index(idInt(self->parent, tree_list), self->id));
		if (self->value_type.Value(e_value_type_HIERARCHY))
		{
			json_save_var_bool(/*"lock_bend"*/ STR(1790), self->lock_bend);
			json_save_var_bool(/*"tree_extend"*/ STR(2398), self->tree_extend);
			json_save_object_start({ /*"inherit"*/ STR(2399) });
			...
			json_save_var_bool(/*"alpha"*/ STR(2248), self->inherit_alpha);
			...
			json_save_var_bool(/*"visibility"*/ STR(2401), self->inherit_visibility);
			json_save_var_bool(/*"bend"*/ STR(1792), self->inherit_bend);
			json_save_var_bool(/*"rot_point"*/ STR(2400), self->inherit_rot_point);
			json_save_var_bool(/*"glow_color"*/ STR(2402), self->inherit_glow_color);
			json_save_var_bool(/*"select"*/ STR(2403), self->inherit_select);
			json_save_object_done();
			json_save_var_bool(/*"scale_resize"*/ STR(2404), self->scale_resize);
		}
		if (self->value_type.Value(e_value_type_ROT_POINT))
		{
			json_save_var_bool(/*"rot_point_custom"*/ STR(2405), self->rot_point_custom);
			json_save_var_point3D(/*"rot_point"*/ STR(2400), self->rot_point);
		}
		if (self->value_type.Value(e_value_type_APPEARANCE))
		{
			json_save_var_bool(/*"backfaces"*/ STR(1791), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2406), self->texture_blur);
			json_save_var_bool(/*"texture_filtering"*/ STR(2335), self->texture_filtering);
		...
		if (self->value_type.Value(e_value_type_PATH))
		{
			...
			json_save_var_bool(/*"closed"*/ STR(2387), self->path_closed);
			json_save_var(/*"detail"*/ STR(2388), self->path_detail);
	...
	RealType recent_load(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->author = value_get_string(DsMap(projectmap).Value(/*"author"*/ STR(2272)), /*""*/ STR(0));
	...
	void recent_save()
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"author"*/ STR(2272), json_string_encode(idVar(recent, author)));
