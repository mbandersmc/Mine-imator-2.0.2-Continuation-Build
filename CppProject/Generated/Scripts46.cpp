...
namespace CppProject
{
	void project_save_timeline(Scope<obj_timeline> self)
	{
		...
		{
			json_save_var_bool(/*"backfaces"*/ STR(1790), self->backfaces);
			json_save_var_bool(/*"texture_blur"*/ STR(2405), self->texture_blur);
			...
			json_save_var_bool(/*"glow_texture"*/ STR(2406), self->glow_texture);
			json_save_var_bool(/*"only_render_glow"*/ STR(2407), self->only_render_glow);
			...
			if (self->type == e_temp_type_SCENERY || self->type == e_temp_type_BLOCK || self->type == e_temp_type_PARTICLE_SPAWNER || self->type == e_temp_type_TEXT || type_is_shape(self->type))
			{
				...
				json_save_var_bool(/*"wind_terrain"*/ STR(2408), self->wind_terrain);
			}
			json_save_var_bool(/*"hq_hiding"*/ STR(2409), self->hq_hiding);
			json_save_var_bool(/*"lq_hiding"*/ STR(2410), self->lq_hiding);
			json_save_var(/*"blend_mode"*/ STR(2411), self->blend_mode);
			json_save_var(/*"alpha_mode"*/ STR(2412), self->alpha_mode);
		}
		if (self->value_type.Value(e_value_type_PATH))
		{
			json_save_object_start({ /*"path"*/ STR(520) });
			json_save_var_bool(/*"smooth"*/ STR(2413), self->path_smooth);
			json_save_var_bool(/*"closed"*/ STR(2386), self->path_closed);
			json_save_var(/*"detail"*/ STR(2387), self->path_detail);
			json_save_var_bool(/*"shape_generate"*/ STR(2414), self->path_shape_generate);
			json_save_var(/*"shape_radius"*/ STR(2415), self->path_shape_radius);
			json_save_var(/*"shape_tex_length"*/ STR(2416), self->path_shape_tex_length);
			json_save_var_bool(/*"shape_invert"*/ STR(2417), self->path_shape_invert);
			json_save_var_bool(/*"shape_tube"*/ STR(2418), self->path_shape_tube);
			json_save_var(/*"shape_detail"*/ STR(2419), self->path_shape_detail);
			json_save_var_bool(/*"shape_smooth_segments"*/ STR(2420), self->path_shape_smooth_segments);
			json_save_var_bool(/*"shape_smooth_ring"*/ STR(2421), self->path_shape_smooth_ring);
			json_save_object_done();
		}
		json_save_object_done();
	}
	...
	void ptype_event_create(Scope<obj_particle_type> self)
	{
		...
		self->text = text_get({ /*"particleeditortypetextsample"*/ STR(2437) });
		...
		self->sprite_template = /*"generic"*/ STR(2438);
	...
	void recent_add(ScopeAny self)
	{
		...
		VarType surf;
		thumbnailfn = sStr(project_folder) + /*"/thumbnail.png"*/ STR(2439);
		surf = null_;
	...
	RealType recent_load(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(recentlist)); i++)
		{
			...
			if (ds_map_valid(projectmap))
			{
				...
				ObjType(obj_recent, recentobj)->last_opened = value_get_real(DsMap(projectmap).Value(/*"last_opened"*/ STR(2440)), -IntType(1));
				ObjType(obj_recent, recentobj)->pinned = value_get_real(DsMap(projectmap).Value(/*"pinned"*/ STR(2441)), false);
				StringType thumbnailfn = filename_path(ObjType(obj_recent, recentobj)->filename) + /*"thumbnail.png"*/ STR(2442);
	...
	void recent_save()
	{
		log({ /*"Updating recent list"*/ STR(2443) });
		...
		for (IntType i = IntType(0); i < ds_list_size(global::_app->recent_list); i++)
		{
			...
			json_save_var(/*"thumbnail"*/ STR(2444), idVar(recent, thumbnail));
			json_save_var(/*"filename"*/ STR(1220), json_string_encode(idVar(recent, filename)));
			json_save_var(/*"last_opened"*/ STR(2440), idVar(recent, last_opened));
			json_save_var_bool(/*"pinned"*/ STR(2441), idVar(recent, pinned));
			json_save_object_done();
		}
		json_save_array_done();
		json_save_object_done();
		json_save_done();
		debug({ /*"Saved recent list"*/ STR(2445) });
	}
	...
	VarType recent_time_string(RealType time)
	{
		...
		if (time < IntType(1))
			return text_get({ /*"recentlastopenednever"*/ STR(2446) });
		...
		if (currenttime < time)
			return text_get({ /*"recentlastopenedfuture"*/ STR(2447) });
		if ((date_get_week(currenttime) != date_get_week(time)) && weeks < IntType(2))
			return text_get({ /*"recentlastopenedlastweek"*/ STR(2448) });
		if ((date_get_day(currenttime) != date_get_day(time)) && hours < IntType(24))
			return text_get({ /*"recentlastopenedyesterday"*/ STR(2449) });
		if (minutes < IntType(1))
			return text_get({ /*"recentlastopenedrecently"*/ STR(2450) });
		if (minutes < IntType(60))
			return text_get({ /*"recentlastopenedminutes"*/ STR(2451), floor(minutes) });
		if (hours < IntType(24))
			return text_get({ /*"recentlastopenedhours"*/ STR(2452), floor(hours) });
		if (days < IntType(7))
			return text_get({ /*"recentlastopeneddays"*/ STR(2453), floor(days) });
		return text_get({ /*"recentlastopeneddate"*/ STR(2454), date_get_day(time), date_get_month(time), date_get_year(time) });
	}
