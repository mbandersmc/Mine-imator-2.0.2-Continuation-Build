...
namespace CppProject
{
		...
		switch ((IntType)vid)
		{
			...
			case e_value_TRANSITION:
			{
				switch ((IntType)val)
				{
					case IntType(0):
					{
						return /*"linear"*/ STR(535);
					}
					case IntType(1):
					{
						return /*"easeinquad"*/ STR(538);
					}
					case IntType(2):
					{
						return /*"easeoutquad"*/ STR(539);
					}
					case IntType(3):
					{
						return /*"easeinoutquad"*/ STR(540);
					}
					case IntType(4):
					{
						return /*"instant"*/ STR(536);
					}
				}
	...
	void project_load_legacy_particles(ScopeAny self)
	{
		...
		ArrType boxtype = ArrType::From({ /*"none"*/ STR(876), /*"ground"*/ STR(135), /*"spawn"*/ STR(1319), /*"custom"*/ STR(584) });
	...
	void project_load_legacy_resource()
	{
		withOne (obj_resource, (new obj_resource)->id, noone)
		{
			...
			if (typename_ == /*"item"*/ STR(507))
				typename_ = /*"itemsheet"*/ STR(527);
			if (typename_ == /*"block"*/ STR(508) || typename_ == /*"blocksheet"*/ STR(529))
				typename_ = /*"legacyblocksheet"*/ STR(528);
			if (typename_ == /*"particles"*/ STR(510))
				typename_ = /*"particlesheet"*/ STR(531);
			if (typename_ == /*"schematic"*/ STR(2132))
				typename_ = /*"scenery"*/ STR(506);
	...
	BoolType project_load_legacy_start(VarType argument0)
	{
		...
		if (global::load_format > e_project_FORMAT_CB_103)
		{
			...
			{
				log({ /*"Too old legacy project, format"*/ STR(2135), global::load_format });
				error(/*"errorfilecorrupted"*/ STR(2134));
				buffer_delete(global::buffer_current);
				return false;
			}
	...
	void project_load_legacy_template()
	{
		withOne (obj_template, (new obj_template)->id, noone)
		{
			...
			if (self->type == e_temp_type_CHARACTER || self->type == e_temp_type_SPECIAL_BLOCK || self->type == e_temp_type_BODYPART)
			{
				...
				{
					self->model_name = DsMap(modelmap).Value(/*"name"*/ STR(787));
					if (!is_undefined(DsMap(modelmap).Value(/*"state"*/ STR(981))))
						self->model_state = string_get_state_vars(DsMap(modelmap).Value(/*"state"*/ STR(981)));
					else
						self->model_state = ArrType();
					
				}
				else
					log({ /*"Could not convert model "*/ STR(2136), self->legacy_model_name });
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (ds_list_valid(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
					...
					log({ /*"Could not convert model part of "*/ STR(2137), self->model_name, self->legacy_bodypart_id });
			...
			if (self->text_font == /*"root"*/ STR(2121))
		...
		withOne (obj_timeline, (new obj_timeline)->id, noone)
		{
			...
			if (self->type == e_temp_type_BODYPART)
			{
				...
				if (!is_undefined(modelpartlist) && self->legacy_bodypart_id < ds_list_size((IntType)(modelpartlist)))
				{
					...
					log({ /*"Could not find model part for"*/ STR(2138), ObjType(obj_template, findtemp)->model_name, ObjType(obj_template, findtemp)->legacy_model_name, self->legacy_bodypart_id });
	...
	void project_load_legacy_values(ScopeAny self, IntType tl)
	{
		...
		if (idArr(tl, value_type).Value(e_value_type_PARTICLES))
		{
			...
			if (sVar(value).Value(e_value_ATTRACTOR) == /*"root"*/ STR(2121))
		...
		if (idArr(tl, value_type).Value(e_value_type_MATERIAL_TEXTURE))
		{
			...
			if (sVar(value).Value(e_value_TEXTURE_OBJ) == /*"root"*/ STR(2121))
	...
	RealType project_load_markers(VarType list)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size((IntType)(list)); i++)
		{
			...
			withOne (obj_marker, (new obj_marker)->id, noone)
			{
				...
				self->name = value_get_string(DsMap(markermap).Value(/*"name"*/ STR(787)), /*""*/ STR(0));
				self->color = value_get_real(DsMap(markermap).Value(/*"color"*/ STR(888)), IntType(0));
			}
	...
	{
		VarType templist = DsMap(map).Value(/*"templates"*/ STR(2130));
		...
		VarType tllist = DsMap(map).Value(/*"timelines"*/ STR(2131));
