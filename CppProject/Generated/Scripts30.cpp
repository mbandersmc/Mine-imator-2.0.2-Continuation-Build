...
namespace CppProject
{
	...
	RealType draw_wheel(ScopeAny self, StringType name, VarType xx, VarType yy, VarType color, VarType value, RealType minval, RealType maxval, VarType def, RealType snapval, VarType tbx, IntType script, RealType rad, VarType sprite)
	{
		...
		StringType text;
		VarType labely;
		context_menu_area(self, { xx - rad, yy - rad, rad * IntType(2), rad * IntType(2), /*"contextmenuvalue"*/ STR(1076), value, e_context_type_NUMBER, script, def });
		draw_set_font(sInt(font_label));
		modval = mod_fix(value, IntType(360));
		capwid = string_width(text_get({ name }) + /*":"*/ STR(763)) + IntType(5);
	...
	RealType draw_wheel_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx - (rad + IntType(10)), yy - (rad + IntType(10)), (rad + IntType(10)) * IntType(2), (rad + IntType(10)) * IntType(2), /*"contextmenuvalue"*/ STR(1076), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
	...
	VarType ease(VarType func, VarType xx)
	{
		...
		switch (StringType(func).id)
		{
			...
			case 556: // easeoutquad
			{
				return -xx * (xx - IntType(2));
			}
			case 557: // easeinoutquad
			{
				if (xx2 < IntType(1))
					return 0.5 * xx2 * xx2;
				return -0.5 * ((xx2 - IntType(1)) * (xx2 - IntType(3)) - IntType(1));
			}
			case 558: // easeincubic
			{
				return xx * xx * xx;
			}
			case 559: // easeoutcubic
			{
				return ((xxm1) * xxm1 * xxm1 + IntType(1));
			}
			case 560: // easeinoutcubic
			{
				if (xx2 < IntType(1))
					return 0.5 * xx2 * xx2 * xx2;
				return 0.5 * ((xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) + IntType(2));
			}
			case 561: // easeinquart
			{
				return xx * xx * xx * xx;
			}
			case 562: // easeoutquart
			{
				return -((xxm1) * xxm1 * xxm1 * xxm1 - IntType(1));
			}
			case 563: // easeinoutquart
			{
				if (xx2 < IntType(1))
					return 0.5 * xx2 * xx2 * xx2 * xx2;
				return -0.5 * ((xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) - IntType(2));
			}
			case 564: // easeinquint
			{
				return xx * xx * xx * xx * xx;
			}
			case 565: // easeoutquint
			{
				return ((xxm1) * xxm1 * xxm1 * xxm1 * xxm1 + IntType(1));
			}
			case 566: // easeinoutquint
			{
				if (xx2 < IntType(1))
					return 0.5 * xx2 * xx2 * xx2 * xx2 * xx2;
				return 0.5 * ((xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) * (xx2 - IntType(2)) + IntType(2));
			}
			case 567: // easeinsine
			{
				return -cos(xx * (pi_ / 2.0)) + IntType(1);
			}
			case 568: // easeoutsine
			{
				return sin(xx * (pi_ / 2.0));
			}
			case 569: // easeinoutsine
			{
				return -0.5 * (cos(pi_ * (RealType)xx / 1.0) - IntType(1));
			}
			case 570: // easeinexpo
			{
				return power(2.0, IntType(10) * (xx - IntType(1)));
			}
			case 571: // easeoutexpo
			{
				return -power(2.0, -IntType(10) * xx) + IntType(1);
			}
			case 572: // easeinoutexpo
			{
				if (xx2 < IntType(1))
					return 0.5 * power(2.0, IntType(10) * (xx2 - IntType(1)));
				return 0.5 * (-power(2.0, -IntType(10) * (xx2 - IntType(1))) + IntType(2));
			}
			case 573: // easeincirc
			{
				return -(sqrt(IntType(1) - xx * xx) - IntType(1));
			}
			case 574: // easeoutcirc
			{
				return sqrt(IntType(1) - (xxm1) * xxm1);
			}
			case 575: // easeinoutcirc
			{
				if (xx2 < IntType(1))
					return -0.5 * (sqrt(IntType(1) - xx2 * xx2) - IntType(1));
				return 0.5 * (sqrt(max({ IntType(0), IntType(1) - (xx2 - IntType(2)) * (xx2 - IntType(2)) })) + IntType(1));
			}
			case 576: // easeinelastic
			{
				RealType p = 0.3;
			...
			case 580: // easeoutback
			{
				RealType s = 1.70158;
				return (xxm1 * xxm1 * ((s + IntType(1)) * xxm1 + s) + IntType(1));
			}
			case 581: // easeinoutback
			{
				RealType s = 1.70158;
				if (xx2 < IntType(1))
					return 0.5 * (xx2 * xx2 * (((s * (1.525)) + IntType(1)) * xx2 - (s * (1.525))));
				return 0.5 * ((xx2 - IntType(2)) * (xx2 - IntType(2)) * (((s * (1.525)) + IntType(1)) * (xx2 - IntType(2)) + (s * (1.525))) + IntType(2));
			}
			case 582: // easeinbounce
			{
				xx = IntType(1) - xx;
				if (xx < (1.0 / 2.75))
	...
	VarType eval(VarType str, VarType def)
	{
		...
		for (i = IntType(1); i < string_length(str) + IntType(1); i++)
		{
			...
			if (char_ == /*" "*/ STR(21))
				...
				if (char_ == /*"("*/ STR(1103))
				{
					if (lastoperator == /*"--"*/ STR(1104))
						ds_stack_push({ ops, /*"-("*/ STR(1105) });
					else
						ds_stack_push({ ops, /*"("*/ STR(1103) });
					...
					if (eval_is_digit(char_))
					{
						...
						{
							if (string_char_at(str, i) == /*"."*/ STR(1106))
							{
								deccheck = true;
								i++;
								continue;
						...
						if (char_ == /*")"*/ STR(1107))
						{
							while (!ds_stack_empty(ops) && !(ds_stack_top(ops) == /*"("*/ STR(1103) || ds_stack_top(ops) == /*"-("*/ STR(1105)))
							{
						...
						{
							if (lastoperator != /*""*/ STR(0) && char_ == /*"-"*/ STR(1108))
							{
								lastoperator = /*"--"*/ STR(1104);
								continue;
							}
	...
	{
		if (a == VarType() && b != VarType() && op == /*"-"*/ STR(1108))
			a = IntType(0);
	...
	{
		ArrType digits = ArrType::From({ /*"0"*/ STR(1069), /*"1"*/ STR(1112), /*"2"*/ STR(1113), /*"3"*/ STR(1114), /*"4"*/ STR(1115), /*"5"*/ STR(1116), /*"6"*/ STR(1117), /*"7"*/ STR(1118), /*"8"*/ STR(1119), /*"9"*/ STR(1120), /*"."*/ STR(1106) });
	...
	{
		log({ /*"execute"*/ STR(1121), file, parameters, wait });
	...
	void export_done_image(ScopeAny self)
	{
		...
		toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportimage"*/ STR(1122) }));
		toast_add_action(self, /*"alertexportimageview"*/ STR(1123), ID_popup_open_url, sStr(export_filename));
	}
	...
	void export_done_movie(ScopeAny self, BoolType cancel)
	{
		...
		if (sStr(exportmovie_format) == /*"png"*/ STR(64))
			fn = filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_1.png"*/ STR(1124);
		else
			fn = sStr(export_filename);
		
		if (!cancel)
		{
			toast_new(self, e_toast_POSITIVE, text_get({ /*"alertexportmovie"*/ STR(1125) }));
			toast_add_action(self, /*"alertexportmovieview"*/ STR(1126), ID_popup_open_url, fn);
		}
	}
	...
	RealType export_update(ScopeAny self)
	{
		...
		if (sInt(export_escape_time) > IntType(0) && current_time() - sInt(export_escape_time) > IntType(1000))
		{
			...
			if (question(text_get({ /*"questionstoprender"*/ STR(1127) })))
			{
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
		{
			global::render_active = /*"movie"*/ STR(1128);
		...
		if (sStr(window_state) == /*"export_movie"*/ STR(53))
		{
			if (sStr(exportmovie_format) == /*"png"*/ STR(64))
			{
				...
				numstr = string_repeat(/*"0"*/ STR(1069), (IntType)((totallen - string_length(numstr)))) + numstr;
				surface_save_lib(sVar(export_surface), filename_new_ext(sStr(export_filename), /*""*/ STR(0)) + /*"_"*/ STR(1129) + numstr + /*".png"*/ STR(10));
			}
			else
			{
				if (!is_cpp())
				...
				{
					export_done_movie(self);
					log({ /*"Error when adding frame, error code"*/ STR(1130), err });
	...
	StringType filename_get_unique(StringType fn)
	{
		...
		while (file_exists_lib(newfn))
			newfn = path + noext + /*" ("*/ STR(1131) + string(num++) + /*")"*/ STR(1107) + ext;
	...
	{
		fn = string_replace_all(fn, /*"/"*/ STR(20), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"\\"*/ STR(1052), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*":"*/ STR(763), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"*"*/ STR(1110), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"?"*/ STR(1132), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"\""*/ STR(1133), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"<"*/ STR(1134), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*">"*/ STR(1135), /*"_"*/ STR(1129));
		fn = string_replace_all(fn, /*"|"*/ STR(859), /*"_"*/ STR(1129));
	...
	VarType filename_new_ext(VarType fn, StringType newext)
	{
		...
		for (p = string_length(fn); p >= IntType(0); p--)
		{
			...
			if (p == IntType(0) || c == /*"\\"*/ STR(1052) || c == /*"/"*/ STR(20))
				return fn + newext;
			if (c == /*"."*/ STR(1106))
				break;
		}
	...
	{
		return file_dialog_open(text_get({ /*"filedialogopenasset"*/ STR(1136) }) + /*"|"*/ STR(859) + asset_exts, /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenassetcaption"*/ STR(1137) }));
	}
	
	StringType file_dialog_open_font()
	{
		return file_dialog_open(text_get({ /*"filedialogopenfont"*/ STR(1138) }) + /*" (*.ttf)|*.ttf"*/ STR(1139), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenfontcaption"*/ STR(1140) }));
	}
	
	StringType file_dialog_open_image()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimage"*/ STR(1141) }) + /*" (*.png; *.jpg; *.dat)|*.png;*.jpg;*.jpeg;*.dat;"*/ STR(1142), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimagecaption"*/ STR(1143) }));
	}
	
	StringType file_dialog_open_image_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenimageorpack"*/ STR(1144) }) + /*" (*.png; *.jpg; *.zip)|*.png;*.jpg;*.jpeg;*.zip"*/ STR(1145), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenimageorpackcaption"*/ STR(1146) }));
	}
	
	StringType file_dialog_open_language()
	{
		return file_dialog_open(text_get({ /*"filedialogopenlanguage"*/ STR(1147) }) + /*" (*.milanguage;*.txt)|*.milanguage;*.txt"*/ STR(1148), /*""*/ STR(0), languages_directory, text_get({ /*"filedialogopenlanguagecaption"*/ STR(1149) }));
	}
	
	StringType file_dialog_open_model()
	{
		return file_dialog_open(text_get({ /*"filedialogopenmodel"*/ STR(1150) }) + /*" (*.mimodel;*.json;*.zip)|*.mimodel;*.json;*.zip"*/ STR(1151), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenmodelcaption"*/ STR(1152) }));
	}
	
	StringType file_dialog_open_pack()
	{
		return file_dialog_open(text_get({ /*"filedialogopenpack"*/ STR(1153) }) + /*" (*.zip)|*.zip"*/ STR(1154), /*""*/ STR(0), /*""*/ STR(0), text_get({ /*"filedialogopenpackcaption"*/ STR(1155) }));
	}
	
	StringType file_dialog_open_particles()
	{
		return file_dialog_open(text_get({ /*"filedialogopenparticles"*/ STR(1156) }) + /*" (*.miparticles; *.zip; *.particles)|*miparticles;*.zip;*.particles;"*/ STR(1157), /*""*/ STR(0), particles_directory, text_get({ /*"filedialogopenparticlecaption"*/ STR(1158) }));
	}
	
	VarType file_dialog_open_project(ScopeAny self)
	{
		return file_dialog_open(text_get({ /*"filedialogopenproject"*/ STR(1159) }) + /*" (*.miproject; *.zip; *.mproj; *.mani)|*miproject;*.zip;*.mproj;*.mani|"*/ STR(1160) + text_get({ /*"filedialogopenbackup"*/ STR(1161) }) + /*" (*.backup*; *.mbackup*)|*.backup*;*.mbackup*"*/ STR(1162), /*""*/ STR(0), sStr(setting_project_folder), text_get({ /*"filedialogopenprojectcaption"*/ STR(1163) }));
	}
