...
{
	VarType render_generate_noise(VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType w, h, surf, normal;
		w = argument[IntType(0)];
		h = argument[IntType(1)];
		surf = null_;
		normal = false;
		if (argument_count > IntType(2))
			surf = argument[IntType(2)];
		if (argument_count > IntType(3))
			normal = argument[IntType(3)];
	...
	RealType render_generate_text(ScopeAny self, VarArgs argument)
	{
		...
		halign = /*"center"*/ STR(1408);
		valign = /*"center"*/ STR(1408);
		aa = true;
		alignmatch = true;
		aamatch = true;
		if (argument_count > IntType(3))
	...
	VarType render_high_glow(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType prevsurf, glowfalloff, glowcolorsurf, glowsurf, resultsurf;
		prevsurf = argument[IntType(0)];
		glowfalloff = false;
		if (argument_count > IntType(1))
			glowfalloff = argument[IntType(1)];
		global::render_surface[IntType(0)] = surface_require(global::render_surface.Value(IntType(0)), global::render_width, global::render_height);
		global::render_surface[IntType(1)] = surface_require(global::render_surface.Value(IntType(1)), global::render_width, global::render_height);
