...
namespace CppProject
{
	void debug_ds_list(VarType argument0)
	{
		...
		debug({ /*"Elements"*/ STR(1033), ds_list_size((IntType)(list)) });
		for (IntType i = IntType(0); i < ds_list_size(list); i++)
			debug({ /*"    "*/ STR(1034) + string(DsList(list).Value(i)) });
	}
	...
	void debug_ds_map(IntType map)
	{
		...
		debug({ /*"Elements"*/ STR(1033), ds_list_size(keylist) });
		for (IntType i = IntType(0); i < ds_list_size(keylist); i++)
			debug({ /*"   "*/ STR(1035) + string(DsList(keylist).Value(i)) + /*" => "*/ STR(1036) + string(DsMap(map).Value(DsList(keylist).Value(i))) });
	...
	RealType debug_info_draw(ScopeAny self)
	{
		...
		str += /*"Performance: \n"*/ STR(1037);
		str += /*"======================================= \n"*/ STR(1038);
		str += /*"FPS: "*/ STR(1039) + string(gmlGlobal::fps) + /*" \n"*/ STR(1040);
		str += /*"FPS real: "*/ STR(1041) + string(gmlGlobal::fps_real) + /*" \n"*/ STR(1040);
		str += /*"delta: "*/ STR(1042) + string(global::delta) + /*" \n"*/ STR(1040);
		str += /*"\n"*/ STR(948);
		str += /*"Window: \n"*/ STR(1043);
		str += /*"======================================= \n"*/ STR(1038);
		str += /*"DPI: "*/ STR(1044) + string(display_get_dpi_x()) + /*","*/ STR(1045) + string(display_get_dpi_y()) + /*" \n"*/ STR(1040);
		str += /*"Size: "*/ STR(1046) + string(sReal(window_width)) + /*","*/ STR(1045) + string(sReal(window_height)) + /*" \n"*/ STR(1040);
		str += /*"window_busy: "*/ STR(1047) + string(sVar(window_busy)) + /*" \n"*/ STR(1040);
		str += /*"window_focus: "*/ STR(1048) + string(sVar(window_focus)) + /*" \n"*/ STR(1040);
		str += /*"current_step: "*/ STR(1049) + string(global::current_step) + /*" \n"*/ STR(1040);
		str += /*"\n"*/ STR(948);
		str += /*"Project: \n"*/ STR(1050);
		str += /*"======================================= \n"*/ STR(1038);
		str += /*"project_file: "*/ STR(1051) + string_replace_all(sVar(project_file), /*"/"*/ STR(20), /*"\\"*/ STR(1052)) + /*" \n"*/ STR(1040);
		str += /*"project_folder: "*/ STR(1053) + string_replace_all(sStr(project_folder), /*"/"*/ STR(20), /*"\\"*/ STR(1052)) + /*" \n"*/ STR(1040);
		str += /*"working_directory: "*/ STR(1054) + string_replace_all(gmlGlobal::working_directory, /*"/"*/ STR(20), /*"\\"*/ STR(1052)) + /*" \n"*/ STR(1040);
		str += /*"file_directory: "*/ STR(1055) + string_replace_all(file_directory, /*"/"*/ STR(20), /*"\\"*/ STR(1052)) + /*" \n"*/ STR(1040);
		str += /*"\n"*/ STR(948);
		str += /*"instance_count: "*/ STR(1056) + string(gmlGlobal::instance_count) + /*" \n"*/ STR(1040);
		str += /*"render_world() calls: "*/ STR(1057) + string(global::render_world_count) + /*" \n"*/ STR(1040);
		str += /*"Vertex buffer triangles: "*/ STR(1058) + string(get_vertex_buffer_triangles()) + /*" \n"*/ STR(1040);
		str += /*"Vertex buffer render calls: "*/ STR(1059) + string(get_vertex_buffer_render_calls()) + /*" \n"*/ STR(1040);
		str += /*"Primitive lines: "*/ STR(1060) + string(get_primitive_lines()) + /*" \n"*/ STR(1040);
		str += /*"Primitive triangles: "*/ STR(1061) + string(get_primitive_triangles()) + /*" \n"*/ STR(1040);
		str += /*"Primitive render calls: "*/ STR(1062) + string(get_primitive_render_calls()) + /*" \n"*/ STR(1040);
		str += /*"\n"*/ STR(948);
		if (dev_mode)
		{
			str += /*"[F7]: Reload Minecraft assets"*/ STR(1063) + /*" \n"*/ STR(1040);
			str += /*"[F9]: Open file directory"*/ STR(1064) + /*" \n"*/ STR(1040);
			str += /*"[F10]: Open working directory"*/ STR(1065) + /*" \n"*/ STR(1040);
			str += /*"[F11]: Open log file"*/ STR(1066) + /*" \n"*/ STR(1040);
		}
		str += /*"[F12 to disable]"*/ STR(1067);
		IntType w = string_width_font(str, sInt(font_label)) + IntType(16);
		IntType h = string_height_font(str, sInt(font_label)) + IntType(16);
		draw_box(IntType(8), sReal(window_height) - h - IntType(8), w, h, false, c_black, .75);
		draw_label(self, { str, IntType(16), sReal(window_height) - IntType(16), fa_left, fa_bottom, c_white, IntType(1), sInt(font_label) });
		return 0.0;
	}
	...
	{
		debug({ str, string(current_time() - global::debug_timer) + /*" msec"*/ STR(1068) });
	}
	...
	{
		return filename_name(filename_dir(dir + /*".ext"*/ STR(1070))) + /*"/"*/ STR(20);
	}
	...
	StringType dir_get_string(IntType dir)
	{
		switch ((IntType)dir)
		{
			...
			case e_dir_SOUTH:
			{
				return /*"south"*/ STR(876);
			}
			case e_dir_NORTH:
			{
				return /*"north"*/ STR(877);
			}
			case e_dir_UP:
			{
				return /*"up"*/ STR(893);
	...
	void draw_bezier_graph(ScopeAny self, VarType xx, VarType yy, RealType wid, RealType hei, ArrType points, BoolType sync)
	{
		...
		if (app_mouse_box(self, easeinxpos - IntType(5), easeinypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereasein"*/ STR(1071);
		...
		if (app_mouse_box(self, easeoutxpos - IntType(5), easeoutypos - IntType(5), IntType(10), IntType(10)))
		{
			...
			if (sBool(mouse_left))
			{
				sVar(window_busy) = /*"beziereaseout"*/ STR(1072);
				sVar(handle_drag_offset_x) = gmlGlobal::mouse_x - easeoutxpos;
				sVar(handle_drag_offset_y) = gmlGlobal::mouse_y - easeoutypos;
			}
		}
		...
		if (sVar(window_busy) == /*"beziereasein"*/ STR(1071))
		{
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (sVar(window_busy) == /*"beziereaseout"*/ STR(1072))
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
	...
	RealType draw_button_accent(ScopeAny self, VarType xx, VarType yy, RealType width, RealType height, IntType index)
	{
		...
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		microani_set(/*"accentlistitem"*/ STR(1073) + string(index), null_, mouseon, mouseclick, sInt(setting_accent) == index, 0.5);
		...
		else
		{
			...
			tip_set(self, text_get({ /*"tooltipcustomaccentcolor"*/ STR(1074) }), xx, yy, width, height);
		}
	...
	RealType draw_button_collapse(ScopeAny self, StringType name, VarType open, IntType script, VarType active, StringType caption, StringType tip)
	{
		...
		if (xx + wid < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || sVar(dy) + IntType(24) < sVar(content_y) || sVar(dy) > sVar(content_y) + sVar(content_height))
			return IntType(0);
		draw_button_icon(self, { name + /*"collapse"*/ STR(1075), xx, yy, IntType(20), IntType(20), open > 0 && active > 0, null_, null_, !(active > 0), /*""*/ STR(0), ID_spr_chevron_ani });
	...
	BoolType draw_button_color(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, VarType color, VarType def, BoolType hsvmode, IntType script)
	{
		...
		active = (sStr(settings_menu_name) == /*"colorpicker"*/ STR(683) && ObjType(obj_colorpicker, sInt(colorpicker))->value_name == name);
		mouseon = app_mouse_box(self, buttonx, yy, buttonw, h) && sBool(content_mouseon);
		mouseclick = mouseon && sBool(mouse_left);
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		click = context_menu_area(self, { xx, yy, w, h, /*"contextmenuvalue"*/ STR(1076), color, e_context_type_COLOR, script, def });
		...
		if (mouseon && sBool(mouse_left_released))
		{
			...
			if (hsvmode)
				ObjType(obj_colorpicker, sInt(colorpicker))->mode = /*"hsv"*/ STR(1016);
		}
		return click;
	}
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(750), name);
			name = /*"modelstate"*/ STR(750) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(771), name);
				name = /*"blockstate"*/ STR(771) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(725);
		...
		draw_box_hover({ xx, yy, wid, hei, global::microani_arr.Value(e_microani_PRESS) });
		mouseon = (app_mouse_box(self, xx, yy, wid, hei) && !(disabled > 0) && sBool(content_mouseon)) || (menuactive && app_mouse_box(self, xx, yy, wid, hei, /*"menu"*/ STR(1077)));
		if (mouseon)
			sInt(mouse_cursor) = cr_handpoint;
		...
		if (showsearch)
		{
			...
			if (sVar(window_busy) == /*"menu"*/ STR(1077))
			{
		...
		if (!menuactive && mouseon && keyboard_check(vk_control) && sReal(mouse_wheel) != IntType(0))
		{
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1078));
			sVar(menu_current) = m;
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			sVar(window_focus) = string(sInt(menu_search_tbx));
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(726);
			app_mouse_clear(self);
		}
		if (!menuactive && mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_value = value;
			ObjType(obj_menu, m)->menu_ani = IntType(0);
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(726);
