...
namespace CppProject
{
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		name = /*"settingskey"*/ STR(1051) + idVar(keyobj, name);
		w = sVar(dw);
		...
		context_menu_area(self, { xx, yy, w, sVar(tab_control_h), /*"keybind"*/ STR(1052), keybindID, null_, null_, null_ });
		...
		draw_label(self, { text_get({ name }) + /*":"*/ STR(755), sVar(dx), sVar(dy) + ((RealType)sVar(tab_control_h) / 2.0), fa_left, fa_middle, textcolor, textalpha, sInt(font_label) });
		...
		if (draw_button_icon(self, { name + /*"edit"*/ STR(116), sVar(dx) + w - IntType(24), sVar(dy) + IntType(2), IntType(24), IntType(24), sVar(window_busy) == name, icons_PENCIL, null_, false, /*"tooltipeditkeybind"*/ STR(1053) }))
		{
