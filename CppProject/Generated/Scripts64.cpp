...
namespace CppProject
{
	...
	void tl_filter_draw(ScopeAny self)
	{
		...
		RealType colorwid;
		VarType px;
		switchwid = text_max_width({ /*"timelinehideghosts"*/ STR(3733) }) + IntType(16) + IntType(24);
		colorwid = max({ text_max_width({ /*"timelinefiltertags"*/ STR(3734) }), (IntType(24) * IntType(9)) - IntType(4) });
		draw_label(self, { text_get({ /*"timelinefiltertags"*/ STR(3734) }), sVar(dx), sVar(dy) + IntType(9), fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary, sInt(font_label) });
		sVar(dy) += IntType(20);
		...
		for (IntType i = IntType(0); i <= IntType(8); i++)
		{
			...
			microani_set(/*"hidecolortag"*/ STR(3735) + string(i), null_, false, false, sVar(timeline_hide_color_tag).Value(i));
		...
		draw_switch(self, /*"timelinehideghosts"*/ STR(3733), sVar(dx), sVar(dy), sVar(setting_timeline_hide_ghosts), ID_action_setting_timeline_hide_ghosts, /*"timelinehideghoststip"*/ STR(3736));
		tab_next(self);
	...
	BoolType tl_get_visible(ScopeAny self)
	{
		...
		if (!sArr(value_inherit).Value(e_value_VISIBLE) || (sVar(hide) > 0 && !global::render_hidden))
			return false;
		if (global::render_active == /*"image"*/ STR(90))
		{
			...
			if (global::render_active == /*"movie"*/ STR(1095))
			{
	...
	void tl_interval_settings_draw(ScopeAny self)
	{
		...
		draw_dragger(self, /*"timelineintervalssize"*/ STR(3737), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_size), .1, IntType(1), no_limit, sVar(project_tempo), IntType(1), idInt(sVar(timeline), tbx_interval_size), ID_action_tl_interval_size);
		tab_next(self);
		tab_control_dragger(self);
		draw_dragger(self, /*"timelineintervalsoffset"*/ STR(3738), sVar(dx), sVar(dy), dragger_width, sVar(timeline_interval_offset), .1, -no_limit, no_limit, IntType(0), IntType(1), idInt(sVar(timeline), tbx_interval_offset), ID_action_tl_interval_offset);
		tab_next(self);
		sVar(settings_menu_w) = (text_max_width({ /*"timelineintervalssize"*/ STR(3737), /*"timelineintervalsoffset"*/ STR(3738) }) + IntType(16) + dragger_width) + IntType(24);
	}
	...
	void tl_keyframes_paste(ScopeAny self, VarType pos)
	{
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			{
				pastemode = /*"fixed"*/ STR(3740);
				break;
			}
			tllast = sArr(copy_kf_tl_save_id).Value(k);
		}
		if (pastemode == /*"fixed"*/ STR(3740))
		{
			pastemode = /*"model"*/ STR(515);
			tllast = null_;
			for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
			{
		...
		if (pastemode == /*"free"*/ STR(3739))
		{
			if (global::tl_edit_amount != IntType(1))
				pastemode = /*"fixed"*/ STR(3740);
		}
		else
			if (pastemode == /*"model"*/ STR(515))
			{
				pastemode = /*"fixed"*/ STR(3740);
		...
		for (IntType k = IntType(0); k < sInt(copy_kf_amount); k++)
		{
			...
			if (tladd == null_)
				continue;
			if (pastemode != /*"free"*/ STR(3739) && idInt(tladd, part_list) != null_ && sArr(copy_kf_tl_model_part_name).Value(k) != /*""*/ STR(0))
			{
				VarType part;
	...
	void tl_remove_clean(ScopeAny self, VarType tl)
	{
		withOne (Object, tl, self->id)
		{
			...
			if (sVar(part_of) != null_)
				ds_list_delete_value(idInt(sVar(part_of), part_list), self->id);
			while (ds_list_size(sInt(tree_list)) > IntType(0))
				withOne (obj_timeline, DsList(sInt(tree_list)).Value(IntType(0)), self->id)
					tl_remove_clean(ScopeAny(self));
			
			
			ds_list_destroy(sInt(tree_list));
			if (sInt(part_list) != null_)
				ds_list_destroy(sInt(part_list));
			if (sVar(part_of) == null_ && sVar(temp) != null_)
				idInt(sVar(temp), count)--;
			...
			while (ds_list_size(sInt(keyframe_list)))
				withOne (obj_keyframe, DsList(sInt(keyframe_list)).Value(IntType(0)), self->id)
					instance_destroy(ScopeAny(self));
			
			
			ds_list_destroy(sInt(keyframe_list));
			withAll (obj_particle, self->id)
				if (self->creator == self.otherId)
					instance_destroy(ScopeAny(self));
			
			if (sInt(particle_list) > 0)
				ds_list_destroy(sInt(particle_list));
			if (sVar(temp) == self->id)
			{
				if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
				{
					...
					if (sInt(model_color_name_map) != null_)
						ds_map_destroy(sInt(model_color_name_map));
					if (sInt(model_color_map) != null_)
						ds_map_destroy(sInt(model_color_map));
				}
				else
			...
			if (surface_exists((IntType)(sVar(cam_surf))))
				surface_free((IntType)(sVar(cam_surf)));
			if (surface_exists((IntType)(sVar(cam_surf_tmp))))
				surface_free((IntType)(sVar(cam_surf_tmp)));
			sBool(delete_ready) = true;
		}
	...
	{
		return ((self->type == e_tl_type_BODYPART && self->model_part != null_ && idInt(self->model_part, bend_part) != null_ && idVar(self->model_part, bend_end_offset) > IntType(0) && idInt(self->model_part, bend_part) == e_part_LOWER) && (idVec(self->model_part, bend_axis).Real(X_) && !idVec(self->model_part, bend_axis).Real(Y_) && !idVec(self->model_part, bend_axis).Real(Z_)));
	}
	...
	void tl_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(773) + DsList(global::tl_type_name_list).Value(sVar(type)) });
			if (sVar(part_of) != null_)
			{
				if (sVar(type) == e_tl_type_BODYPART)
				{
					if (sVar(model_part) != null_)
						...
						sVar(display_name) = text_get({ /*"timelineunusedbodypart"*/ STR(3741) });
					...
					if (sVar(type) == e_tl_type_SPECIAL_BLOCK)
					{
						if (sVar(model_file) != null_)
							sVar(display_name) = minecraft_asset_get_name(/*"model"*/ STR(515), idVar(sVar(model_file), name));
					}
					else
						if (sVar(type) == e_tl_type_BLOCK)
						{
							sVar(display_name) = minecraft_asset_get_name(/*"block"*/ STR(508), ObjType(obj_block, DsMap(ObjType(obj_minecraft_assets, global::mc_assets)->block_name_map).Value(sVar(block_name)))->name);
						}
