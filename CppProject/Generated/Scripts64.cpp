...
namespace CppProject
{
	...
	void temp_particles_init(ScopeAny self)
	{
		...
		sVar(pc_spawn_region_type) = /*"sphere"*/ STR(531);
		...
		sVar(pc_bounding_box_type) = /*"none"*/ STR(883);
	...
	IntType temp_particles_type_add(ScopeAny self)
	{
		...
		ObjType(obj_particle_type, ptype)->name = text_get({ /*"particleeditortypedefault"*/ STR(3825), string(ds_list_size(sInt(pc_type_list))) });
	...
	{
		if (idVar(temp, model_name) != /*"armor"*/ STR(756))
			return IntType(0);
		VarType state, type;
		state = /*""*/ STR(0);
		type = /*""*/ STR(0);
	...
	void temp_update_display_name(ScopeAny self)
	{
		if (sVar(name) == /*""*/ STR(0))
		{
			sVar(display_name) = text_get({ /*"type"*/ STR(780) + DsList(global::temp_type_name_list).Value(sVar(type)) });
			switch ((IntType)sVar(type))
			{
				...
				case e_temp_type_BODYPART:
				{
					if (sVar(model_part) != null_)
						sVar(display_name) = text_get({ /*"librarybodypartof"*/ STR(3323), minecraft_asset_get_name(/*"modelpart"*/ STR(753), idVar(sVar(model_part), name)), minecraft_asset_get_name(/*"model"*/ STR(8), idVar(sVar(model_file), name)) });
					else
						sVar(display_name) = text_get({ /*"librarybodypartunknown"*/ STR(3324) });
	...
	RealType temp_update_model_color(ScopeAny self)
	{
		...
		if (sVar(model_file) != null_ && idVar(sVar(model_file), model_color) != /*"none"*/ STR(883))
		{
