...
namespace CppProject
{
	...
	IntType new_biome(StringType name, RealType tx, RealType ty, BoolType hardcoded, IntType grass, IntType foliage, IntType water, IntType base)
	{
		withOne (obj_biome, (new obj_biome)->id, noone)
		{
			...
			self->group = false;
			self->display_name = minecraft_asset_get_name(/*"biome"*/ STR(1694), name);
	...
	{
		StringType charlist = /*" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿỳ"*/ STR(1925);
	...
	IntType new_minecraft_map(StringType fn)
	{
		...
		{
			log({ /*"Minecraft map error"*/ STR(1927), /*"Not a map file"*/ STR(1928) });
			return IntType(0);
		}
		data = DsMap(rootmap).Value(/*""*/ STR(0));
		data = DsMap(data).Value(/*"data"*/ STR(1758));
		if (is_undefined(data))
		{
			log({ /*"Minecraft map error"*/ STR(1927), /*"Data not available"*/ STR(1929) });
			return IntType(0);
		}
		colors = DsMap(data).Value(/*"colors"*/ STR(1736));
		if (is_undefined(colors))
		{
			log({ /*"Minecraft map error"*/ STR(1927), /*"Colors not found"*/ STR(1930) });
			return IntType(0);
		}
		...
		surface_set_target(surf);
	{
		...
		for (IntType xx = IntType(0); xx < IntType(128); xx++)
		{
			for (IntType yy = IntType(0); yy < IntType(128); yy++)
			{
				...
				if (col > IntType(3))
					draw_point_color(xx + (IntType(1) * (IntType)is_cpp()), yy + (IntType(1) * (IntType)is_cpp()), (IntType)(global::minecraft_map_color_array.Value(col)));
			}
		}
	}
	...
	IntType new_res(ScopeAny self, VarType fn, VarType type)
	{
		...
		if (res != null_)
		{
			if (question(text_get({ /*"questionreplace"*/ STR(1931) })))
				replaced = true;
			else
			{
				res = null_;
		...
		log({ /*"Add resource"*/ STR(1932), DsList(global::res_type_name_list).Value(type) });
		log({ /*"filename"*/ STR(1252), ObjType(obj_resource, res)->filename });
		return res;
	}
	...
	IntType new_shader(StringType name)
	{
		withOne (obj_shader, (new obj_shader)->id, noone)
		{
			...
			self->script = asset_get_index(name + /*"_set"*/ STR(1933));
			self->uniform_map = ds_string_map_create();
			self->sampler_map = ds_string_map_create();
			new_shader_sampler(self, /*"uTexture"*/ STR(1934));
			new_shader_uniform(self, /*"uTextureSize"*/ STR(1935));
			new_shader_uniform(self, /*"uBlendColor"*/ STR(1936));
			new_shader_uniform(self, /*"uTime"*/ STR(1937));
			new_shader_uniform(self, /*"uWindEnable"*/ STR(1938));
			new_shader_uniform(self, /*"uWindTerrain"*/ STR(1939));
			new_shader_uniform(self, /*"uWindSpeed"*/ STR(1940));
			new_shader_uniform(self, /*"uWindStrength"*/ STR(1941));
			new_shader_uniform(self, /*"uWindDirection"*/ STR(1942));
			new_shader_uniform(self, /*"uWindDirectionalSpeed"*/ STR(1943));
			new_shader_uniform(self, /*"uWindDirectionalStrength"*/ STR(1944));
			new_shader_uniform(self, /*"uFogShow"*/ STR(1945));
			new_shader_uniform(self, /*"uFogColor"*/ STR(1946));
			new_shader_uniform(self, /*"uFogDistance"*/ STR(1947));
			new_shader_uniform(self, /*"uFogSize"*/ STR(1948));
			new_shader_uniform(self, /*"uFogHeight"*/ STR(1949));
			new_shader_uniform(self, /*"uCameraPosition"*/ STR(1950));
			new_shader_uniform(self, /*"uTAAMatrix"*/ STR(1951));
			new_shader_uniform(self, /*"uSampleIndex"*/ STR(1952));
			new_shader_uniform(self, /*"uAlphaHash"*/ STR(1953));
	...
	IntType new_tab(StringType location, BoolType show, IntType header, IntType window)
	{
		...
		location = string_replace(location, /*"_top"*/ STR(1954), /*""*/ STR(0));
		location = string_replace(location, /*"_bottom"*/ STR(1955), /*"_secondary"*/ STR(1956));
		if (!ds_map_exists(global::_app->panel_map, location))
			location = /*"right"*/ STR(198);
	...
	{
		return new_textbox(true, IntType(64), /*".0123456789"*/ STR(1957) + /*"+-/*^()"*/ STR(1958));
	}
	
	IntType new_textbox_hex()
	{
		return new_textbox(true, IntType(6), /*"0123456789ABCDEFabcdef"*/ STR(1959));
	}
	
	IntType new_textbox_integer()
	{
		return new_textbox(true, IntType(64), /*"0123456789"*/ STR(1027) + /*"+-/*^()"*/ STR(1958));
	}
	
	IntType new_textbox_ndecimals()
	{
		return new_textbox(true, IntType(64), /*"-.0123456789"*/ STR(1960) + /*"+-/*^()"*/ STR(1958));
	}
	
	IntType new_textbox_ninteger()
	{
		return new_textbox(true, IntType(64), /*" - 0123456789"*/ STR(1961) + /*"+-/*^()"*/ STR(1958));
	}
	...
	IntType new_transition_texture_map(RealType w, RealType h, RealType padding, BoolType center)
	{
			...
			surface_set_target(surf);
		{
			...
			for (RealType d = IntType(0); d <= quality; d++)
			{
				...
				if (DsList(global::transition_list).Value(t) == /*"bezier"*/ STR(554))
				{
	...
	RealType object_save(ScopeAny self)
	{
		...
		if (fn == /*""*/ STR(0))
			return IntType(0);
		fn = filename_new_ext(fn, /*".miobject"*/ STR(689));
		log({ /*"Saving object"*/ STR(1962), fn });
		...
		project_save_done();
		log({ /*"Object saved"*/ STR(1963) });
		return 0.0;
	}
	
	void open_url(StringType url)
	{
		log({ /*"Open URL"*/ STR(1964), url });
		lib_open_url(url);
	}
	...
	void panel_area_draw(ScopeAny self)
	{
		...
		withAll (obj_panel, global::_app->id)
		{
			...
			self->size_real_ani = ceil((self->size_real - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(574), self->size_ani)));
			self->glow_ani += test_reduced_motion(IntType(1), 0.035 * global::delta);
			self->glow_ani = clamp(self->glow_ani, IntType(0), IntType(1));
			self->size_glow = (self->size - IntType(16)) + (IntType(16) * ease(/*"easeoutcirc"*/ STR(574), self->glow_ani));
			self->size_real = self->size * (self->tab_list_amount > IntType(0));
		}
		
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"left_secondary"*/ STR(193)])->size_real -= max({ IntType(0), ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real - sReal(panel_area_width) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"left"*/ STR(197)])->size_real -= max({ IntType(0), ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real - sReal(panel_area_width) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"bottom"*/ STR(195)])->size_real -= max({ IntType(0), ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real - sReal(panel_area_height) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"left"*/ STR(197)])->size_real = min({ ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real, sReal(panel_area_width) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"right"*/ STR(198)])->size_real = min({ ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real, sReal(panel_area_width) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"top"*/ STR(196)])->size_real = min({ ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real, sReal(panel_area_height) });
		ObjType(obj_panel, DsMap(sInt(panel_map))[/*"bottom"*/ STR(195)])->size_real = min({ ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real, sReal(panel_area_height) });
		view_area_draw(self);
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)));
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)));
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)));
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)));
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)));
		panel_draw(self, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)));
		if (sVar(window_busy) == /*"panelresize"*/ STR(1965))
		{
			if (sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)) || sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))
			{
				...
				if (sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)) || sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))
				{
					sInt(mouse_cursor) = cr_size_we;
					idVar(sInt(panel_resize), size) = max({ IntType(200), sReal(panel_resize_size) - (gmlGlobal::mouse_x - sInt(mouse_click_x)) });
				}
				else
					if (sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))
					{
						...
						if (sInt(panel_resize) == DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))
						{
							sInt(mouse_cursor) = cr_size_ns;
		...
		if (sVar(window_busy) == /*"tabmove"*/ STR(1966))
		{
			...
			if (sInt(tab_move_mouseon_panel) == null_)
			{
				...
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real > 0))
					toph *= 0.33;
				else
					if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real > 0))
						toph *= 0.5;
				
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real > 0))
					bottomh *= 0.33;
				else
					if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real > 0))
						bottomh *= 0.5;
				
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real > 0))
					lefttopw *= 0.5;
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real > 0))
					lefttopw *= 0.5;
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real > 0))
					leftbottomw *= 0.5;
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real > 0) || !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real > 0))
					righttopw *= 0.5;
				if (!(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))->size_real > 0))
					righttopw *= 0.5;
				if (gmlGlobal::mouse_y >= sReal(view_area_y) + sReal(view_area_height) - bottomh && !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real > 0))
					sInt(tab_move_mouseon_panel) = DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195));
				else
					if (gmlGlobal::mouse_x <= sReal(view_area_x) + lefttopw && !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real > 0))
						sInt(tab_move_mouseon_panel) = DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197));
					else
						if (gmlGlobal::mouse_x <= sReal(view_area_x) + leftbottomw && !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))->size_real > 0))
							sInt(tab_move_mouseon_panel) = DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193));
						else
							if (gmlGlobal::mouse_x >= sReal(view_area_x) + sReal(view_area_width) - righttopw && !(ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real > 0))
								sInt(tab_move_mouseon_panel) = DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198));
							else
								sInt(tab_move_mouseon_panel) = DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194));
	...
	RealType panel_draw(ScopeAny self, IntType panel)
	{
		...
		if (ObjType(obj_panel, panel)->size_real < IntType(1) && !(ObjType(obj_panel, panel)->glow > 0) && panel != sInt(panel_window_obj))
			return IntType(0);
		if (panel == DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))
		{
			boxx = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real_ani;
			boxy = sReal(panel_area_y) + sReal(panel_area_height) - ObjType(obj_panel, panel)->size_real_ani;
			boxw = sReal(panel_area_width) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real_ani;
			...
			if (panel == DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))
			{
				boxx = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real_ani;
				...
				if (panel == DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))
				{
					boxx = sReal(panel_area_x);
					boxy = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani;
					boxw = ObjType(obj_panel, panel)->size_real_ani;
					boxh = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani;
					...
					if (panel == DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))
					{
						boxx = sReal(panel_area_x) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)))->size_real_ani;
						boxy = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani;
						boxw = ObjType(obj_panel, panel)->size_real_ani;
						boxh = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real_ani;
						sInt(content_direction) = e_scroll_VERTICAL;
						if (ObjType(obj_panel, panel)->glow > 0)
							draw_box(boxx, boxy, ObjType(obj_panel, panel)->size_glow, boxh, false, global::c_accent, glow_alpha);
					}
					else
						if (panel == DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))
						{
							...
							if (panel == DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))
							{
								boxx = sReal(panel_area_x) + sReal(panel_area_width) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)))->size_real_ani - ObjType(obj_panel, panel)->size_real_ani;
								boxy = sReal(panel_area_y) + ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani;
								boxw = ObjType(obj_panel, panel)->size_real_ani;
								boxh = sReal(panel_area_height) - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"top"*/ STR(196)))->size_real_ani - ObjType(obj_panel, DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(195)))->size_real_ani;
								sInt(content_direction) = e_scroll_VERTICAL;
		...
		if (idBool(sVar(content_tab), movable))
		{
			...
			for (IntType t = IntType(0); t < ObjType(obj_panel, panel)->tab_list_amount; t++)
			{
				...
				if (idBool(tab, closeable) && (hover > 0 || sel > 0))
				{
					...
					if (draw_button_icon(self, { /*"tabclose"*/ STR(1967) + string(tab), floor(dx + dw - IntType(20)), dy + IntType(4), IntType(16), IntType(16), false, icons_CLOSE_SMALL }))
					{
						tab_close(self, tab);
						return IntType(0);
					}
				...
				if (sVar(window_busy) == /*"tabmove"*/ STR(1966))
				{
					...
					sVar(window_busy) = /*"tabmove"*/ STR(1966);
				}
				if (idVar(tab, glow) > IntType(0))
					draw_box(dx, dy, dw, dh, false, global::c_accent, idVar(tab, glow) * glow_alpha);
				dx += tabw.Value(t);
			}
			if (panel == DsMap(sInt(panel_map)).Value(/*"right"*/ STR(198)) || panel == DsMap(sInt(panel_map)).Value(/*"right_secondary"*/ STR(194)))
		...
		resizemouseon = false;
		if (panel == DsMap(sInt(panel_map)).Value(/*"left"*/ STR(197)) || panel == DsMap(sInt(panel_map)).Value(/*"left_secondary"*/ STR(193)))
		{
		...
		{
			sVar(window_busy) = /*"panelresize"*/ STR(1965);
		...
		{
			sVar(window_busy) = /*"tabclick"*/ STR(1968);
			sVar(tab_move) = sVar(content_tab);
		}
		if (sVar(window_busy) == /*"tabclick"*/ STR(1968))
		{
