...
namespace CppProject
{
	void panel_draw_content(ScopeAny self)
	{
		...
		else
		{
			...
			if (sVar(dh) < minh)
			{
				if (idInt(sVar(tab), panel) != DsMap(sInt(panel_map)).Value(/*"bottom"*/ STR(186)))
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			unzip(fn);
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1909));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1909));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1910));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		{
			log({ /*"Opening particles"*/ STR(1911), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1912), fn });
		...
		if (!legacy)
		{
			...
			{
				temp_particles_type_clear(self);
				project_load_particles(self, DsMap(rootmap).Value(/*"particles"*/ STR(510)));
				temp_particles_restart(self);
			}
	...
	RealType particles_save(ScopeAny self)
	{
		...
		log({ /*"Saving particles"*/ STR(1914), fn });
		...
		if (idVar(global::temp_edit, pc_spawn_region_type) == /*"path"*/ STR(520) && idVar(global::temp_edit, pc_spawn_region_path) != null_)
		{
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(876) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
							...
							else
							{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1319))
								{
									if (!(idVar(temp, pc_spawn_region_use) > 0))
										continue;
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(513))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1313))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
												...
												if (idVar(temp, pc_spawn_region_type) == /*"path"*/ STR(520) && idVar(temp, pc_spawn_region_path) != null_)
												{
