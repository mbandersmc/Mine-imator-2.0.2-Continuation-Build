...
namespace CppProject
{
	...
	RealType particles_load(ScopeAny self, VarType fn, VarType temp)
	{
		...
		if (filename_ext(fn) == /*".zip"*/ STR(5))
		{
			...
			unzip({ fn });
			fn = file_find_single(unzip_directory, /*".miparticles;*.particles"*/ STR(1905));
			if (!file_exists_lib(fn))
				fn = file_find_single(unzip_directory + name + /*"/"*/ STR(16), /*".miparticles;*.particles"*/ STR(1905));
			if (!file_exists_lib(fn))
			{
				error(/*"erroropenparticleszip"*/ STR(1906));
				return IntType(0);
			}
		}
		if (!file_exists_lib(fn))
		...
		{
			log({ /*"Opening particles"*/ STR(1907), fn });
		...
		{
			log({ /*"Opening legacy particles"*/ STR(1908), fn });
	...
	RealType particles_save(ScopeAny self)
	{
		...
		log({ /*"Saving particles"*/ STR(1910), fn });
			...
			for (RealType s = sReal(spawn_laststep); s < sReal(spawn_currentstep); s++)
			{
				...
				for (IntType i = IntType(0); i < ds_list_size(sInt(particle_list)); i++)
				{
					...
					if (!sBool(is_timeline) || !(idVar(ObjType(obj_particle, pt)->creator, value).Value(e_value_FREEZE) > 0))
					{
						...
						if (idVar(temp, pc_bounding_box_type) != /*"none"*/ STR(875) && idVar(ObjType(obj_particle, pt)->type, bounding_box) > 0)
						{
								...
								if (idVar(temp, pc_bounding_box_type) == /*"spawn"*/ STR(1318))
								{
									...
									if (idVar(temp, pc_spawn_region_type) == /*"sphere"*/ STR(513))
									{
										...
										if (idVar(temp, pc_spawn_region_type) == /*"cube"*/ STR(3))
										{
											...
											if (idVar(temp, pc_spawn_region_type) == /*"box"*/ STR(1312))
											{
												for (IntType a = X_; a <= Z_; a++)
												{
