...
namespace CppProject
{
					...
					if (app_mouse_box(self, sVar(content_x) + (nextcoord2d.Real(X_) - IntType(13)), sVar(content_y) + (nextcoord2d.Real(Y_) - IntType(13)), IntType(26), IntType(26)) && sBool(content_mouseon) && ObjType(obj_view, view)->control_mouseon_last == null_)
					{
						...
						if (sBool(mouse_left_pressed))
						{
							...
							if (sVar(setting_timeline_select_jump) > 0)
								tl_jump(pointtl);
							sVar(window_busy) = /*"viewpathpointclick"*/ STR(3819);
						}
					}
				}
			}
	...
	RealType view_toolbar_draw(ScopeAny self, IntType view, RealType xx, RealType yy)
	{
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolselect"*/ STR(3820), xx, yy, IntType(24), IntType(24), sBool(setting_tool_select), icons_SELECT, null_, false, /*"viewtoolselecttip"*/ STR(3821) }))
			{
		...
		if (draw_button_icon(self, { /*"viewtoolmove"*/ STR(3822), xx, yy, IntType(24), IntType(24), sVar(setting_tool_move), icons_MOVE, null_, false, /*"viewtoolmovetip"*/ STR(3823) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolrotate"*/ STR(3824), xx, yy, IntType(24), IntType(24), sBool(setting_tool_rotate), icons_ROTATE, null_, false, /*"viewtoolrotatetip"*/ STR(3825) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtoolscale"*/ STR(3826), xx, yy, IntType(24), IntType(24), sBool(setting_tool_scale), icons_SCALE, null_, false, /*"viewtoolscaletip"*/ STR(3827) }))
			{
				if (sVar(setting_separate_tool_modes) > 0)
				{
					action_tools_disable_all(self);
		...
		if (draw_button_icon(self, { /*"viewtoolbend"*/ STR(3828), xx, yy, IntType(24), IntType(24), sBool(setting_tool_bend), icons_BEND, null_, false, /*"viewtoolbendtip"*/ STR(3829) }))
		{
			if (sVar(setting_separate_tool_modes) > 0)
			{
				action_tools_disable_all(self);
		...
		if (sVar(setting_separate_tool_modes) > 0)
		{
			...
			if (draw_button_icon(self, { /*"viewtooltransform"*/ STR(3830), xx, yy, IntType(24), IntType(24), sBool(setting_tool_transform), icons_MULTITRANSFORM, null_, false, /*"viewtooltransformtip"*/ STR(3831) }))
			{
				action_tools_disable_all(self);
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(113)))
		{
			...
			if (sBool(mouse_left_pressed))
			{
				sVar(window_busy) = /*"viewclick"*/ STR(3832);
			...
			if ((!(cam > 0) || editcamobj) && sBool(mouse_right_pressed))
			{
				...
				sVar(window_busy) = /*"viewmovecamera"*/ STR(3833);
				sVar(window_focus) = string(view);
				if (cam > 0)
		...
		if (((((sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == /*"place"*/ STR(113)) && sBool(content_mouseon)) || (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3834) && sVar(window_focus) == string(view)))) && sReal(mouse_wheel) != IntType(0))
		{
			if (!(cam > 0))
		...
		{
			if (sVar(window_busy) == /*"viewclick"*/ STR(3832))
			{
				sInt(mouse_cursor) = cr_handpoint;
				if ((!(cam > 0) || editcamobj) && sReal(mouse_move) > IntType(5))
				{
					...
					{
						sVar(window_busy) = /*"viewpancamera"*/ STR(3835);
						sVar(window_focus) = string(view);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
					else
					{
						sInt(view_click_x) = display_mouse_get_x();
						sInt(view_click_y) = display_mouse_get_y();
						sVar(window_busy) = /*"viewrotatecamera"*/ STR(3834);
						if (cam > 0)
							action_tl_select_single(self, { cam });
					}
			...
			if (sVar(window_busy) == /*"viewrotatecamera"*/ STR(3834))
			{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3833))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2851);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2852);
			...
			if (sVar(window_busy) == /*"viewpancamera"*/ STR(3835))
			{
		...
		if (sVar(window_busy) == /*"viewpathpointclick"*/ STR(3819))
			sVar(window_busy) = /*""*/ STR(0);
	}
		...
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				...
				if (sStr(window_state) == /*"startup"*/ STR(25))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2662))
					{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3109) }) + /*" - Mine-imator"*/ STR(674));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3836) : /*"exporttimelefthours"*/ STR(3837)), string(timelefthours) }) + /*", "*/ STR(1011);
		if (timeleftmins > IntType(0))
			timeleftstr += text_get({ ((timeleftmins == IntType(1)) ? /*"exporttimeleftminute"*/ STR(3838) : /*"exporttimeleftminutes"*/ STR(3839)), string(timeleftmins) }) + /*" "*/ STR(17) + text_get({ /*"exporttimeleftand"*/ STR(3840) }) + /*" "*/ STR(17);
		timeleftstr += text_get({ ((timeleftsecs == IntType(1)) ? /*"exporttimeleftsecond"*/ STR(3841) : /*"exporttimeleftseconds"*/ STR(3842)), string(timeleftsecs) });
		draw_label(self, { text_get({ /*"exporttimeleft"*/ STR(3843), timeleftstr }), framex + (RealType)framew / 2.0, framey + frameh + IntType(33), fa_center, fa_bottom, global::c_text_secondary, global::a_text_secondary, sVar(font_label_big) });
		VarType loadtext, loadw;
		RealType sw, sh;
		loadtext = text_get({ /*"exportloading"*/ STR(3844), string(floor(perc * IntType(100))) });
		loadw = framew;
		...
		if (usesamples > 0)
			samplecount = text_get({ /*"exportsamples"*/ STR(3845), string(max({ global::render_samples, IntType(1) })), string(sVar(project_render_samples)) });
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3846), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1098) + samplecount + /*")"*/ STR(1074)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
		...
		draw_label(self, { sStr(window_state) == /*"export_movie"*/ STR(49) ? text_get({ /*"exportmovietitle"*/ STR(3847) }) : text_get({ /*"exportimagetitle"*/ STR(3848) }), framex + (RealType)framew / 2.0, framey - IntType(35), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading_big) });
		draw_label(self, { text_get({ /*"exportstop"*/ STR(3849) }), framex + (RealType)framew / 2.0, framey - IntType(16), fa_center, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
		return 0.0;
	}
	...
	RealType window_draw_load_assets(ScopeAny self)
	{
		...
		{
			global::load_assets_stage = /*"exit"*/ STR(3850);
			return IntType(0);
		}
		else
			if (global::load_assets_stage == /*"exit"*/ STR(3850))
			{
		...
		draw_label(self, { /*"Mine-imator "*/ STR(3851) + string(mineimator_version), xoff + IntType(95), yoff + IntType(289), fa_middle, fa_bottom, global::c_text_secondary, global::a_text_secondary, sInt(font_heading) });
		...
		draw_label(self, { text_get({ /*"startuploadingassets"*/ STR(3852), global::_app->setting_minecraft_assets_version, floor(global::load_assets_progress * IntType(100)) }), xoff + IntType(95), yoff + IntType(437), fa_middle, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_caption) });
	...
	RealType window_draw_new_assets(ScopeAny self)
	{
		...
		draw_label(self, { text_get({ /*"newassetstitle"*/ STR(3853), sStr(new_assets_version) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(227), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading) });
		draw_label(self, { text_get({ /*"newassetssubtitle"*/ STR(3854) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(244), fa_center, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(264);
		sVar(dx) += IntType(12);
		sVar(dw) -= 24.0;
		if (sStr(new_assets_stage) == /*"download"*/ STR(104))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), sReal(new_assets_download_progress), text_get({ /*"newassetsdownloading"*/ STR(3855) }));
			tab_next(self);
		}
		else
		{
			draw_set_font(sInt(font_button));
			IntType capwid = string_width(text_get({ /*"newassetsdownload"*/ STR(3856) })) + button_padding;
			if (draw_button_label(self, /*"newassetsdownload"*/ STR(3856), sVar(dx) + sVar(dw), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_PRIMARY, null_, fa_right))
			{
				if (sVar(new_assets_format) > minecraft_assets_format)
				{
					if (question(text_get({ /*"questionassetsnewer"*/ STR(3857) })))
					{
			...
			if (draw_button_label(self, /*"newassetslater"*/ STR(3858), (sVar(dx) + sVar(dw)) - (capwid + IntType(8)), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_SECONDARY, null_, fa_right))
			{
			...
			sVar(dy_start) = sVar(dy);
			sVar(dy) += IntType(16);
			draw_label(self, { text_get({ /*"newassetschangelog"*/ STR(3859) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(20);
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(sArr(new_assets_changes_lines))); i++)
			{
				...
				if (string_copy(text, IntType(1), IntType(2)) == /*"- "*/ STR(3860))
				{
				...
				else
				{
					if (string_copy(text, IntType(1), IntType(2)) == /*"* "*/ STR(3861))
						text = /*"• "*/ STR(3862) + string_delete(text, IntType(1), IntType(2));
