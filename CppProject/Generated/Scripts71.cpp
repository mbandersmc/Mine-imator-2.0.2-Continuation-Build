...
namespace CppProject
{
	...
	{
		vbuffer_start();
		VecType texsize = point2D(1.0 / 3.0, 0.5);
		if (mapped > 0)
		{
			if (invert > 0)
		...
		for (IntType i = IntType(0); i < array_length(VarType::CreateRef(frames)) - IntType(1); i++)
		{
			...
			while (j < IntType(1))
			{
				...
				if (!(closed > 0))
				{
					...
					{
						if (invert > 0)
							vbuffer_add_triangle({ p2, points.Value(i), p1, t2, t3, t1 });
						else
					...
					if (i == (array_length(VarType::CreateRef(points)) - IntType(2)))
					{
						...
						t3.Real(X_) += (1.0 / 3.0);
						if (invert > 0)
							vbuffer_add_triangle({ p3, points.Value(i + IntType(1)), p4, t1, t3, t2 });
						else
	...
	{
		log({ /*"Create vertex format"*/ STR(3903) });
		vertex_format_begin();
	...
	void view_area_draw(ScopeAny self)
	{
		...
		if (sVar(window_busy) == /*"viewresizehor"*/ STR(3904) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3905))
		{
		...
		if (sVar(window_busy) == /*"viewresizever"*/ STR(3906) || sVar(window_busy) == /*"viewresizeboth"*/ STR(3905))
		{
		...
		if (sVar(window_busy) == /*"viewresizeboth"*/ STR(3905))
		{
		...
		if (sVar(window_busy) == /*"viewresizesplithor"*/ STR(3907))
		{
			sInt(mouse_cursor) = cr_size_we;
		...
		if (sVar(window_busy) == /*"viewresizesplitver"*/ STR(3908))
		{
			sInt(mouse_cursor) = cr_size_ns;
	...
	{
		IntType surf = surface_create((IntType)(sVar(content_width)), (IntType)(sVar(content_height)));
