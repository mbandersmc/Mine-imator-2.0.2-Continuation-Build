...
namespace CppProject
{
	...
	void view_update(ScopeAny self, IntType view, VarType cam)
	{
		...
		if (sVar(window_focus) == string(view))
		{
			...
			if (sVar(window_busy) == /*"viewmovecamera"*/ STR(3830))
			{
				global::render_samples = -IntType(1);
				if (cam == null_)
					sStr(shortcut_bar_state) = /*"cameramove"*/ STR(2848);
				else
					sStr(shortcut_bar_state) = /*"tlcameramove"*/ STR(2849);
		...
		switch ((IntType)window_get_current())
		{
			case e_window_MAIN:
			{
				...
				if (sStr(window_state) == /*"startup"*/ STR(25))
				{
					...
					if (sStr(window_state) == /*"world_import"*/ STR(2659))
					{
			...
			case e_window_TIMELINE:
			{
				...
				if (sInt(menu_popup) == null_ && sInt(popup) > 0)
					menu_draw(self);
				window_set_caption(text_get({ /*"tabtimeline"*/ STR(3106) }) + /*" - Mine-imator"*/ STR(674));
				break;
			}
	...
	RealType window_draw_export(ScopeAny self)
	{
		...
		if (timelefthours > IntType(0))
			timeleftstr += text_get({ ((timelefthours == IntType(1)) ? /*"exporttimelefthour"*/ STR(3833) : /*"exporttimelefthours"*/ STR(3834)), string(timelefthours) }) + /*", "*/ STR(1011);
		...
		VarType text = /*""*/ STR(0);
		if (sStr(window_state) == /*"export_movie"*/ STR(49))
			text = text_get({ /*"exportframe"*/ STR(3843), string(sReal(exportmovie_frame)), string(totalframes) }) + ((usesamples > 0) ? (/*" ("*/ STR(1098) + samplecount + /*")"*/ STR(1074)) : /*""*/ STR(0));
		else
			if (usesamples > 0)
				text = samplecount;
