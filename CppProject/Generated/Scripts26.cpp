...
namespace CppProject
{
	RealType draw_button_label(ScopeAny self, VarType name, VarType xx, VarType yy, VarType w, IntType icon, IntType type, IntType script, IntType anchor, BoolType disabled)
	{
		...
		IntType font;
		VarType cap;
		BoolType customw, mouseon, mouseclick;
		h = IntType(32);
		cap = text_get({ name });
	...
	BoolType draw_button_menu(ScopeAny self, VarArgs argument)
	{
		...
		{
			cap = minecraft_asset_get_name(/*"modelstate"*/ STR(744), name);
			name = /*"modelstate"*/ STR(744) + name;
		}
		else
			if (sVar(menu_block_current) != null_)
			{
				cap = minecraft_asset_get_name(/*"blockstate"*/ STR(763), name);
				name = /*"blockstate"*/ STR(763) + name;
			}
			else
				cap = text_get({ name });
		...
		for (IntType i = IntType(0); i < ds_list_size(sInt(menu_list)); i++)
		{
			...
			{
				menuactive = true;
				menuhide = ObjType(obj_menu, DsList(sInt(menu_list)).Value(i))->menu_ani_type == /*"hide"*/ STR(719);
		...
		if (mouseon && sBool(mouse_left_released) && menuhide)
		{
			...
			ObjType(obj_menu, menuid)->menu_ani_type = /*"show"*/ STR(720);
		...
		if (mouseon && sBool(mouse_left_released) && !menuhide)
		{
			...
			ObjType(obj_menu, m)->menu_ani_type = /*"show"*/ STR(720);
			...
			ObjType(obj_menu, m)->menu_include_tl_edit = (ObjType(obj_menu, m)->menu_name != /*"timelineeditorparent"*/ STR(1041));
	...
	void draw_button_text(ScopeAny self, VarType text, VarType xx, VarType yy, IntType script, VarType value, StringType tip, IntType font)
	{
		IntType wid, hei;
		BoolType mouseon;
		draw_set_font(font);
	...
	RealType draw_button_transition(ScopeAny self, VarType xx, VarType yy, VarType transition)
	{
		...
		tipname = transition;
		if (tipname != /*"linear"*/ STR(535) && tipname != /*"instant"*/ STR(536) && tipname != /*"bezier"*/ STR(537))
		{
			tipname = string_replace(tipname, /*"easeinout"*/ STR(1042), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easein"*/ STR(1043), /*""*/ STR(0));
			tipname = string_replace(tipname, /*"easeout"*/ STR(1044), /*""*/ STR(0));
			tipname = /*"ease"*/ STR(1045) + tipname;
		}
		if (transition == /*"bezier"*/ STR(537) && !(sReal(setting_advanced_mode) > 0))
			return IntType(0);
		return draw_button_icon(self, { /*"menu"*/ STR(1040) + transition, xx, yy, IntType(36), IntType(36), active, null_, null_, false, /*"transition"*/ STR(1046) + tipname, DsMap(sVar(transition_texture_map)).Value(transition) });
	}
	...
	RealType draw_dragger(ScopeAny self, StringType name, VarType xx, VarType yy, RealType wid, VarType value, RealType mul, RealType minval, VarType maxval, VarType def, RealType snapval, VarType tbx, IntType script, VarType capwidth, BoolType showcaption, BoolType disabled, StringType tip)
	{
		...
		if (!disabled)
			context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1039), value, e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
		...
		if (dragmouseon && sBool(mouse_left_pressed))
			sVar(window_focus) = name + /*"press"*/ STR(1047);
		if (sVar(window_focus) == name + /*"press"*/ STR(1047))
		{
		...
		if (sVar(window_busy) != name + /*"drag"*/ STR(30) && sVar(window_busy) != name + /*"press"*/ STR(1047) && sVar(window_focus) != string(tbx))
			idVar(tbx, text) = string_decimals(value);
		return 0.0;
	}
	...
	RealType draw_dragger_sky(ScopeAny self, StringType name, VarType xx, VarType yy, VarType value, RealType def, IntType script, IntType tbx, RealType time)
	{
		...
		context_menu_area(self, { xx, yy, wid + capwidth, hei, /*"contextmenuvalue"*/ STR(1039), value, (time > 0) ? e_context_type_TIME : e_context_type_NUMBER, script, def });
		fieldx = xx + capwidth;
	...
	RealType draw_inputbox(ScopeAny self, StringType inputname, VarType xx, VarType yy, RealType w, RealType h, VarType placeholder, VarType tbx, IntType script, BoolType disabled, BoolType err, IntType font, IntType type, RealType alpha)
	{
		IntType capwid;
		RealType padding;
		BoolType focused, update;
		...
		if (xx + w < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || yy + h < sVar(content_y) || yy > sVar(content_y) + sVar(content_height))
			return IntType(0);
		BoolType mouseon;
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == string(tbx) + /*"click"*/ STR(1048)) && !disabled;
		...
		if (err)
		{
			draw_image({ ID_spr_icons, icons_WARNING_TRIANGLE, xx + w - IntType(14), yy + (h / 2.0), IntType(1), IntType(1), global::c_error, IntType(1) });
			w -= 28.0;
		}
		if (string_contains(inputname, /*"search"*/ STR(1049)))
		{
			draw_image({ ID_spr_icons, icons_SEARCH, xx + w - IntType(14), yy + (h / 2.0), IntType(1), IntType(1), bordercolor, borderalpha });
			w -= 28.0;
		}
		draw_set_font(font);
		RealType tbxh;
		...
		{
			textx = xx + IntType(8);
			texty = yy + floor(h / 2.0) - ceil(tbxh / 2.0);
			texthalign = fa_left;
			textvalign = fa_top;
			placeholderx = textx;
		...
		if (!disabled)
		{
			...
			{
				RealType textwid = min({ w, string_width(idVar(tbx, text) + idVar(tbx, suffix)) });
				update = textbox_draw(self, tbx, xx + w / 2.0 - textwid / 2.0, yy + IntType(5), textwid, h - IntType(9));
			}
			else
		...
		if (sVar(window_focus) == string(tbx))
			context_menu_area(self, { xx, yy, w, h, /*"contextmenutextbox"*/ STR(1050), tbx, e_context_type_NONE, null_, null_ });
	...
	RealType draw_keybind(ScopeAny self, IntType keybindID, VarType xx, VarType yy)
	{
		...
		context_menu_area(self, { xx, yy, w, sReal(tab_control_h), /*"keybind"*/ STR(1052), keybindID, null_, null_, null_ });
		...
		draw_label(self, { text_get({ name }) + /*":"*/ STR(755), sVar(dx), sVar(dy) + (sReal(tab_control_h) / 2.0), fa_left, fa_middle, textcolor, textalpha, sInt(font_label) });
