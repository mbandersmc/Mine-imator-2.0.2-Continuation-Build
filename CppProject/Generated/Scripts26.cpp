...
namespace CppProject
{
	...
	RealType draw_button_label(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType name, xx, yy, w, icon, type, script, anchor, disabled;
		RealType h;
		IntType font;
		VarType cap;
		RealType capwid;
		...
		if (yy > sVar(content_y) + sVar(content_height) || yy + h < sVar(content_y) || xx > sVar(content_x) + sVar(content_width) || xx + w < sVar(content_x))
			return IntType(0);
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && !(disabled > 0);
	...
	RealType draw_dragger(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType name, xx, yy, wid, value, mul, minval, maxval, def, snapval, tbx, script, capwidth, showcaption, disabled, tip;
		...
		script = argument[IntType(11)];
		if (argument_count > IntType(12))
			capwidth = argument[IntType(12)];
		else
			capwidth = null_;
		...
		if (argument_count > IntType(14))
			disabled = argument[IntType(14)];
		else
			disabled = false;
		
		if (argument_count > IntType(15))
			tip = argument[IntType(15)];
		else
			tip = /*""*/ STR(0);
		
		hei = sReal(ui_small_height);
		if (capwidth == null_ && showcaption > 0)
		...
		fieldx = xx + capwidth;
		dragmouseon = app_mouse_box(self, fieldx, yy, wid, hei) && sBool(content_mouseon) && (sVar(window_focus) != string(tbx)) && !(disabled > 0);
		...
		if (sVar(window_focus) == name + /*"press"*/ STR(1047))
		{
			...
			if (!sBool(mouse_left))
			{
				...
				app_mouse_clear(self);
				if (app_mouse_box(self, fieldx, yy, wid, hei) && !(disabled > 0))
				{
	...
	RealType draw_help_circle(ScopeAny self, VarType text, VarType xx, VarType yy, VarType disabled)
	{
		...
		IntType color;
		RealType alpha;
		mouseon = app_mouse_box(self, xx, yy, IntType(20), IntType(20)) && sBool(content_mouseon) && !(disabled > 0);
	...
	RealType draw_inputbox(ScopeAny self, VarArgs argument)
	{
		...
		if (xx + w < sVar(content_x) || xx > sVar(content_x) + sVar(content_width) || yy + h < sVar(content_y) || yy > sVar(content_y) + sVar(content_height))
			return IntType(0);
		BoolType mouseon;
		mouseon = app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && (sVar(window_busy) == /*""*/ STR(0) || sVar(window_busy) == string(tbx) + /*"click"*/ STR(1048)) && !(disabled > 0);
