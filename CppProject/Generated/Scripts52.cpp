...
namespace CppProject
{
	void render_world_tl_reset()
	{
		...
		render_set_uniform_int(/*"uGlintEnabled"*/ STR(2572), IntType(0));
	...
	void res_load(ScopeAny self, BoolType reload)
	{
		...
		debug({ /*"Loading "*/ STR(2577) + DsList(global::res_type_name_list).Value(sVar(type)), fn });
		switch ((IntType)sVar(type))
		{
			...
			case e_res_type_BLOCK_SHEET:
			{
				...
				if (sVar(type) == e_res_type_LEGACY_BLOCK_SHEET)
				{
					...
					if (global::load_folder == global::save_folder)
						sVar(filename) = filename_new_ext(filename_name(fn), /*"_converted"*/ STR(2578) + filename_ext(fn));
			...
			{
				StringType cachefn = fn + /*".meshcache"*/ STR(2579);
			...
			case e_res_type_TEXTURE:
			{
				if (sVar(texture) > 0)
					texture_free(sVar(texture));
				sVar(texture) = null_;
				if (filename_ext(fn) == /*".dat"*/ STR(715))
				{
					IntType map = new_minecraft_map(fn);
					if (!(map > 0))
					{
						error(/*"errorloadmap"*/ STR(2580));
						break;
					}
					else
	...
	RealType res_load_pack(ScopeAny self)
	{
		...
		switch (StringType(sStr(load_stage)).id)
		{
			case 1295: // unzip
			{
				debug({ /*"res_load_pack"*/ STR(2587), /*"unzip"*/ STR(1295) });
				if (sVar(type) != e_res_type_PACK_UNZIPPED)
				{
					if (!unzip(fname))
					{
						log({ /*"Error unzipping pack"*/ STR(2588) });
						error(/*"errorunzippack"*/ STR(2589));
						withOne (app, global::_app->id, self->id)
				...
				sStr(load_stage) = /*"modeltextures"*/ STR(1377);
				global::load_assets_dir = unzip_directory;
				res_load_pack_version(self);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackmodeltextures"*/ STR(2590) });
					ObjType(obj_popup, global::_app->popup_loading)->progress = 0.25;
				}
			...
			{
				debug({ /*"res_load_pack"*/ STR(2587), /*"modeltextures"*/ STR(1377) });
				res_load_pack_model_textures(self);
				sStr(load_stage) = /*"blocktextures"*/ STR(2591);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackblocktextures"*/ STR(2592) });
					ObjType(obj_popup, global::_app->popup_loading)->progress = 0.5;
				}
				
				break;
			}
			
			case 2591: // blocktextures
			{
				debug({ /*"res_load_pack"*/ STR(2587), /*"blocktextures"*/ STR(2591) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"blocks"*/ STR(999), global::load_assets_dir + mc_textures_directory + /*"block"*/ STR(4));
				res_load_pack_block_textures(self);
				sStr(load_stage) = /*"itemtextures"*/ STR(2593);
				withOne (app, global::_app->id, self->id)
				{
					ObjType(obj_popup, global::_app->popup_loading)->text = text_get({ /*"loadpackitemtextures"*/ STR(2594) });
					ObjType(obj_popup, global::_app->popup_loading)->progress = 0.75;
				}
				
				break;
			}
			
			case 2593: // itemtextures
			{
				debug({ /*"res_load_pack"*/ STR(2587), /*"itemtextures"*/ STR(2593) });
				file_rename_lib(global::load_assets_dir + mc_textures_directory + /*"items"*/ STR(2595), global::load_assets_dir + mc_textures_directory + /*"item"*/ STR(526));
				res_load_pack_item_textures(self, /*"diffuse"*/ STR(612), /*""*/ STR(0));
				res_load_pack_item_textures(self, /*"material"*/ STR(617), /*"_s"*/ STR(2596));
				res_load_pack_item_textures(self, /*"normal"*/ STR(606), /*"_n"*/ STR(2597));
				res_load_pack_particle_textures(self);
				res_load_pack_misc(self);
				res_update_colors(self);
				sBool(ready) = true;
				global::_app->history_resource_update = true;
				log({ /*"Pack loaded"*/ STR(2598) });
