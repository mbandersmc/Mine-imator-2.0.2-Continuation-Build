...
namespace CppProject
{
	void res_load_pack_block_sheet(ScopeAny self, StringType type, StringType suffix)
	{
		...
		log({ /*"Block textures"*/ STR(2494), type, /*"load static"*/ STR(2495) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Block texture not found"*/ STR(2496), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
			
		}
		log({ /*"Block textures"*/ STR(2494), type, /*"load animated"*/ STR(2497) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Animated block texture not found"*/ STR(2498), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix });
		...
		log({ /*"Block textures, blocksize"*/ STR(2500), type, blocksize });
		draw_texture_start();
		surf = surface_create((IntType)(block_sheet_width * blocksize), (IntType)(block_sheet_height * blocksize));
		log({ /*"Block textures"*/ STR(2494), /*"static block surface"*/ STR(2501) });
		surface_set_target(surf);
	{
		...
		if (self->id == global::mc_res)
		{
			...
			log({ /*"Block textures"*/ STR(2494), /*"static block preview"*/ STR(2502) });
		...
		log({ /*"Block textures"*/ STR(2494), type, /*"animated block surfaces"*/ STR(2503) });
		...
		draw_texture_start();
		gpu_set_blendmode_ext(bm_one, bm_inv_src_alpha);
		log({ /*"Block textures"*/ STR(2494), type, /*"animated frames"*/ STR(2504) });
		for (IntType t = IntType(0); t < ds_list_size(texanilist); t++)
		{
			...
			fname = global::load_assets_dir + mc_textures_directory + DsList(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list).Value(t) + suffix + /*".png.mcmeta"*/ STR(2505);
			if (file_exists_lib(fname))
			{
				IntType map = json_load({ fname });
				if (ds_map_valid(map))
				{
					VarType animation = DsMap(map).Value(/*"animation"*/ STR(2506));
					if (!is_undefined(animation))
					{
						if (!is_undefined(DsMap(animation).Value(/*"interpolate"*/ STR(2507))))
							framefade = DsMap(animation).Value(/*"interpolate"*/ STR(2507));
						if (!is_undefined(DsMap(animation).Value(/*"frametime"*/ STR(2508))))
							frametime = DsMap(animation).Value(/*"frametime"*/ STR(2508));
		...
		if (self->id == global::mc_res)
		{
			log({ /*"Block textures"*/ STR(2494), /*"animated block preview"*/ STR(2509) });
		...
		if (type == /*"diffuse"*/ STR(607))
		{
			...
			log({ /*"Block textures"*/ STR(2494), /*"find static block depths"*/ STR(2510) });
			...
			log({ /*"Block textures"*/ STR(2494), /*"find animated block depths"*/ STR(2511) });
		...
		log({ /*"Block textures"*/ STR(2494), type, /*"done"*/ STR(1699) });
		debug_timer_stop(/*"Block textures: "*/ STR(2512) + type);
	}
	...
	void res_load_pack_block_textures(ScopeAny self)
	{
		...
		if (self->id != global::mc_res)
		{
			res_load_pack_block_sheet(self, /*"material"*/ STR(612), /*"_s"*/ STR(2491));
			res_load_pack_block_sheet(self, /*"normal"*/ STR(601), /*"_n"*/ STR(2492));
		}
		else
		{
		...
		log({ /*"Block textures all"*/ STR(2513), /*"done"*/ STR(1699) });
	}
	...
	void res_load_pack_item_textures(ScopeAny self, StringType type, StringType suffix)
	{
		...
		log({ /*"Item textures"*/ STR(2514), type, /*"load"*/ STR(2515) });
		texlist = ds_list_create();
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list)); t++)
		{
			...
			{
				if (dev_mode)
					log({ /*"Item texture not found"*/ STR(2516), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->item_texture_list).Value(t) + suffix });
				ds_list_add({ texlist, null_ });
			}
		...
		if (itemsize == null_)
			itemsize = item_size;
		log({ /*"Item textures"*/ STR(2514), /*"surface"*/ STR(514) });
		draw_texture_start();
		...
		surface_set_target(surf);
	{
		...
		surface_free(surf);
		ds_list_destroy(texlist);
		log({ /*"Item textures"*/ STR(2514), type, /*"done"*/ STR(1699) });
		debug_timer_stop(/*"Item textures: "*/ STR(2518) + type);
	}
	...
	void res_load_pack_model_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Model textures"*/ STR(2519), /*"load"*/ STR(2515) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list)); t++)
		{
			...
			matfname = global::load_assets_dir + mc_textures_directory + name + /*"_s.png"*/ STR(2520);
			norfname = global::load_assets_dir + mc_textures_directory + name + /*"_n.png"*/ STR(2521);
			if (file_exists_lib(fname))
			{
				if (name == /*"entity/steve"*/ STR(2522))
				...
				{
					if (dev_mode)
						log({ /*"Model texture not found"*/ STR(2523), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->model_texture_list).Value(t) });
			...
			{
				if (matfname == /*"entity/steve_s"*/ STR(2524))
			...
			{
				if (norfname == /*"entity/steve_n"*/ STR(2525))
		...
		log({ /*"Model textures"*/ STR(2519), /*"done"*/ STR(1699) });
		debug_timer_stop(/*"Model textures"*/ STR(2519));
	}
	...
	void res_load_pack_particle_textures(ScopeAny self)
	{
		...
		debug_timer_start();
		log({ /*"Particle textures"*/ STR(2526), /*"load"*/ STR(2515) });
		...
		for (IntType t = IntType(0); t < ds_list_size((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list)); t++)
		{
			...
			if (file_exists_lib(fname))
			{
				...
				{
					if (dev_mode)
						log({ /*"Particle texture not found"*/ STR(2527), DsList(ObjType(obj_minecraft_assets, global::mc_assets)->particle_texture_list).Value(t) });
					tex = texture_create_missing();
				}
			...
			{
				if (string_contains(name, /*"explosion"*/ STR(2528)))
				{
		...
		log({ /*"Particle textures"*/ STR(2526), /*"legacy sheets"*/ STR(2529) });
		...
		log({ /*"Particle textures"*/ STR(2526), /*"done"*/ STR(1699) });
		debug_timer_stop(/*"Particle textures"*/ STR(2526));
	}
	
	BoolType res_load_pack_version(ScopeAny self)
	{
		StringType fn = global::load_assets_dir + /*"pack.mcmeta"*/ STR(2530);
		if (file_exists_lib(fn))
		{
			IntType map = json_load({ fn });
			if (!ds_map_valid(map))
			{
				log({ /*"Error loading pack.mcmeta"*/ STR(2531) });
			...
			if (ds_map_valid(DsMap(map).Value(/*"pack"*/ STR(524))))
			{
				...
				sVar(pack_format) = value_get_real({ DsMap(packmap).Value(/*"pack_format"*/ STR(2532)) });
			}
		}
		else
			sVar(pack_format) = e_minecraft_pack_LATEST;
