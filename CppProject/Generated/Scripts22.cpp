...
namespace CppProject
{
	VarType block_load_model_file(StringType fname, IntType res)
	{
		...
		withOne (obj_block_load_model_file, (new obj_block_load_model_file)->id, noone)
		{
			...
			self->parent = null_;
			if (res == null_ && is_string(DsMap(map).Value(/*"parent"*/ STR(816))))
				self->parent = block_load_model_file(global::load_assets_dir + mc_models_directory + string_replace(DsMap(map).Value(/*"parent"*/ STR(816)), /*"minecraft:"*/ STR(813), /*""*/ STR(0)) + /*".json"*/ STR(702));
			self->texture_map = null_;
			if (is_real(DsMap(map).Value(/*"textures"*/ STR(817))))
			{
				self->texture_map = ds_map_create();
				if (ds_map_find_value((IntType)(DsMap(typemap).Value(map)), /*"textures"*/ STR(817)) == e_json_type_ARRAY && ds_list_valid(DsMap(map).Value(/*"textures"*/ STR(817))))
				{
					for (IntType i = IntType(0); i < ds_list_size((IntType)(DsMap(map).Value(/*"textures"*/ STR(817)))); i++)
					{
						StringType texname = ds_list_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(817))), i);
						texname = string_replace(texname, /*"minecraft:"*/ STR(813), /*""*/ STR(0));
					...
					{
						VarType key = ds_map_find_first((IntType)(DsMap(map).Value(/*"textures"*/ STR(817))));
						while (!is_undefined(key))
						{
							StringType texname = ds_map_find_value((IntType)(DsMap(map).Value(/*"textures"*/ STR(817))), key);
							texname = string_replace(texname, /*"minecraft:"*/ STR(813), /*""*/ STR(0));
							DsMap(self->texture_map)[key] = block_load_model_file_texture(self, texname, res);
							key = ds_map_find_next((IntType)(DsMap(map).Value(/*"textures"*/ STR(817))), key);
						}
			...
			VarType elementslist = DsMap(map).Value(/*"elements"*/ STR(818));
			if (!is_undefined(elementslist))
			{
				...
				for (IntType i = IntType(0); i < ds_list_size((IntType)(elementslist)); i++)
				{
					withOne (obj_block_load_element, (new obj_block_load_element)->id, self->id)
					{
						...
						self->from = value_get_point3D(DsMap(elementmap).Value(/*"from"*/ STR(819)));
						self->to = value_get_point3D(DsMap(elementmap).Value(/*"to"*/ STR(820)));
						self->size = point3D_sub(self->to, self->from);
						self->volume = self->size.Value(X_) * self->size.Value(Y_) * self->size.Value(Z_);
						VarType rotationmap = DsMap(elementmap).Value(/*"rotation"*/ STR(821));
						if (ds_map_valid(rotationmap))
						{
							VarType origin, angle;
							VecType rot, scale;
							origin = value_get_point3D(DsMap(rotationmap).Value(/*"origin"*/ STR(822)), point3D(IntType(8), IntType(8), IntType(8)));
							angle = IntType(0);
							rot = vec3(IntType(0));
							scale = vec3(IntType(1));
							if (is_real(DsMap(rotationmap).Value(/*"angle"*/ STR(823))))
								angle = snap(clamp(DsMap(rotationmap).Value(/*"angle"*/ STR(823)), -IntType(45), IntType(45)), 22.5);
							if (is_bool(DsMap(rotationmap).Value(/*"rescale"*/ STR(824))) && DsMap(rotationmap).Value(/*"rescale"*/ STR(824)) > 0)
								scale = vec3(1.0 / dcos(abs(angle)));
							if (is_string(DsMap(rotationmap).Value(/*"axis"*/ STR(807))))
							{
								switch (StringType(DsMap(rotationmap).Value(/*"axis"*/ STR(807))).id)
								{
									case 825: // x
									{
										rot.Real(X_) = angle;
						...
						VarType facesmap = DsMap(elementmap).Value(/*"faces"*/ STR(827));
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							if (!is_undefined(curmap))
							{
								...
								if (ds_list_valid(DsMap(curmap).Value(/*"uv"*/ STR(828))))
								{
									VarType uvlist = DsMap(curmap).Value(/*"uv"*/ STR(828));
								...
								self->face_texture[f] = DsMap(curmap).Value(/*"texture"*/ STR(549));
								self->face_rotation[f] = IntType(0);
								if (is_real(DsMap(curmap).Value(/*"rotation"*/ STR(821))))
									self->face_rotation[f] = DsMap(curmap).Value(/*"rotation"*/ STR(821));
							}
							else
	...
	VarType block_load_model_file_texture(Scope<obj_block_load_model_file> self, VarType name, IntType res)
	{
		...
		if (string_pos(/*"assets/minecraft_"*/ STR(829), name) == IntType(1))
			name = string_replace(name, /*"assets/minecraft_"*/ STR(829), /*"block/"*/ STR(796));
		if (res == null_)
			return name;
		if (string_pos(/*"blocks/"*/ STR(830), name) == IntType(1))
		{
			name = string_replace(name, /*"blocks/"*/ STR(830), /*"block/"*/ STR(796));
		...
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + string_replace(name, /*"blocks/"*/ STR(830), /*""*/ STR(0)) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + string_replace(name, /*"block/"*/ STR(796), /*""*/ STR(0)) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/"*/ STR(20) + filename_name(name) + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/"*/ STR(831) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/item/"*/ STR(832) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			fn = global::load_folder + /*"/../../textures/block/"*/ STR(833) + name + /*".png"*/ STR(10);
		if (!file_exists_lib(fn))
			return name;
					...
					withOne (obj_block_render_element, (new obj_block_render_element)->id, self->id)
					{
						...
						for (IntType f = IntType(0); f < e_dir_amount; f++)
						{
							...
							else
							{
								...
								if (opaque)
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" opaque"*/ STR(836));
								if (slot < IntType(0))
									slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(837));
								if (slot < IntType(0))
								...
								if (slot < IntType(0))
								{
									...
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_list), texname + /*" noalpha"*/ STR(837));
									if (slot < IntType(0))
										slot = ds_list_find_index((IntType)(ObjType(obj_minecraft_assets, global::mc_assets)->block_texture_ani_list), texname + /*" opaque"*/ STR(836));
									if (slot < IntType(0))
									{
										self->face_render[nd] = false;
										continue;
									...
									if (!is_undefined(col) && col == /*"water"*/ STR(792))
										...
										if (alpha > IntType(0))
										{
											...
											else
											{
												...
												switch ((IntType)self->face_block_vbuffer.Value(nd))
												{
													...
													{
														colstr = /*"grass"*/ STR(838);
														break;
													}
													case e_block_vbuffer_FOLIAGE:
													{
														colstr = /*"foliage"*/ STR(839);
														break;
													}
													case e_block_vbuffer_WATER:
													{
														colstr = /*"water"*/ STR(792);
														break;
													}
