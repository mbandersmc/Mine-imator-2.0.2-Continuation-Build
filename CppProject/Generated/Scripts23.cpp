...
namespace CppProject
{
	...
	{
		if (block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(915)) == /*"true"*/ STR(745))
			return IntType(0);
		...
		if (!sBool(build_edge_zp))
		{
			...
			if (otherblock != null_ && idVar(otherblock, type) == /*"snow"*/ STR(916))
				snowy = /*"true"*/ STR(745);
		}
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"snowy"*/ STR(915), snowy);
		return IntType(0);
	}
	...
	RealType block_set_stairs(ScopeAny self)
	{
		...
		StringType shape;
		VarType half, facing;
		shape = /*"straight"*/ STR(917);
		half = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"half"*/ STR(888));
		facing = block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"facing"*/ STR(873));
		if (!sBool(build_edge_xp) && (facing == /*"east"*/ STR(866) || facing == /*"west"*/ STR(867)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(918))
			{
				IntType stateid = builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(888)) == half)
				{
					VarType otherfacing = block_get_state_id_value(block, stateid, /*"facing"*/ STR(873));
					if (facing == /*"east"*/ STR(866))
					{
						if (otherfacing == /*"south"*/ STR(868))
							shape = /*"outer_right"*/ STR(919);
						else
							if (otherfacing == /*"north"*/ STR(869))
								shape = /*"outer_left"*/ STR(920);
						
					}
					else
					{
						if (otherfacing == /*"south"*/ STR(868))
							shape = /*"inner_left"*/ STR(921);
						else
							if (otherfacing == /*"north"*/ STR(869))
								shape = /*"inner_right"*/ STR(922);
		...
		if (!sBool(build_edge_xn) && (facing == /*"east"*/ STR(866) || facing == /*"west"*/ STR(867)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) - IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(918))
			{
		...
		if (!sBool(build_edge_yp) && (facing == /*"south"*/ STR(868) || facing == /*"north"*/ STR(869)))
		{
			...
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(918))
			{
				...
				if (block_get_state_id_value(block, stateid, /*"half"*/ STR(888)) == half)
				{
					...
					{
						if (otherfacing == /*"east"*/ STR(866))
							shape = /*"outer_left"*/ STR(920);
						else
							if (otherfacing == /*"west"*/ STR(867))
								shape = /*"outer_right"*/ STR(919);
		...
		if (!sBool(build_edge_yn) && (facing == /*"south"*/ STR(868) || facing == /*"north"*/ STR(869)))
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y) - IntType(1), sInt(build_pos_z));
			if (block != null_ && idVar(block, type) == /*"stairs"*/ STR(918))
			{
	...
	RealType block_set_tripwire(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z));
			if (block != null_ && (idVar(block, name) == /*"tripwire"*/ STR(923) || idVar(block, name) == /*"tripwire_hook"*/ STR(924)))
				east = /*"true"*/ STR(745);
		}
	...
	RealType block_set_twisting_vines(ScopeAny self)
	{
		if (!sBool(build_edge_zn))
		{
			...
			if (block == null_ || idVar(block, type) != /*"twisting_vines"*/ STR(925))
				global::vertex_wave_zmin = sInt(block_pos_z);
		}
		else
			global::vertex_wave_zmin = sInt(block_pos_z);
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(783)) == /*"twisting_vines_plant"*/ STR(926))
			return IntType(0);
		if (sInt(build_pos_z) != (sInt(build_size_z) - IntType(1)))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(783), /*"twisting_vines_plant"*/ STR(926) }));
		return IntType(0);
	}
	...
	RealType block_set_vine(ScopeAny self)
	{
		...
		sReal(block_state_id_current) = block_set_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"up"*/ STR(885), up);
		return IntType(0);
	}
	...
	RealType block_set_wall(ScopeAny self)
	{
		...
		states = ArrType::From({ /*"none"*/ STR(875), /*"low"*/ STR(582), /*"tall"*/ STR(927) });
		tall = false;
		i = IntType(0);
		if (!sBool(build_edge_zp))
		{
			...
			if (!sBool(build_edge_xp) && (i == IntType(0) || (i == IntType(1) && east > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_xp) == e_block_depth_DEPTH0 && sBool(block_face_full_xp)))
						...
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x) + IntType(1), sInt(build_pos_y), sInt(build_pos_z) + i), /*"facing"*/ STR(873));
			...
			if (!sBool(build_edge_yp) && (i == IntType(0) || (i == IntType(1) && south > 0)))
			{
				...
				if (block != null_)
				{
					if (idVar(block, type) == idVar(sInt(block_current), type) || (sInt(block_face_min_depth_yp) == e_block_depth_DEPTH0 && sBool(block_face_full_yp)))
						...
						if (idVar(block, type) == /*"fence_gate"*/ STR(903))
						{
							VarType facing = block_get_state_id_value(block, builder_get_state_id(self, sInt(build_pos_x), sInt(build_pos_y) + IntType(1), sInt(build_pos_z) + i), /*"facing"*/ STR(873));
		...
		sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(783), variant, /*"east"*/ STR(866), states.Value(east), /*"west"*/ STR(867), states.Value(west), /*"south"*/ STR(868), states.Value(south), /*"north"*/ STR(869), states.Value(north), /*"up"*/ STR(885), up }));
		return IntType(0);
	}
	...
	RealType block_set_weeping_vines(ScopeAny self)
	{
		...
		{
			IntType block = builder_get_block(self, sInt(build_pos_x), sInt(build_pos_y), sInt(build_pos_z) + IntType(1));
			if (block == null_ || idVar(block, type) != /*"weeping_vines"*/ STR(928))
				global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		}
		else
			global::vertex_wave_zmax = sInt(block_pos_z) + block_size_;
		
		if ((sBool(builder_scenery) && !sBool(builder_scenery_legacy)) || block_get_state_id_value(sInt(block_current), sReal(block_state_id_current), /*"variant"*/ STR(783)) == /*"weeping_vines_plant"*/ STR(929))
			return IntType(0);
		if (sInt(build_pos_z) == IntType(0))
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(783), /*"weeping_vines"*/ STR(928) }));
		else
			sReal(block_state_id_current) = block_get_state_id(sInt(block_current), ArrType::From({ /*"variant"*/ STR(783), /*"weeping_vines_plant"*/ STR(929) }));
	...
	void block_tile_entity_banner(Scope<obj_builder_thread> self, IntType map)
	{
		...
		patterns = DsMap(map).Value(/*"Patterns"*/ STR(930));
		if (ds_map_exists(map, /*"Base"*/ STR(931)))
		{
			base = DsMap(map).Value(/*"Base"*/ STR(931));
			legacy = true;
		}
		else
		{
		...
		if (ds_list_valid(patterns))
		{
			for (IntType i = IntType(0); i < ds_list_size((IntType)(patterns)); i++)
			{
				...
				pattern = value_get_string({ DsMap(patternmap).Value(/*"Pattern"*/ STR(932)), DsList(global::minecraft_pattern_short_list).Value(IntType(1)) });
				color = value_get_real({ DsMap(patternmap).Value(/*"Color"*/ STR(933)), IntType(0) });
		...
		{
			VarType color = block_get_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(887));
	...
	{
		VarType colorid = DsMap(map).Value(/*"color"*/ STR(887));
		if (!is_real(colorid))
			return IntType(0);
		if (is_undefined(colorid) || colorid >= ds_list_size(global::minecraft_color_list))
			return IntType(0);
		RealType newstate = block_set_state_id_value(self->block_current, self->block_state_id_current, /*"color"*/ STR(887), DsList(global::minecraft_color_name_list).Value(colorid));
	...
	RealType block_tile_entity_sign(Scope<obj_builder_thread> self, IntType map)
	{
		...
		IntType color;
		text = /*""*/ STR(0);
		colorname = value_get_string({ DsMap(map).Value(/*"Color"*/ STR(933)), /*"black"*/ STR(600) });
		...
		{
			VarType line = DsMap(map).Value(/*"Text"*/ STR(934) + string(i + IntType(1)));
			if (!is_string(line))
				return IntType(0);
			...
			if (ds_map_valid(textmap))
			{
				...
				if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(935))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(935)))) > IntType(0))
				{
					IntType extramap = ds_list_find_value(DsMap(textmap).Value(/*"extra"*/ STR(935)), IntType(0));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(937);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(938);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(939)));
		map = DsMap(map).Value(/*"Properties"*/ STR(940));
		if (!ds_map_valid(map))
			return IntType(0);
		...
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(941)));
		...
		map = DsMap(map).Value(/*"SKIN"*/ STR(942));
		texurl = DsMap(map).Value(/*"url"*/ STR(943)).Str();
