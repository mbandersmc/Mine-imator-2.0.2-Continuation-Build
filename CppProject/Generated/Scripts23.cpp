...
namespace CppProject
{
	...
	RealType block_tile_entity_sign(Scope<obj_builder_thread> self, IntType map)
	{
		...
		{
			VarType line = DsMap(map).Value(/*"Text"*/ STR(935) + string(i + IntType(1)));
			if (!is_string(line))
				return IntType(0);
			...
			if (ds_map_valid(textmap))
			{
				...
				if (ds_list_valid(DsMap(textmap).Value(/*"extra"*/ STR(936))) && ds_list_size((IntType)(DsMap(textmap).Value(/*"extra"*/ STR(936)))) > IntType(0))
				{
					IntType extramap = ds_list_find_value(DsMap(textmap).Value(/*"extra"*/ STR(936)), IntType(0));
	...
	RealType block_tile_entity_skull(Scope<obj_builder_thread> self, VarType map)
	{
		...
		VarType texlist;
		mapname = /*"Owner"*/ STR(938);
		if (!ds_map_valid(DsMap(map).Value(mapname)))
		{
			mapname = /*"SkullOwner"*/ STR(939);
			if (!ds_map_valid(DsMap(map).Value(mapname)))
				return IntType(0);
		}
		map = DsMap(map).Value(mapname);
		ownerid = string(DsMap(map).Value(/*"Id"*/ STR(940)));
		map = DsMap(map).Value(/*"Properties"*/ STR(941));
		if (!ds_map_valid(map))
			return IntType(0);
		...
		texvalue = base64_decode(DsMap(map).Value(/*"Value"*/ STR(942)));
		...
		map = DsMap(map).Value(/*"SKIN"*/ STR(943));
		texurl = DsMap(map).Value(/*"url"*/ STR(944)).Str();
