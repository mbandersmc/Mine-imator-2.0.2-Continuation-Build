...
namespace CppProject
{
	RealType sortlist_draw(ScopeAny self, IntType slist, VarType xx, VarType yy, VarType w, RealType h, VarType select, BoolType filter, VarType name)
	{
		...
		{
			if (draw_button_icon(self, { /*"listfilter"*/ STR(2999) + string(slist), xx, yy, IntType(24), IntType(24), !ds_list_empty(idInt(slist, filter_list)), icons_FILTER, null_, false, /*"tooltipfilterlist"*/ STR(3000) }))
			{
				menu_settings_set(self, xx, yy, /*"listfilter"*/ STR(2999) + string(slist), IntType(24));
				sInt(settings_menu_script) = ID_sortlist_filters_draw;
				sVar(settings_menu_sortlist) = slist;
				sVar(settings_menu_h_max) = IntType(256);
			}
			if ((sStr(settings_menu_name) == /*"listfilter"*/ STR(2999) + string(slist)) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(725))
		...
		if (draw_textfield(self, /*"listsearch"*/ STR(3001) + string(slist), searchx, yy, searchw, IntType(24), idInt(slist, search_tbx), null_, text_get({ /*"listsearch"*/ STR(3001) }), /*"none"*/ STR(883)))
		{
		...
		if (sVar(window_busy) == /*"sortlist_resize"*/ STR(3002) && sInt(sortlist_resize) == slist)
		{
		...
		for (IntType c = IntType(0); c < idInt(slist, columns); c++)
		{
			...
			if (c > IntType(0) && app_mouse_box(self, xx + dx - IntType(5), yy, IntType(10), colsh) && sBool(content_mouseon))
			{
				...
				if (sBool(mouse_left_pressed))
				{
					...
					sVar(window_busy) = /*"sortlist_resize"*/ STR(3002);
				}
			}
			icon = null_;
			if (idInt(slist, column_sort) == c)
				icon = ((idReal(slist, sort_asc) > 0) ? icons_SORT_UP : icons_SORT_DOWN);
			if (sortlist_draw_button(self, /*"column"*/ STR(3003) + idArr(slist, column_name).Value(c), xx + dx, yy + IntType(4), idArr(slist, column_w).Value(c), colsh, idInt(slist, column_sort) == c, icon, (c == IntType(0)), (c == idInt(slist, columns) - IntType(1)), colmouseon))
			{
				if (idInt(slist, column_sort) == c)
				{
		...
		if (sVar(window_focus) == string(idInt(slist, scroll)))
		{
			...
			if (!app_mouse_box(self, xx, yy, w, h) && sBool(content_mouseon) && sBool(mouse_left) && sVar(window_busy) != /*"scrollbar"*/ STR(2663))
	...
	RealType sortlist_filters_draw(ScopeAny self)
	{
		...
		for (IntType i = IntType(0); i < ds_list_size(typelist); i++)
		{
			...
			{
				if (itemname == /*"packunzipped"*/ STR(542) || itemname == /*"legacyblocksheet"*/ STR(545) || itemname == /*"fromworld"*/ STR(547))
					continue;
			}
			capwid = max({ capwid, string_width(text_get({ /*"type"*/ STR(780) + itemname })) });
			...
			if (draw_checkbox(self, /*"type"*/ STR(780) + itemname, sVar(dx), sVar(dy) + scroll, active, null_))
			{
				if (active)
					...
					for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_block, block)->default_state)) && !match; i += IntType(2))
					{
						...
						for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
							if (string_count(check, string_lower(minecraft_asset_get_name(/*"blockstatevalue"*/ STR(5), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
								match = true;
						if (string_count(check, string_lower(text_get({ /*"blockstate"*/ STR(771) + state }))) > IntType(0))
							match = true;
					}
						...
						for (IntType i = IntType(0); i < array_length(VarType::CreateRef(ObjType(obj_model, model)->default_state)) && !match; i += IntType(2))
						{
							...
							for (IntType s = IntType(0); s < idReal(statecurrent, value_amount) && !match; s++)
								if (string_count(check, string_lower(minecraft_asset_get_name(/*"modelstatevalue"*/ STR(9), idVar(statecurrent, value_name).Value(s)))) > IntType(0))
									match = true;
							if (string_count(check, string_lower(text_get({ /*"modelstate"*/ STR(750) + state }))) > IntType(0))
								match = true;
						}
	...
	{
		return string_replace_all(string_upper(string_char_at(str, IntType(1))) + string_delete(str, IntType(1), IntType(1)), /*"_"*/ STR(1129), /*" "*/ STR(21));
	}
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1108), /*""*/ STR(0)), /*"."*/ STR(1106), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(3004)))
			str = string_replace(str, /*"-."*/ STR(3004), /*"-0."*/ STR(3005));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(3007)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(948))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(3007)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(3007);
					c = linestart;
					break;
				}
			...
			{
				if (char_ == /*" "*/ STR(21) || char_ == /*", "*/ STR(1032) || char_ == /*"."*/ STR(1106) || char_ == /*"/"*/ STR(20) || char_ == /*"-"*/ STR(1108))
	...
	ArrType string_split(VarType str, StringType sep)
	{
		...
		while (true)
		{
			...
			if (pos == IntType(0))
				break;
			IntType escapepos = string_pos(/*"\\"*/ STR(1052) + sep, str);
