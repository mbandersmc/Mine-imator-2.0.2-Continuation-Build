...
namespace CppProject
{
	void view_control_camera(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && (sVar(view_control_edit) == e_view_control_ROT_ANGLE_XY || sVar(view_control_edit) == e_view_control_ROT_ANGLE_Z))
		{
			...
			if (!(sInt(mouse_still) > 0))
			{
				...
				if (sVar(view_control_edit) == e_view_control_ROT_ANGLE_XY)
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit) == e_view_control_ROT_DISTANCE)
		{
			sInt(mouse_cursor) = cr_handpoint;
	...
	void view_control_move(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && sVar(view_control_edit) >= e_view_control_POS_XY && sVar(view_control_edit) <= e_view_control_POS_PAN)
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
				...
				{
					if (i == Z_ && sVar(view_control_edit) == e_view_control_POS_XY)
						continue;
					else
						if (i == Y_ && sVar(view_control_edit) == e_view_control_POS_XZ)
							continue;
						else
							if (i == X_ && sVar(view_control_edit) == e_view_control_POS_YZ)
								continue;
			...
			if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && sVar(view_control_edit) >= e_view_control_POS_X && sVar(view_control_edit) <= e_view_control_POS_Z)
			{
				sInt(mouse_cursor) = cr_handpoint;
				RealType veclen = vec2_length(sVec(view_control_vec));
				if (veclen > IntType(0) && !(sInt(mouse_still) > 0))
				{
					...
					move = vec3(IntType(0));
					global::axis_edit = sVar(view_control_edit) - e_view_control_POS_X;
					vecmouse = vec2(sReal(mouse_dx), sReal(mouse_dy));
					vecdot = vec2_dot(vec2_normalize(sVec(view_control_vec)), vec2_normalize(vecmouse));
	...
	RealType view_control_move_axis(ScopeAny self, VarArgs argument)
	{
		IntType argument_count = argument.Size();
		VarType view, control, vid, color, start3D, end3D, fade;
		VarType center3D;
		ArrType center2D;
		VecType start2D, end2D;
		view = argument[IntType(0)];
		control = argument[IntType(1)];
		vid = argument[IntType(2)];
		color = argument[IntType(3)];
		start3D = argument[IntType(4)];
		end3D = argument[IntType(5)];
		fade = true;
		if (argument_count > IntType(6))
			fade = argument[IntType(6)];
		...
		if ((sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit) == control) || idVar(view, control_mouseon_last) == control || !(sVar(setting_fade_gizmos) > 0))
			alpha = IntType(1);
		if (alpha == IntType(0))
			return IntType(0);
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31))
		{
			...
			if (idVar(view, control_mouseon_last) == control)
			{
		...
		if (sInt(place_tl) == null_ && sBool(content_mouseon) && point_line_distance(start2D.Real(X_), start2D.Real(Y_), end2D.Real(X_), end2D.Real(Y_), gmlGlobal::mouse_x - sVar(content_x), gmlGlobal::mouse_y - sVar(content_y)) < view_3d_control_width / 2.0)
			idVar(view, control_mouseon) = control;
		draw_set_color(c_white);
		draw_set_alpha(1.0);
		return 0.0;
	}
	...
	RealType view_control_move_pan(ScopeAny self, IntType view, RealType radius)
	{
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31))
		{
			if (sVar(view_control_edit) != e_view_control_POS_PAN || sVar(view_control_edit_view) != view)
				return IntType(0);
			...
			if (ObjType(obj_view, view)->control_mouseon_last == e_view_control_POS_PAN)
			{
				...
				{
					sVar(window_busy) = /*"rendercontrol"*/ STR(31);
					sVar(view_control_edit) = e_view_control_POS_PAN;
					sVar(view_control_edit_view) = view;
	...
	RealType view_control_move_plane(ScopeAny self, IntType view, IntType control, VecType axes, IntType color, MatrixType mat, VecType normal, VecType corner1, VecType corner2, VecType corner3, VecType corner4)
	{
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit) == control || !(sVar(setting_fade_gizmos) > 0))
			alpha = IntType(1);
	...
	void view_control_rotate(ScopeAny self, IntType view)
	{
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && sVar(view_control_edit) >= e_view_control_ROT_X && sVar(view_control_edit) <= e_view_control_ROT_Z)
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
				RealType ang, prevang, rot, snapval;
				VarType axesang, newval;
				global::axis_edit = sVar(view_control_edit) - e_view_control_ROT_X;
				ang = point_direction(gmlGlobal::mouse_x - sVar(content_x), gmlGlobal::mouse_y - sVar(content_y), sVec(view_control_pos).Real(X_), sVec(view_control_pos).Real(Y_));
				prevang = point_direction(sVar(mouse_previous_x) - sVar(content_x), sVar(mouse_previous_y) - sVar(content_y), sVec(view_control_pos).Real(X_), sVec(view_control_pos).Real(Y_));
				rot = angle_difference_fix(ang, prevang) * negate(sReal(view_control_flip));
				sReal(view_control_move_distance) += rot * sReal(dragger_multiplier);
	...
	RealType view_control_rotate_axis(ScopeAny self, IntType view, IntType control, IntType vid, IntType color, MatrixType mat, VarType len)
	{
		...
		for (RealType i = IntType(0); i <= IntType(1); i += 1.0 / detail)
		{
			...
			if (sVar(view_control_edit) != control)
			{
	...
	void view_control_scale(ScopeAny self, IntType view)
	{
		...
		if (sVar(view_control_matrix) != null_ && sVar(view_control_edit) != null_)
		...
		if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && sVar(view_control_edit) >= e_view_control_SCA_XY && sVar(view_control_edit) <= e_view_control_SCA_YZ)
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
				...
				for (IntType i = X_; i <= Z_; i++)
				{
					if (i == Z_ && sVar(view_control_edit) == e_view_control_SCA_XY)
						continue;
					else
						if (i == Y_ && sVar(view_control_edit) == e_view_control_SCA_XZ)
							continue;
						else
							if (i == X_ && sVar(view_control_edit) == e_view_control_SCA_YZ)
								continue;
			...
			if (sVar(window_busy) == /*"rendercontrol"*/ STR(31) && sVar(view_control_edit_view) == view && sVar(view_control_edit) >= e_view_control_SCA_X && sVar(view_control_edit) <= e_view_control_SCA_Z)
			{
				sInt(mouse_cursor) = cr_handpoint;
				RealType veclen = vec2_length(sVec(view_control_vec));
				if (veclen > IntType(0) && !(sInt(mouse_still) > 0))
				{
					...
					RealType vecdot, snapval;
					VarType move, newval;
					global::axis_edit = sVar(view_control_edit) - e_view_control_SCA_X;
					vecmouse = vec2(sReal(mouse_dx), sReal(mouse_dy));
					vecdot = vec2_dot(vec2_normalize(sVec(view_control_vec)), vec2_normalize(vecmouse));
		...
		if (sVar(view_control_edit_view) == view && sVar(view_control_edit) == e_view_control_SCA_XYZ)
		{
			sInt(mouse_cursor) = cr_handpoint;
			if (!(sInt(mouse_still) > 0))
			{
