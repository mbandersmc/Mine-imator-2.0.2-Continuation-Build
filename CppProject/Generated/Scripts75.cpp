...
namespace CppProject
{
	RealType window_draw_new_assets(ScopeAny self)
	{
		...
		draw_label(self, { text_get({ /*"newassetstitle"*/ STR(3988), sStr(new_assets_version) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(227), fa_center, fa_bottom, global::c_accent, IntType(1), sInt(font_heading) });
		draw_label(self, { text_get({ /*"newassetssubtitle"*/ STR(3989) }), sVar(dx) + (RealType)sVar(dw) / 2.0, sVar(dy) + IntType(244), fa_center, fa_bottom, global::c_text_main, global::a_text_main, sInt(font_value) });
		sVar(dy) += IntType(264);
		sVar(dx) += IntType(12);
		sVar(dw) -= 24.0;
		if (sStr(new_assets_stage) == /*"download"*/ STR(109))
		{
			...
			draw_loading_bar(self, sVar(dx), sVar(dy), sVar(dw), IntType(8), sReal(new_assets_download_progress), text_get({ /*"newassetsdownloading"*/ STR(3990) }));
			tab_next(self);
		}
		else
		{
			draw_set_font(sInt(font_button));
			IntType capwid = string_width(text_get({ /*"newassetsdownload"*/ STR(3991) })) + button_padding;
			if (draw_button_label(self, /*"newassetsdownload"*/ STR(3991), sVar(dx) + sVar(dw), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_PRIMARY, null_, fa_right))
			{
				if (sVar(new_assets_format) > minecraft_assets_format)
				{
					if (question(text_get({ /*"questionassetsnewer"*/ STR(3992) })))
					{
			...
			if (draw_button_label(self, /*"newassetslater"*/ STR(3993), (sVar(dx) + sVar(dw)) - (capwid + IntType(8)), (sVar(content_y) + sVar(content_height)) - IntType(44), null_, null_, e_button_SECONDARY, null_, fa_right))
			{
			...
			sVar(dy_start) = sVar(dy);
			sVar(dy) += IntType(16);
			draw_label(self, { text_get({ /*"newassetschangelog"*/ STR(3994) }), sVar(dx), sVar(dy), fa_left, fa_bottom, global::c_text_tertiary, global::a_text_tertiary, sInt(font_subheading) });
			sVar(dy) += IntType(20);
			for (IntType i = IntType(0); i < array_length(VarType::CreateRef(sArr(new_assets_changes_lines))); i++)
			{
				...
				if (string_copy(text, IntType(1), IntType(2)) == /*"- "*/ STR(3995))
				{
				...
				{
					if (string_copy(text, IntType(1), IntType(2)) == /*"* "*/ STR(3996))
						text = /*"• "*/ STR(3997) + string_delete(text, IntType(1), IntType(2));
	...
	void window_draw_startup(ScopeAny self)
	{
		...
		StringType trial = ((global::trial_version > 0) ? /*" "*/ STR(21) + text_get({ /*"startuptrial"*/ STR(1978) }) : /*""*/ STR(0));
		draw_button_text(self, { text_get({ /*"startupversion"*/ STR(3998), mineimator_version_full + trial }), (sReal(window_width) / 2.0) + IntType(259), floor((headersize / 2.0) + ((RealType)sprite_get_height(ID_spr_logo) / 2.0)) + IntType(3), ID_popup_switch, sInt(popup_about) });
		...
		{
			draw_label(self, { text_get({ /*"recentnone"*/ STR(3999) }), sReal(window_width) / 2.0, sVar(dy), fa_center, fa_middle, global::c_accent, IntType(1), sInt(font_heading_big) });
			sVar(dy) += IntType(48);
		}
		draw_set_font(sInt(font_button));
		RealType newprojectwidth, browsewidth, centerx;
		newprojectwidth = string_width(text_get({ /*"startupnewproject"*/ STR(4000) })) + button_icon_padding;
		browsewidth = string_width(text_get({ /*"startupbrowse"*/ STR(4001) })) + button_icon_padding;
		...
		if (draw_button_label(self, /*"startupnewproject"*/ STR(4000), sVar(dx), sVar(dy), null_, icons_FILE))
		{
		...
		if (draw_button_label(self, /*"startupbrowse"*/ STR(4001), sVar(dx), sVar(dy), null_, icons_FOLDER, e_button_SECONDARY))
		{
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			if (draw_button_icon(self, { /*"startuprecentdisplay"*/ STR(4002), sVar(dx) - IntType(24) - IntType(8), sVar(dy) + IntType(4), IntType(24), IntType(24), false, sStr(recent_display_mode) == /*"grid"*/ STR(618) ? icons_VIEW_LIST : icons_VIEW_GRID, null_, false, sStr(recent_display_mode) == /*"grid"*/ STR(618) ? /*"tooltipviewlist"*/ STR(4003) : /*"tooltipviewgrid"*/ STR(4004) }))
			{
				if (sStr(recent_display_mode) == /*"list"*/ STR(1097))
					sStr(recent_display_mode) = /*"grid"*/ STR(618);
				else
					sStr(recent_display_mode) = /*"list"*/ STR(1097);
		...
		if (sReal(recent_list_amount) > IntType(0))
		{
			...
			draw_label(self, { text_get({ /*"startuprecentprojects"*/ STR(4005) }), sVar(dx), sVar(dy) + IntType(16), fa_left, fa_middle, global::c_accent, IntType(1) });
			IntType labelwid = string_width(text_get({ /*"startuprecentprojects"*/ STR(4005) }));
			if (draw_button_label(self, /*"startupsortby"*/ STR(1478), sVar(dx) + labelwid + IntType(16), sVar(dy), null_, icons_SORT_DOWN, e_button_TERTIARY))
				menu_settings_set(self, sVar(dx) + labelwid + IntType(16), sVar(dy), /*"startupsortby"*/ STR(1478), IntType(32));
			if (sStr(settings_menu_name) == /*"startupsortby"*/ STR(1478) && sStr(settings_menu_ani_type) != /*"hide"*/ STR(725))
		...
		else
		{
			...
			midy = snap((IntType(2) * headersize) + ((sReal(window_height) - (IntType(2) * headersize)) / 2.0), IntType(2));
			if ((midy + ((RealType)sprite_get_height(ID_spr_jonathan_splash) / 2.0)) <= sReal(window_height))
	...
	void window_draw_toasts(ScopeAny self)
	{
		...
		if (sInt(popup) > 0 && busy == /*"popup"*/ STR(1611) + idVar(sInt(popup), name))
			sVar(window_busy) = /*""*/ STR(0);
		for (RealType i = sInt(toast_amount) - IntType(1); i >= IntType(0); i--)
		{
			toast = DsList(sInt(toast_list)).Value(i);
			draw_set_alpha(ease(/*"easeoutcirc"*/ STR(574), ObjType(obj_toast, toast)->remove_alpha));
	...
	{
		global::_app->world_import_settings_popup = new_popup({ /*"worldsettings"*/ STR(4006), ID_popup_worldsettings_draw, IntType(600), IntType(532), true, false, false, true, ID_world_import_apply_settings });
		...
		sortlist_column_add(global::_app->world_import_settings_block_list, /*"blockfilter"*/ STR(2996), IntType(0));
		...
		sortlist_column_add(global::_app->world_import_settings_filter_list, /*"blockfilter"*/ STR(2996), IntType(0));
	...
	{
		sStr(window_state) = /*"world_import"*/ STR(2763);
		sStr(popup_ani_type) = /*""*/ STR(0);
		sStr(world_import_world_root) = /*""*/ STR(0);
		sVar(world_import_world_name) = text_get({ /*"worldimportnoworld"*/ STR(4007) });
		sStr(world_import_dimension) = /*"overworld"*/ STR(4008);
	...
	{
		menu_add_item({ /*"overworld"*/ STR(4008), text_get({ /*"worldimportoverworld"*/ STR(4018) }) });
		if (world_import_has_dimension(/*"nether"*/ STR(4019)))
			menu_add_item({ /*"nether"*/ STR(4019), text_get({ /*"worldimportnether"*/ STR(4020) }) });
		if (world_import_has_dimension(/*"end"*/ STR(4021)))
			menu_add_item({ /*"end"*/ STR(4021), text_get({ /*"worldimportend"*/ STR(4022) }) });
	}
	...
	void window_draw_world_import(ScopeAny self)
	{
		...
		if (hasselection)
		{
			...
			confirmw = string_width(text_get({ /*"worldimportconfirm"*/ STR(4023) })) + IntType(70);
		}
		else
			confirmw = IntType(0);
		...
		sBool(content_mouseon) = true;
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportcancel"*/ STR(4024) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportcancel"*/ STR(4024), sVar(dx), sVar(content_y) + IntType(4), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT))
			world_import_cancel();
		sVar(dx) += sVar(dw);
		sVar(dx) += IntType(12);
		...
		sVar(dw) = IntType(256);
		capwid = IntType(50);
		draw_button_menu(self, { /*"worldimportworld"*/ STR(1490), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_world_root), sVar(world_import_world_name), ID_world_import_select_world, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = IntType(208);
		capwid = IntType(80);
		draw_button_menu(self, { /*"worldimportdimension"*/ STR(1491), e_menu_LIST, sVar(dx), sVar(dy), sVar(dw), IntType(24), sStr(world_import_dimension), text_get({ /*"worldimport"*/ STR(2988) + sStr(world_import_dimension) }), ID_world_import_select_dimension, false, null_, null_, /*""*/ STR(0), null_, null_, capwid });
		sVar(dx) += sVar(dw) + IntType(12);
		sVar(dw) = IntType(24);
		spacing = IntType(4);
		if (draw_button_icon(self, { /*"worldimportbrowse"*/ STR(4025), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_FOLDER, null_, false, /*"worldimportbrowsetip"*/ STR(4026) }))
		{
			StringType leveldat = file_dialog_open(text_get({ /*"worldimportbrowseworlds"*/ STR(4027) }) + /*" (level.dat)|level.dat;"*/ STR(4028), /*""*/ STR(0), world_import_get_saves_dir(), text_get({ /*"worldimportbrowsecaption"*/ STR(4029) }));
		...
		if (draw_button_icon(self, { /*"worldimportreload"*/ STR(4030), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_REFRESH, null_, !worldpicked, /*"worldimportreloadtip"*/ STR(4031) }))
			world_import_select_world(self, sStr(world_import_world_root), sStr(world_import_dimension));
		sVar(dx) += sVar(dw) + IntType(12);
		draw_divide_vertical(sVar(dx), sVar(content_y) + IntType(6), sVar(content_height) - IntType(12));
		sVar(dx) += IntType(12);
		if (draw_button_icon(self, { /*"worldimportgotoplayer"*/ STR(4032), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_PATH_POINT, null_, !worldpicked, /*"worldimportgotoplayertip"*/ STR(4033) }))
			world_import_go_to_player();
		sVar(dx) += sVar(dw) + spacing;
		if (draw_button_icon(self, { /*"worldimportsettings"*/ STR(4034), sVar(dx), sVar(dy), sVar(dw), sVar(dw), false, icons_SETTINGS, null_, false, /*"worldimportsettingstip"*/ STR(4035) }))
		...
		spacing = IntType(12);
		sVar(dw) = string_width(text_get({ /*"worldimportselection"*/ STR(2991) }) + /*":"*/ STR(763));
		draw_label(self, { text_get({ /*"worldimportselection"*/ STR(2991) }) + /*":"*/ STR(763), sVar(dx), sVar(content_y) + (RealType)sVar(content_height) / 2.0, fa_left, fa_middle, global::c_text_secondary, global::a_text_secondary });
		sVar(dx) += sVar(dw) + spacing;
		draw_set_font(sInt(font_button));
		sVar(dw) = string_width(text_get({ /*"worldimportselectionsmall"*/ STR(4036) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionsmall"*/ STR(4036), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"small"*/ STR(4037));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionmedium"*/ STR(4038) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionmedium"*/ STR(4038), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"medium"*/ STR(602));
		sVar(dx) += sVar(dw) + spacing;
		sVar(dw) = string_width(text_get({ /*"worldimportselectionlarge"*/ STR(4039) })) + IntType(24);
		if (draw_button_label(self, /*"worldimportselectionlarge"*/ STR(4039), sVar(dx), sVar(dy), null_, null_, e_button_SECONDARY, null_, e_anchor_LEFT, !worldpicked))
			world_import_set_selection(/*"large"*/ STR(4040));
		if (world_import_has_selection())
		{
			sVar(dx) += sVar(dw) + IntType(20);
			VecType size = world_import_get_selection_size();
			draw_label(self, { text_get({ /*"worldimportblockstotal"*/ STR(4041), size.Real(X_) * size.Real(Y_) * size.Real(Z_) }), sVar(dx), (sVar(content_y) + (RealType)sVar(content_height) / 2.0) - IntType(8), fa_left, fa_middle, global::c_text_main, global::a_text_main, sInt(font_value) });
			draw_label(self, { text_get({ /*"worldimportblockssizes"*/ STR(4042), size.Real(X_), size.Real(Y_), size.Real(Z_) }), sVar(dx), (sVar(content_y) + (RealType)sVar(content_height) / 2.0) + IntType(8), fa_left, fa_middle, global::c_text_main, global::a_text_main, sInt(font_value) });
		}
		sVar(content_x) = IntType(0);
		sVar(content_y) = IntType(0);
		sVar(content_width) = sReal(window_width);
		sVar(content_height) = sReal(window_height);
		if (hasselection)
			if (draw_button_label(self, /*"worldimportconfirm"*/ STR(4023), confirmx, confirmy, confirmw, null_, e_button_BIG, null_, e_anchor_LEFT))
		...
		{
			VarType filtertext = text_get({ /*"worldimportfilteractive"*/ STR(4043) });
			...
			draw_label(self, { filtertext, confirmx + confirmw / 2.0, confirmy + IntType(80), fa_center, fa_top, global::c_warning, IntType(1), sInt(font_heading_big) });
		}
	}
	...
	void popup_worldsettings_draw(ScopeAny self)
	{
		...
		draw_switch(self, /*"worldsettingsunloadregions"*/ STR(4044), sVar(dx), sVar(dy), sVar(setting_world_import_unload_regions), ID_action_world_import_settings_unload_regions, /*"worldsettingsunloadregionstip"*/ STR(4045));
		tab_next(self);
		tab_control_switch(self);
		draw_switch(self, /*"worldsettingsfilterenabled"*/ STR(4046), sVar(dx), sVar(dy), sVar(setting_world_import_filter_enabled), ID_action_world_import_settings_filter_enabled);
		tab_next(self);
		if (sVar(setting_world_import_filter_enabled) > 0)
		{
			tab_control_togglebutton(self);
			togglebutton_add(self, { /*"worldsettingsfilterremove"*/ STR(4047), null_, IntType(0), sVar(setting_world_import_filter_mode) == IntType(0), ID_action_world_import_settings_filter_mode });
			togglebutton_add(self, { /*"worldsettingsfilterkeep"*/ STR(4048), null_, IntType(1), sVar(setting_world_import_filter_mode) == IntType(1), ID_action_world_import_settings_filter_mode });
			draw_togglebutton(self, /*"worldsettingsfiltermode"*/ STR(4049), sVar(dx), sVar(dy));
			tab_next(self);
			RealType listdw = sVar(content_width) - IntType(24);
			sVar(dy) += IntType(8);
			tab_control_sortlist(self, IntType(10));
			sortlist_draw(self, sInt(world_import_settings_block_list), sVar(dx), sVar(dy), listdw / 2.0 - IntType(20), sReal(tab_control_h), sInt(world_import_settings_block_select), false, text_get({ /*"worldsettingsfilterblocks"*/ STR(4050) }));
			sortlist_draw(self, sInt(world_import_settings_filter_list), sVar(dx) + listdw / 2.0 + IntType(20), sVar(dy), listdw / 2.0 - IntType(20), sReal(tab_control_h), sInt(world_import_settings_filter_select), false, text_get({ /*"worldsettingsfilterfiltered"*/ STR(4051) }));
			if (draw_button_icon(self, { /*"worldsettingsfilterright"*/ STR(4052), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + (sReal(tab_control_h) / 2.0) - IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_RIGHT_TINY, null_, sInt(world_import_settings_block_select) == null_ }))
			{
			...
			if (draw_button_icon(self, { /*"worldsettingsfilterleft"*/ STR(4053), sVar(dx) + listdw / 2.0 - IntType(12), sVar(dy) + (sReal(tab_control_h) / 2.0) + IntType(16), IntType(24), IntType(24), false, icons_CHEVRON_LEFT_TINY, null_, sInt(world_import_settings_filter_select) == null_ }))
			{
			...
			sVar(dy) += IntType(8);
			draw_tooltip_label(self, /*"worldsettingsfilterhelp"*/ STR(4054), icons_INFO, e_toast_INFO);
		}
	}
	
	StringType yesno(RealType val)
	{
		return ((val > 0) ? /*"yes"*/ STR(4055) : /*"no"*/ STR(4056));
	}
