...
namespace CppProject
{
	...
	VarType string_get_real(VarType str, VarType inv)
	{
		...
		if (string_replace(string_replace(str, /*"-"*/ STR(1075), /*""*/ STR(0)), /*"."*/ STR(1073), /*""*/ STR(0)) == /*""*/ STR(0))
			return inv;
		if (string_contains(str, /*"-."*/ STR(2894)))
			str = string_replace(str, /*"-."*/ STR(2894), /*"-0."*/ STR(2895));
		if (state >= IntType(2))
			return real(str);
		return inv;
	}
	...
	VarType string_get_state_vars(VarType argument0)
	{
		...
		{
			ArrType nameval = string_split(vars.Value(i), /*"="*/ STR(2896));
	...
	StringType string_limit_ext(VarType str, VarType w, VarType h)
	{
		...
		dx = IntType(0);
		dy = IntType(0);
		if (lh > h || string_width(/*"..."*/ STR(2897)) > w)
			return /*""*/ STR(0);
		linestart = IntType(1);
		wordstart = IntType(1);
		for (c = IntType(1); c <= string_length(str); c++)
		{
			...
			if (char_ == /*"\n"*/ STR(937))
			{
				...
				if (dy + lh > h)
				{
					while (c > linestart && string_width(string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2897)) > w)
						c--;
					
					nstr += string_copy(str, linestart, (IntType)(c - linestart)) + /*"..."*/ STR(2897);
					c = linestart;
					break;
				}
